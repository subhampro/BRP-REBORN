...
AddEventHandler('INDCore:Client:OnPlayerLoaded', function()
    ...
    INDCore.Functions.TriggerCallback("weapons:server:GetConfig", function(RepairPoints)
...
RegisterNetEvent("addAttachment", function(component)
    ...
    local WeaponData = INDCore.Shared.Weapons[weapon]
...
RegisterNetEvent('weapons:client:AddAmmo', function(type, amount, itemData)
    ...
    if CurrentWeaponData then
        if INDCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and INDCore.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            local total = GetAmmoInPedWeapon(ped, weapon)
            local _, maxAmmo = GetMaxAmmo(ped, weapon)
            if total < maxAmmo then
                INDCore.Functions.Progressbar("taking_bullets", Lang:t('info.loading_bullets'), Config.ReloadTime, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Done
                    if INDCore.Shared.Weapons[weapon] then
                        ...
                        TriggerEvent('inventory:client:ItemBox', INDCore.Shared.Items[itemData.name], "remove")
                        TriggerEvent('INDCore:Notify', Lang:t('success.reloaded'), "success")
                    end
                end, function()
                    INDCore.Functions.Notify(Lang:t('error.canceled'), "error")
                end)
            else
                INDCore.Functions.Notify(Lang:t('error.max_ammo'), "error")
            end
        else
            INDCore.Functions.Notify(Lang:t('error.no_weapon'), "error")
        end
    else
        INDCore.Functions.Notify(Lang:t('error.no_weapon'), "error")
    end
    ...
    if weapon ~= `WEAPON_UNARMED` then
        ...
        if WeaponAttachments[WeaponData.name] then
            if WeaponAttachments[WeaponData.name][attachment]['item'] == ItemData.name then
                ...
                INDCore.Functions.Notify(Lang:t('error.no_support_attachment'), "error")
            end
        end
    else
        INDCore.Functions.Notify(Lang:t('error.no_weapon_in_hand'), "error")
    end
end)
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if CurrentWeaponData and next(CurrentWeaponData) then
                if IsPedShooting(ped) or IsControlJustPressed(0, 24) then
                    local weapon = GetSelectedPedWeapon(ped)
                    if CanShoot then
                        if weapon and weapon ~= 0 and INDCore.Shared.Weapons[weapon] then
                            INDCore.Functions.TriggerCallback('prison:server:checkThrowable', function(result)
                        ...
                        if weapon ~= `WEAPON_UNARMED` then
                            TriggerEvent('inventory:client:CheckWeapon', INDCore.Shared.Weapons[weapon]["name"])
                            INDCore.Functions.Notify(Lang:t('error.weapon_broken'), "error")
                            MultiplierAmount = 0
                        end
                    end
                end
                ...
                if distance < 10 then
                    ...
                    if distance < 1 then
                        if data.IsRepairing then
                            ...
                            if CurrentWeaponData and next(CurrentWeaponData) then
                                if not data.RepairingData.Ready then
                                    local WeaponData = INDCore.Shared.Weapons[GetHashKey(CurrentWeaponData.name)]
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        INDCore.Functions.TriggerCallback('weapons:server:RepairWeapon', function(HasMoney)
                                            if HasMoney then
                                                CurrentWeaponData = {}
                                            end
