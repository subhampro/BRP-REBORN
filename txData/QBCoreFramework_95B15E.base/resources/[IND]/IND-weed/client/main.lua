...
RegisterNetEvent('IND-weed:client:getHousePlants', function(house)
    INDCore.Functions.TriggerCallback('IND-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
                ...
                for k, _ in pairs(housePlants[currentHouse]) do
                    ...
                    if plyDistance < 0.8 then
                        ...
                        if plantData["plantStats"]["health"] > 0 then
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    INDCore.Functions.Progressbar("remove_weed_plant", Lang:t('text.harvesting_plant'), 8000, false, true, {
                                        ...
                                        TriggerServerEvent('IND-weed:server:harvestPlant', currentHouse, amount, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                                    end, function() -- Cancel
                                        ClearPedTasks(ped)
                                        INDCore.Functions.Notify("Process Canceled", "error")
                                    end)
                                end
                            end
                        elseif plantData["plantStats"]["health"] == 0 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                INDCore.Functions.Progressbar("remove_weed_plant", Lang:t('text.removing_the_plant'), 8000, false, true, {
                                    disableMovement = true,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true,
                                    ...
                                    ClearPedTasks(ped)
                                    TriggerServerEvent('IND-weed:server:removeDeathPlant', currentHouse, plantData["plantStats"]["plantId"])
                                end, function() -- Cancel
                                    ClearPedTasks(ped)
...
RegisterNetEvent('IND-weed:client:leaveHouse', function()
    despawnHousePlants()
...
RegisterNetEvent('IND-weed:client:refreshHousePlants', function(house)
...
RegisterNetEvent('IND-weed:client:refreshPlantStats', function()
    if insideHouse then
        despawnHousePlants()
        SetTimeout(100, function()
            INDCore.Functions.TriggerCallback('IND-weed:server:getBuildingPlants', function(plants)
...
RegisterNetEvent('IND-weed:client:placePlant', function(type, item)
    local ped = PlayerPedId()
	...
	LocalPlayer.state:set("inv_busy", true, true)
            INDCore.Functions.Progressbar("plant_weed_plant", Lang:t('text.planting'), 8000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
		...
		LocalPlayer.state:set("inv_busy", false, true)
                ...
                ClearPedTasks(ped)
                TriggerServerEvent('IND-weed:server:placePlant', json.encode(plantData["plantCoords"]), type, currentHouse)
                TriggerServerEvent('IND-weed:server:removeSeed', item.slot, type)
            end, function() -- Cancel
                ClearPedTasks(ped)
                INDCore.Functions.Notify(Lang:t('error.process_canceled'), "error")
		LocalPlayer.state:set("inv_busy", false, true)
            ...
            INDCore.Functions.Notify(Lang:t('error.cant_place_here'), 'error', 3500)
        end
    else
        INDCore.Functions.Notify(Lang:t('error.not_safe_here'), 'error', 3500)
    end
end)

RegisterNetEvent('IND-weed:client:foodPlant', function()
    if currentHouse ~= nil then
        if ClosestTarget ~= 0 then
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    INDCore.Functions.Notify(Lang:t('error.not_need_nutrition'), 'error', 3500)
                else
		            LocalPlayer.state:set("inv_busy", true, true)
                    INDCore.Functions.Progressbar("plant_weed_plant", Lang:t('text.feeding_plant'), math.random(4000, 8000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        ...
                        TriggerServerEvent('IND-weed:server:foodPlant', currentHouse, newFood, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                    end, function() -- Cancel
                        ClearPedTasks(ped)
			            LocalPlayer.state:set("inv_busy", false, true)
                        INDCore.Functions.Notify(Lang:t('error.process_canceled'), "error")
                ...
                INDCore.Functions.Notify(Lang:t('error.cant_place_here'), "error")
            end
        else
            INDCore.Functions.Notify(Lang:t('error.cant_place_here'), "error")
        end
