...
INDCore.Functions.CreateCallback('IND-weed:server:getBuildingPlants', function(_, cb, building)
...
RegisterNetEvent('IND-weed:server:placePlant', function(coords, sort, currentHouse)
    ...
    TriggerClientEvent('IND-weed:client:refreshHousePlants', -1, currentHouse)
end)

RegisterNetEvent('IND-weed:server:removeDeathPlant', function(building, plantId)
    MySQL.query('DELETE FROM house_plants WHERE plantid = ? AND building = ?', {plantId, building})
    TriggerClientEvent('IND-weed:client:refreshHousePlants', -1, building)
end)
...
CreateThread(function()
    while true do
        ...
        TriggerClientEvent('IND-weed:client:refreshPlantStats', -1)
        Wait((60 * 1000) * 19.2)
    end
end)
...
INDCore.Functions.CreateUseableItem("weed_white-widow_seed", function(source, item)
    TriggerClientEvent('IND-weed:client:placePlant', source, 'white-widow', item)
end)

INDCore.Functions.CreateUseableItem("weed_skunk_seed", function(source, item)
    TriggerClientEvent('IND-weed:client:placePlant', source, 'skunk', item)
end)

INDCore.Functions.CreateUseableItem("weed_purple-haze_seed", function(source, item)
    TriggerClientEvent('IND-weed:client:placePlant', source, 'purple-haze', item)
end)

INDCore.Functions.CreateUseableItem("weed_og-kush_seed", function(source, item)
    TriggerClientEvent('IND-weed:client:placePlant', source, 'og-kush', item)
end)

INDCore.Functions.CreateUseableItem("weed_amnesia_seed", function(source, item)
    TriggerClientEvent('IND-weed:client:placePlant', source, 'amnesia', item)
end)

INDCore.Functions.CreateUseableItem("weed_ak47_seed", function(source, item)
    TriggerClientEvent('IND-weed:client:placePlant', source, 'ak47', item)
end)

INDCore.Functions.CreateUseableItem("weed_nutrition", function(source, item)
    TriggerClientEvent('IND-weed:client:foodPlant', source, item)
end)

RegisterServerEvent('IND-weed:server:removeSeed', function(itemslot, seed)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    Player.Functions.RemoveItem(seed, 1, itemslot)
end)

RegisterNetEvent('IND-weed:server:harvestPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
            ...
            if house ~= nil then
                ...
                if result[1] ~= nil then
                    ...
                    MySQL.query('DELETE FROM house_plants WHERE plantid = ? AND building = ?',
                        {plantId, house})
                    TriggerClientEvent('INDCore:Notify', src,  Lang:t('text.the_plant_has_been_harvested'), 'success', 3500)
                    TriggerClientEvent('IND-weed:client:refreshHousePlants', -1, house)
                else
                    TriggerClientEvent('INDCore:Notify', src, Lang:t('error.this_plant_no_longer_exists'), 'error', 3500)
                end
            else
                TriggerClientEvent('INDCore:Notify', src, Lang:t('error.house_not_found'), 'error', 3500)
            end
        else
            TriggerClientEvent('INDCore:Notify', src, Lang:t('error.you_dont_have_enough_resealable_bags'), 'error', 3500)
        end
    else
        TriggerClientEvent('INDCore:Notify', src, Lang:t('error.you_Dont_have_enough_resealable_bags'), 'error', 3500)
    end
...
RegisterNetEvent('IND-weed:server:foodPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('INDCore:Notify', src,
