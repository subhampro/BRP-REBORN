...
RegisterCommand(Config.HandsUp.command, function()
    ...
    handsup = not handsup
    if exports['IND-policejob']:IsHandcuffed() then return end
    if handsup then
        TaskPlayAnim(ped, animDict, anim, 8.0, 8.0, -1, 50, 0, false, false, false)
        exports['IND-smallresources']:addDisableControls(Config.disableHandsupControls)
    else
        ClearPedTasks(ped)
        exports['IND-smallresources']:removeDisableControls(Config.disableHandsupControls)
    end
end, false)
