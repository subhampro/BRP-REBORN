...
function TakeOutImpound(vehicle)
    ...
    if coords then
        INDCore.Functions.TriggerCallback('INDCore:Server:SpawnVehicle', function(netId)
            ...
            INDCore.Functions.TriggerCallback('IND-garage:server:GetVehicleProperties', function(properties)
...
function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
        INDCore.Functions.TriggerCallback('INDCore:Server:SpawnVehicle', function(netId)
			...
			INDCore.Shared.SetDefaultVehicleExtras(veh, Config.VehicleSettings[vehicleInfo].extras)
		end
		if Config.VehicleSettings[vehicleInfo].livery ~= nil then
            ...
            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
            TriggerEvent("vehiclekeys:client:SetOwner", INDCore.Functions.GetPlate(veh))
            TriggerServerEvent("inventory:server:addTrunkItems", INDCore.Functions.GetPlate(veh), Config.CarItems)
            SetVehicleEngineOn(veh, true, true)
...
local function IsArmoryWhitelist() -- being removed
    ...
    if INDCore.Functions.GetPlayerData().job.name == 'police' then
        retval = true
    end
    return retval
end
...
local function SetWeaponSeries()
    for k, _ in pairs(Config.Items.items) do
        if k < 6 then
            Config.Items.items[k].info.serie = tostring(INDCore.Shared.RandomInt(2) .. INDCore.Shared.RandomStr(3) .. INDCore.Shared.RandomInt(1) .. INDCore.Shared.RandomStr(2) .. INDCore.Shared.RandomInt(3) .. INDCore.Shared.RandomStr(4))
        end
    end
end
...
function MenuGarage(currentSelection)
    ...
    local authorizedVehicles = Config.AuthorizedVehicles[INDCore.Functions.GetPlayerData().job.grade.level]
...
function MenuImpound(currentSelection)
    ...
    INDCore.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        local shouldContinue = false
        if result == nil then
            INDCore.Functions.Notify(Lang:t("error.no_impound"), "error", 5000)
        else
            shouldContinue = true
            for _ , v in pairs(result) do
                local enginePercent = INDCore.Shared.Round(v.engine / 10, 0)
                local currentFuel = v.fuel
                local vname = INDCore.Shared.Vehicles[v.vehicle].name
        ...
        if shouldContinue then
            ...
            exports['IND-menu']:openMenu(impoundMenu)
        end
    end)
...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
           INDCore.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                ...
                TriggerEvent('INDCore:Notify', Lang:t('success.impounded'), 'success')
                ClearPedTasks(ped)
            end, function() -- Play When Cancel
                ClearPedTasks(ped)
                TriggerEvent('INDCore:Notify', Lang:t('error.canceled'), 'error')
            end)
        end
    end
end)
    ...
    INDCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "police" then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                INDCore.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result then
                        for _, v in pairs(result) do
                            INDCore.Functions.Notify(''..v..'')
                        end
                    end
                end, playerId)
            else
                INDCore.Functions.Notify(Lang:t("error.none_nearby"), "error")
            end
        end
    end)
end)
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function(data)
    ...
    if #(pos - takeLoc) <= 1.0 then
        local drawer = exports['IND-input']:ShowInput({
...
RegisterNetEvent('IND-policejob:ToggleDuty', function()
    TriggerServerEvent("INDCore:ToggleDuty")
    TriggerServerEvent("police:server:UpdateCurrentCops")
    TriggerServerEvent("police:server:UpdateBlips")
end)

RegisterNetEvent('IND-police:client:scanFingerPrint', function()
    local player, distance = GetClosestPlayer()
...
RegisterNetEvent('IND-police:client:openArmoury', function()
...
RegisterNetEvent('IND-police:client:spawnHelicopter', function(k)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        INDCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
    else
...
RegisterNetEvent("IND-police:client:openStash", function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "policestash_"..INDCore.Functions.GetPlayerData().citizenid)
    TriggerEvent("inventory:client:SetCurrentStash", "policestash_"..INDCore.Functions.GetPlayerData().citizenid)
end)

RegisterNetEvent('IND-police:client:openTrash', function()
    ...
    CreateThread(function()
        while true do
            ...
            if inFingerprint and PlayerJob.name == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("IND-police:client:scanFingerPrint")
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inArmoury and PlayerJob.name == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("IND-police:client:openArmoury")
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inHelicopter and PlayerJob.name == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("IND-police:client:spawnHelicopter")
                    break
                end
            else
                break
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["duty"]) do
            exports['IND-target']:AddBoxZone("PoliceDuty_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "PoliceDuty_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "IND-policejob:ToggleDuty",
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['IND-target']:AddBoxZone("PoliceStash_"..k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = "PoliceStash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "IND-police:client:openStash",
                        icon = "fas fa-dungeon",
        ...
        for k, v in pairs(Config.Locations["trash"]) do
            exports['IND-target']:AddBoxZone("PoliceTrash_"..k, vector3(v.x, v.y, v.z), 1, 1.75, {
                name = "PoliceTrash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "IND-police:client:openTrash",
                        icon = "fas fa-trash",
                        label = "Open Trash",
                        job = "police",
                    },
        ...
        for k, v in pairs(Config.Locations["fingerprint"]) do
            exports['IND-target']:AddBoxZone("PoliceFingerprint_"..k, vector3(v.x, v.y, v.z), 2, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "IND-police:client:scanFingerPrint",
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['IND-target']:AddBoxZone("PoliceArmory_"..k, vector3(v.x, v.y, v.z), 5, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "IND-police:client:openArmoury",
                        icon = "fas fa-swords",
                        label = "Open Armory",
                        job = "police",
                    },
    ...
    dutyCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if not PlayerJob.onduty then
                exports['IND-core']:DrawText(Lang:t('info.on_duty'),'left')
                dutylistener()
            else
                exports['IND-core']:DrawText(Lang:t('info.off_duty'),'left')
                dutylistener()
            end
        else
            dutylisten = false
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['IND-core']:DrawText(Lang:t('info.stash_enter'), 'left')
                stash()
            end
        else
            inStash = false
    ...
    trashCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inTrash = true
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['IND-core']:DrawText(Lang:t('info.trash_enter'),'left')
                trash()
            end
        else
            inTrash = false
    ...
    fingerprintCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inFingerprint = true
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['IND-core']:DrawText(Lang:t('info.scan_fingerprint'),'left')
                fingerprint()
            end
        else
            inFingerprint = false
    ...
    armouryCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inArmoury = true
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['IND-core']:DrawText(Lang:t('info.enter_armory'),'left')
                armoury()
            end
        else
            inArmoury = false
...
CreateThread(function()
    ...
    helicopterCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['IND-core']:HideText()
                    exports['IND-core']:DrawText(Lang:t('info.store_heli'), 'left')
                    heli()
                else
                    exports['IND-core']:DrawText(Lang:t('info.take_heli'), 'left')
                    heli()
                end
            end
        else
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['IND-core']:DrawText(Lang:t('info.impound_veh'), 'left')
                    impound()
                else
    ...
    garageCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['IND-core']:DrawText(Lang:t('info.store_veh'), 'left')
		    garage()
                else
                    local currentSelection = 0
