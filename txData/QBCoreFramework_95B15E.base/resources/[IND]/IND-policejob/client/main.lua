...
AddEventHandler('INDCore:Client:OnPlayerLoaded', function()
    ...
    if player.metadata.tracker then
        ...
        TriggerEvent('IND-clothing:client:loadOutfit', trackerClothingData)
    else
        local trackerClothingData = {
            outfitData = {
                ["accessory"] = {
...
RegisterNetEvent("INDCore:Client:SetDuty", function(newDuty)
...
RegisterNetEvent('police:client:policeAlert', function(coords, text)
    ...
    INDCore.Functions.Notify({text = text, caption = street1name.. ' ' ..street2name}, 'police')
