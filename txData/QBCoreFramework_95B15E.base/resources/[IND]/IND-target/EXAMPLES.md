...
```lua
exports['IND-target']:AddBoxZone("MissionRowDutyClipboard", vector3(441.7989, -982.0529, 30.67834), 0.45, 0.35, {
...
exports['IND-target']:AddTargetModel(Config.Peds, {
	options = {
		{
...
exports['IND-target']:AddTargetEntity(mule, {
    options = {
        {
            type = "client",
...
## Passing Item Data
In this example, we define the model of the coffee machines you see around the map, and allow players to purchase a coffee. You'll have to provide your own logic for the purchase, but this is how you would handle it with IND-target, and how you would pass data through to an event for future use.
...
```lua
exports['IND-target']:AddTargetModel(`prop_vend_coffe_01`, {
    options = {
        {
            type = "client",
            event = "coffee:buy",
...
RegisterNetEvent('coffee:buy',function(data)
    ...
    INDCore.Functions.Notify("You purchased a " .. data.label .. " for $" .. data.price .. ". Enjoy!", 'success')
end)
```
...
AddEventHandler('plantpotato',function()
	...
	exports['IND-target']:AddEntityZone("potato-growing-"..plant, plant, {
