...
function INDCore.Functions.GetPlayerData(cb)
    if not cb then return INDCore.PlayerData end
    cb(INDCore.PlayerData)
end
...
function INDCore.Functions.HasItem(items, amount)
    return exports['IND-inventory']:HasItem(items, amount)
end

-- Utility

function INDCore.Functions.DrawText(x, y, width, height, scale, r, g, b, a, text)
...
function INDCore.Functions.DrawText3D(x, y, z, text)
    -- Use local function instead
...
function INDCore.Functions.RequestAnimDict(animDict)
...
function INDCore.Functions.PlayAnim(animDict, animName, upperbodyOnly, duration)
    local flags = upperbodyOnly and 16 or 0
    local runTime = duration or -1
    INDCore.Functions.RequestAnimDict(animDict)
    TaskPlayAnim(PlayerPedId(), animDict, animName, 8.0, 1.0, runTime, flags, 0.0, false, false, true)
    RemoveAnimDict(animDict)
end

function INDCore.Functions.LoadModel(model)
...
function INDCore.Functions.LoadAnimSet(animSet)
...
RegisterNUICallback('getNotifyConfig', function(_, cb)
    cb(INDCore.Config.Notify)
end)

function INDCore.Functions.Notify(text, texttype, length)
...
function INDCore.Debug(resource, obj, depth)
    TriggerServerEvent('INDCore:DebugSomething', resource, obj, depth)
end

-- Callback Functions --

-- Client Callback
function INDCore.Functions.CreateClientCallback(name, cb)
    INDCore.ClientCallbacks[name] = cb
end

function INDCore.Functions.TriggerClientCallback(name, cb, ...)
    if not INDCore.ClientCallbacks[name] then return end
    INDCore.ClientCallbacks[name](cb, ...)
end

-- Server Callback
function INDCore.Functions.TriggerCallback(name, cb, ...)
    INDCore.ServerCallbacks[name] = cb
    TriggerServerEvent('INDCore:Server:TriggerCallback', name, ...)
end

function INDCore.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    if GetResourceState('progressbar') ~= 'started' then error('progressbar needs to be started in order for INDCore.Functions.Progressbar to work') end
...
function INDCore.Functions.GetVehicles()
    return GetGamePool('CVehicle')
end

function INDCore.Functions.GetObjects()
...
function INDCore.Functions.GetPeds(ignoreList)
...
function INDCore.Functions.GetClosestPed(coords, ignoreList)
    ...
    ignoreList = ignoreList or {}
    local peds = INDCore.Functions.GetPeds(ignoreList)
...
function INDCore.Functions.IsWearingGloves()
    local ped = PlayerPedId()
    local armIndex = GetPedDrawableVariation(ped, 3)
    local model = GetEntityModel(ped)
    if model == `mp_m_freemode_01` then
        if INDCore.Shared.MaleNoGloves[armIndex] then
            return false
        end
    else
        if INDCore.Shared.FemaleNoGloves[armIndex] then
            return false
        end
    end
    return true
...
function INDCore.Functions.GetClosestPlayer(coords)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    local closestPlayers = INDCore.Functions.GetPlayersFromCoords(coords)
    local closestDistance = -1
...
function INDCore.Functions.GetPlayersFromCoords(coords, distance)
...
function INDCore.Functions.GetClosestVehicle(coords)
    local ped = PlayerPedId()
...
function INDCore.Functions.GetClosestObject(coords)
    local ped = PlayerPedId()
...
function INDCore.Functions.GetClosestBone(entity, list)
...
function INDCore.Functions.GetBoneDistance(entity, boneType, boneIndex)
    local bone
    if boneType == 1 then
...
function INDCore.Functions.AttachProp(ped, model, boneId, x, y, z, xR, yR, zR, vertex)
    local modelHash = type(model) == 'string' and GetHashKey(model) or model
    local bone = GetPedBoneIndex(ped, boneId)
    INDCore.Functions.LoadModel(modelHash)
...
function INDCore.Functions.SpawnVehicle(model, cb, coords, isnetworked, teleportInto)
    local ped = PlayerPedId()
    ...
    INDCore.Functions.LoadModel(model)
...
function INDCore.Functions.DeleteVehicle(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

function INDCore.Functions.GetPlate(vehicle)
    if vehicle == 0 then return end
    return INDCore.Shared.Trim(GetVehicleNumberPlateText(vehicle))
end

function INDCore.Functions.GetVehicleLabel(vehicle)
    if vehicle == nil or vehicle == 0 then return end
    return GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)))
end

function INDCore.Functions.SpawnClear(coords, radius)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
...
function INDCore.Functions.GetVehicleProperties(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        return {
            ...
            plate = INDCore.Functions.GetPlate(vehicle),
            plateIndex = GetVehicleNumberPlateTextIndex(vehicle),
            bodyHealth = INDCore.Shared.Round(GetVehicleBodyHealth(vehicle), 0.1),
            engineHealth = INDCore.Shared.Round(GetVehicleEngineHealth(vehicle), 0.1),
            tankHealth = INDCore.Shared.Round(GetVehiclePetrolTankHealth(vehicle), 0.1),
            fuelLevel = INDCore.Shared.Round(GetVehicleFuelLevel(vehicle), 0.1),
            dirtLevel = INDCore.Shared.Round(GetVehicleDirtLevel(vehicle), 0.1),
            oilLevel = INDCore.Shared.Round(GetVehicleOilLevel(vehicle), 0.1),
            color1 = colorPrimary,
...
function INDCore.Functions.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
...
function INDCore.Functions.LoadParticleDictionary(dictionary)
...
function INDCore.Functions.StartParticleAtCoord(dict, ptName, looped, coords, rot, scale, alpha, color, duration)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
        coords = GetEntityCoords(PlayerPedId())
    end
    INDCore.Functions.LoadParticleDictionary(dict)
...
function INDCore.Functions.StartParticleOnEntity(dict, ptName, looped, entity, bone, offset, rot, scale, alpha, color, evolution, duration)
    INDCore.Functions.LoadParticleDictionary(dict)
    UseParticleFxAssetNextCall(dict)
...
function INDCore.Functions.GetStreetNametAtCoords(coords)
...
function INDCore.Functions.GetZoneAtCoords(coords)
    return GetLabelText(GetNameOfZone(coords))
end

function INDCore.Functions.GetCardinalDirection(entity)
...
function INDCore.Functions.GetCurrentTime()
    local obj = {}
...
function INDCore.Functions.GetGroundZCoord(coords)
...
function INDCore.Functions.GetGroundHash(entity)
    local coords = GetEntityCoords(entity)
