...
RegisterNetEvent('IND-admin:client:inventory', function(targetPed)
    TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", targetPed)
end)

RegisterNetEvent('IND-admin:client:spectate', function(targetPed)
...
RegisterNetEvent('IND-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('IND-admin:server:SendReport', name, src, msg)
end)
...
RegisterNetEvent('IND-admin:client:SaveCar', function()
    local ped = PlayerPedId()
    local veh = GetVehiclePedIsIn(ped)

    if veh ~= nil and veh ~= 0 then
        local plate = INDCore.Functions.GetPlate(veh)
        local props = INDCore.Functions.GetVehicleProperties(veh)
        local hash = props.model
        local vehname = getVehicleFromVehList(hash)
        if INDCore.Shared.Vehicles[vehname] ~= nil and next(INDCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('IND-admin:server:SaveCar', props, INDCore.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
            INDCore.Functions.Notify(Lang:t("error.no_store_vehicle_garage"), 'error')
        end
    else
...
RegisterNetEvent('IND-admin:client:SetModel', function(skin)
    local ped = PlayerPedId()
...
RegisterNetEvent('IND-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('IND-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    local ped = PlayerPedId()
    if weapon ~= "current" then
        weapon = weapon:upper()
        SetPedAmmo(ped, GetHashKey(weapon), ammo)
        INDCore.Functions.Notify(Lang:t("info.ammoforthe", {value = ammo, weapon = INDCore.Shared.Weapons[weapon]["label"]}), 'success')
    else
...
RegisterNetEvent('IND-admin:client:GiveNuiFocus', function(focus, mouse)
...
RegisterNetEvent('IND-admin:client:maxmodVehicle', function()
