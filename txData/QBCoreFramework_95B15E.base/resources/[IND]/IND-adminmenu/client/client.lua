...
RegisterNetEvent('IND-admin:client:openMenu', function()
...
menu2_admin_display_names:On('change', function()
    TriggerEvent('IND-admin:client:toggleNames')
end)

-- Toggle player blip display
menu2_admin_display_blips:On('change', function()
    TriggerEvent('IND-admin:client:toggleBlips')
end)
...
-- Weapons list
for _,v in pairs(INDCore.Shared.Weapons) do
    menu15:AddButton({icon = '🎁',
        label = v.label ,
        value = v.value ,
        description = Lang:t("desc.spawn_weapons_desc"),
        select = function(_)
            TriggerServerEvent('IND-admin:giveWeapon', v.name)
            INDCore.Functions.Notify(Lang:t("success.spawn_weapon"))
        end
    })
end
...
local function OpenPermsMenu(permsply)
    INDCore.Functions.TriggerCallback('IND-admin:server:getrank', function(rank)
        if rank then
            ...
            menu11:AddButton({
                ...
                select = function(_)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('IND-admin:server:setPermissions', permsply.id, selectedgroup)
                        INDCore.Functions.Notify(Lang:t("success.changed_perm"), 'success')
                        selectedgroup = 'Unknown'
                    else
                        INDCore.Functions.Notify(Lang:t("error.changed_perm_failed"), 'error')
                    end
                end
            })
        else
    ...
    menu10:AddButton({
        ...
        select = function(_)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('IND-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
                INDCore.Functions.Notify(Lang:t("error.missing_reason"), 'error')
            end
        end
    })
end
    ...
    menu9:AddButton({
        ...
        select = function(_)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('IND-admin:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
                INDCore.Functions.Notify(Lang:t("error.invalid_reason_length_ban"), 'error')
            end
        end
    })
end
        ...
        Players:AddButton({
            ...
            select = function(btn)
                ...
                if values ~= "ban" and values ~= "kick" and values ~= "perms" then
                    TriggerServerEvent('IND-admin:server:'..values, player)
...
player_management:On('select', function(_)
    menu4:ClearItems()
    INDCore.Functions.TriggerCallback('test:getplayers', function(players)
...
menu3_server_weather:On("select",function()
    ...
    for _, v in ipairs(elements) do
        menu8:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            TriggerServerEvent('IND-weathersync:server:setWeather', selection.value)
            INDCore.Functions.Notify(Lang:t("weather.weather_changed", {value = selection.label}))
        end})
    end
end)
...
menu3_server_time:On("select", function(_, value)
    TriggerServerEvent("IND-weathersync:server:setTime", value, value)
    INDCore.Functions.Notify(Lang:t("time.changed", {time = value}))
end)
...
local function OpenCarModelsMenu(category)
    ...
    for k, v in pairs(category) do
        menu13:AddButton({
            ...
            select = function(_)
                TriggerServerEvent('INDCore:CallCommand', "car", { k })
            end
        })
    end
end
...
menu5_vehicles_spawn:On('Select', function(_)
    ...
    for k, v in pairs(vehicles) do
        menu12:AddButton({
            label = INDCore.Shared.FirstToUpper(k),
            value = v,
...
menu5_vehicles_fix:On('Select', function(_)
    TriggerServerEvent('INDCore:CallCommand', "fix", {})
end)

menu5_vehicles_buy:On('Select', function(_)
    TriggerServerEvent('INDCore:CallCommand', "admincar", {})
end)

menu5_vehicles_remove:On('Select', function(_)
    TriggerServerEvent('INDCore:CallCommand', "dv", {})
end)

menu5_vehicles_max_upgrades:On('Select', function(_)
    TriggerServerEvent('INDCore:CallCommand', "maxmods", {})
end)
...
menu7_dev_info_mode:On('change', function(_, _, _)
    ...
    TriggerEvent('IND-admin:client:ToggleDevmode')
...
local function CopyToClipboard(dataType)
    ...
    if dataType == 'coords2' then
        ...
        local x = INDCore.Shared.Round(coords.x, 2)
        local y = INDCore.Shared.Round(coords.y, 2)
        SendNUIMessage({
            string = string.format('vector2(%s, %s)', x, y)
        })
        INDCore.Functions.Notify(Lang:t("success.coords_copied"), "success")
    elseif dataType == 'coords3' then
        local coords = GetEntityCoords(ped)
        local x = INDCore.Shared.Round(coords.x, 2)
        local y = INDCore.Shared.Round(coords.y, 2)
        local z = INDCore.Shared.Round(coords.z, 2)
        SendNUIMessage({
    ...
    elseif dataType == 'coords4' then
        ...
        local h = INDCore.Shared.Round(heading, 2)
        SendNUIMessage({
    ...
    elseif dataType == 'heading' then
        ...
        INDCore.Functions.Notify(Lang:t("success.heading_copied"), "success")
    elseif dataType == 'freeaimEntity' then
        ...
        if entity then
            ...
            local x = INDCore.Shared.Round(entityCoords.x, 2)
            local y = INDCore.Shared.Round(entityCoords.y, 2)
            local z = INDCore.Shared.Round(entityCoords.z, 2)
            local rotX = INDCore.Shared.Round(entityRotation.x, 2)
            local rotY = INDCore.Shared.Round(entityRotation.y, 2)
            local rotZ = INDCore.Shared.Round(entityRotation.z, 2)
            local h = INDCore.Shared.Round(entityHeading, 2)
            SendNUIMessage({
                string = string.format('Model Name:\t%s\nModel Hash:\t%s\n\nHeading:\t%s\nCoords:\t\tvector3(%s, %s, %s)\nRotation:\tvector3(%s, %s, %s)', entityName, entityHash, h, x, y, z, rotX, rotY, rotZ)
            })
            INDCore.Functions.Notify(Lang:t("success.entity_copy"), "success")
        else
            INDCore.Functions.Notify(Lang:t("error.failed_entity_copy"), "error")
        end
    end
end

RegisterNetEvent('IND-admin:client:copyToClipboard', function(dataType)
...
local function ToggleShowCoordinates()
    ...
    CreateThread(function()
        while showCoords do
            ...
            local c = {}
            c.x = INDCore.Shared.Round(coords.x, 2)
            c.y = INDCore.Shared.Round(coords.y, 2)
            c.z = INDCore.Shared.Round(coords.z, 2)
            heading = INDCore.Shared.Round(heading, 2)
            Wait(0)
    ...
    CreateThread(function()
        while vehicleDevMode do
            ...
            if IsPedInAnyVehicle(ped, false) then
                ...
                Draw2DText(string.format(Lang:t("info.eng_health") .. '~b~%s~s~ | ' .. Lang:t("info.body_health") .. '~b~%s~s~', INDCore.Shared.Round(eHealth, 2), INDCore.Shared.Round(bHealth, 2)), 4, {255, 255, 255}, 0.4, x + 0.0, y + 0.075)
            end
        end
    end)
end

RegisterNetEvent('IND-admin:client:ToggleCoords', function()
...
entity_view_distance:On("select", function(_, value)
    ...
    INDCore.Functions.Notify(Lang:t("info.entity_view_distance", {distance = value}))
end)
    ...
    for _, v in ipairs(elements) do
        EditDealer:AddButton({
            ...
            select = function(btn)
                local values = btn.Value
                if values == "goto" then
                    TriggerServerEvent('INDCore:CallCommand', "dealergoto", { dealer["name"] })
                elseif values == "remove" then
                    TriggerServerEvent('INDCore:CallCommand', "deletedealer", { dealer["name"] })
                    EditDealer:Close()
                    menu6:Close()
                end
            end
...
menu1_dealer_list:On('Select', function(_)
    menu6:ClearItems()
    INDCore.Functions.TriggerCallback('test:getdealers', function(dealers)
