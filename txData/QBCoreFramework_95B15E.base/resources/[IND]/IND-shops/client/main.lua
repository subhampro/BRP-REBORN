...
local function openShop(shop, data)
    INDCore.Functions.TriggerCallback('IND-shops:server:SetShopInv', function(shopInvJson)
        ...
        if data.type == "weapon" and Config.FirearmsLicenseCheck then
            if PlayerData.metadata["licences"] and PlayerData.metadata["licences"].weapon and INDCore.Functions.HasItem("weaponlicense") then
                ShopItems.items = SetupItems()
                INDCore.Functions.Notify(Lang:t("success.dealer_verify"), "success")
                Wait(500)
            else
                ShopItems.items = SetupItems(true)
                INDCore.Functions.Notify(Lang:t("error.dealer_decline"), "error")
                Wait(500)
                INDCore.Functions.Notify(Lang:t("error.talk_cop"), "error")
                Wait(1000)
            end
        else
            ShopItems.items = SetupItems()
        ...
        while listen do
            if IsControlJustPressed(0, 38) then -- E
                ...
                if inChips then
                    ...
                    TriggerServerEvent("IND-shops:server:sellChips")
                else
                    exports["IND-core"]:KeyPressed()
...
local function createPeds()
    ...
    for k, v in pairs(Config.Locations) do
        ...
        if Config.UseTarget then
            exports['IND-target']:AddTargetEntity(ShopPed[k], {
                options = {
                    {
                        label = v["targetLabel"],
                        icon = v["targetIcon"],
    ...
    if Config.UseTarget then
        exports['IND-target']:AddTargetEntity(ShopPed["casino"], {
            options = {
                {
                    label = 'Sell Chips',
...
-- Events
RegisterNetEvent("IND-shops:client:UpdateShop", function(shop, itemData, amount)
    TriggerServerEvent("IND-shops:server:UpdateShopItems", shop, itemData, amount)
end)

RegisterNetEvent("IND-shops:client:SetShopItems", function(shop, shopProducts)
    Config.Locations[shop]["products"] = shopProducts
end)

RegisterNetEvent("IND-shops:client:RestockShopItems", function(shop, amount)
    ...
    CreateThread(function()
        ...
        combo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                exports["IND-core"]:DrawText(Lang:t("info.open_shop"))
                listenForControl()
            else
        ...
        sellChips:onPlayerInOut(function(isPointInside)
            if isPointInside then
                inChips = true
                exports["IND-core"]:DrawText(Lang:t("info.sell_chips"))
                listenForControl()
            else
                inChips = false
                exports["IND-core"]:HideText()
