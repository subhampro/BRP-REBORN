...
RegisterNetEvent('IND-storerobbery:server:takeMoney', function(register, isDone)
    local src = source
...
RegisterNetEvent('IND-storerobbery:server:setRegisterStatus', function(register)
    Config.Registers[register].robbed = true
    Config.Registers[register].time = Config.resetTime
    TriggerClientEvent('IND-storerobbery:client:setRegisterStatus', -1, register, Config.Registers[register])
end)

RegisterNetEvent('IND-storerobbery:server:setSafeStatus', function(safe)
    Config.Safes[safe].robbed = true
    TriggerClientEvent('IND-storerobbery:client:setSafeStatus', -1, safe, true)

    SetTimeout(math.random(40, 80) * (60 * 1000), function()
        Config.Safes[safe].robbed = false
        TriggerClientEvent('IND-storerobbery:client:setSafeStatus', -1, safe, false)
    end)
end)

RegisterNetEvent('IND-storerobbery:server:SafeReward', function(safe)
    local src = source
	local Player = INDCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if luck <= 10 then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, INDCore.Shared.Items["rolex"], "add")
        if luck == odd then
            Wait(500)
            Player.Functions.AddItem("goldbar", 1)
            TriggerClientEvent('inventory:client:ItemBox', src, INDCore.Shared.Items["goldbar"], "add")
        end
    end
end)

RegisterNetEvent('IND-storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    local cameraId
    if type == "safe" then
    ...
    TriggerClientEvent("IND-storerobbery:client:robberyCall", -1, type, safe, streetLabel, coords)
    TriggerClientEvent("IND-phone:client:addPoliceAlert", -1, alertData)
end)

RegisterNetEvent('IND-storerobbery:server:removeAdvancedLockpick', function()
...
RegisterNetEvent('IND-storerobbery:server:removeLockpick', function()
    local Player = INDCore.Functions.GetPlayer(source)
...
CreateThread(function()
    while true do
        ...
        if #toSend > 0 then
            ...
            TriggerClientEvent('IND-storerobbery:client:setRegisterStatus', -1, toSend, false)
        end
...
INDCore.Functions.CreateCallback('IND-storerobbery:server:isCombinationRight', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

INDCore.Functions.CreateCallback('IND-storerobbery:server:getPadlockCombination', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

INDCore.Functions.CreateCallback('IND-storerobbery:server:getRegisterStatus', function(_, cb)
    cb(Config.Registers)
end)

INDCore.Functions.CreateCallback('IND-storerobbery:server:getSafeStatus', function(_, cb)
    cb(Config.Safes)
end)
