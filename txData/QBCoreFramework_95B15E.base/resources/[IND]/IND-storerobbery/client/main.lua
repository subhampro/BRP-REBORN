...
CreateThread(function()
    Wait(1000)
    if INDCore.Functions.GetPlayerData().job ~= nil and next(INDCore.Functions.GetPlayerData().job) then
                            ...
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if Config.Safes[safe].type == "keypad" then
                                        ...
                                        INDCore.Functions.TriggerCallback('IND-storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    if not copsCalled then
                                        ...
                                        TriggerServerEvent("IND-storerobbery:server:callCops", "safe", currentSafe, streetLabel, pos)
                                        copsCalled = true
                                    end
                                else
                                    INDCore.Functions.Notify(Lang:t("error.minimum_store_robbery_police", { MinimumStoreRobberyPolice = Config.MinimumStoreRobberyPolice}), "error")
                                end
                            end
                        else
            ...
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                if usingAdvanced then
                    ...
                    if not copsCalled then
                        ...
                        TriggerServerEvent("IND-storerobbery:server:callCops", "cashier", currentRegister, streetLabel, pos)
                        copsCalled = true
                    end
                else
                    lockpick(true)
...
function setupRegister()
    INDCore.Functions.TriggerCallback('IND-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    INDCore.Functions.TriggerCallback('IND-storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function(_, cb)
    if currentRegister ~= 0 then
        lockpick(false)
        TriggerServerEvent('IND-storerobbery:server:setRegisterStatus', currentRegister)
        local lockpickTime = 25000
        LockpickDoorAnim(lockpickTime)
        INDCore.Functions.Progressbar("search_register", Lang:t("text.emptying_the_register"), lockpickTime, false, true, {
            ...
            TriggerServerEvent('IND-storerobbery:server:takeMoney', currentRegister, true)
        end, function() -- Cancel
            openingDoor = false
            ClearPedTasks(PlayerPedId())
            INDCore.Functions.Notify(Lang:t("error.process_canceled"), "error")
            currentRegister = 0
        end)
        CreateThread(function()
...
function LockpickDoorAnim(time)
    ...
    CreateThread(function()
        while openingDoor do
            ...
            Wait(2000)
            time = time - 2
            TriggerServerEvent('IND-storerobbery:server:takeMoney', currentRegister, false)
            if time <= 0 then
                openingDoor = false
        ...
        if won then
            if currentSafe ~= 0 then
                if not Config.Safes[currentSafe].robbed then
                    ...
                    TriggerServerEvent("IND-storerobbery:server:SafeReward", currentSafe)
                    TriggerServerEvent("IND-storerobbery:server:setSafeStatus", currentSafe)
                    currentSafe = 0
                    takeAnim()
                end
            else
...
RegisterNUICallback('fail', function(_ ,cb)
    if usingAdvanced then
        if math.random(1, 100) < 20 then
            TriggerServerEvent("IND-storerobbery:server:removeAdvancedLockpick")
            TriggerEvent('inventory:client:ItemBox', INDCore.Shared.Items["advancedlockpick"], "remove")
        end
    else
        if math.random(1, 100) < 40 then
            TriggerServerEvent("IND-storerobbery:server:removeLockpick")
    ...
    if (IsWearingGloves() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(PlayerPedId())
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        INDCore.Functions.Notify(Lang:t("error.you_broke_the_lock_pick"))
    end
    lockpick(false)
    cb('ok')
end)
...
RegisterNUICallback('TryCombination', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-storerobbery:server:isCombinationRight', function(combination)
...
RegisterNetEvent('IND-storerobbery:client:setRegisterStatus', function(batch, val)
...
RegisterNetEvent('IND-storerobbery:client:setSafeStatus', function(safe, bool)
    Config.Safes[safe].robbed = bool
end)

RegisterNetEvent('IND-storerobbery:client:robberyCall', function(_, _, _, coords)
