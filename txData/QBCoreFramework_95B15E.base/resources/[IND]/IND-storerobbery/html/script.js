...
Padlock.Close = function() {
    ...
    $.post('https://IND-storerobbery/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://IND-storerobbery/PadLockClose');
    if (data.error != null) {
        $.post('https://IND-storerobbery/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://IND-storerobbery/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://IND-storerobbery/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://IND-storerobbery/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
    ...
    document.onkeyup = function (data) {
        if (data.which == 27 ) {
            if (CurrentType == "keypad") {
                ...
                $.post('https://IND-storerobbery/exit');
            }
        }
    };
}); //docready
...
function outOfPins() {
    gameOver = true;
    $.post('https://IND-storerobbery/fail');
    setTimeout(function(){
        reset()
    }, 250)
}
...
function unlock() {
    gameOver = true;
    $.post('https://IND-storerobbery/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
