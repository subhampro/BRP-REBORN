...
local function SaveSkin()
    ...
    TriggerServerEvent("IND-clothing:saveSkin", model, clothing)
end
...
local function reloadSkin(health)
    ...
    local gender = INDCore.Functions.GetPlayerData().charinfo.gender
    ...
    TriggerServerEvent("IND-clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("IND-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
...
local function getOutfits(gradeLevel, data)
    local gender = "male"
    if INDCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    INDCore.Functions.TriggerCallback('IND-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('IND-clothing:client:openMenu')
AddEventHandler('IND-clothing:client:openMenu', function()
...
RegisterNetEvent('IND-clothing:client:reloadOutfits')
AddEventHandler('IND-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
        action = "reloadMyOutfits",
        outfits = myOutfits
    })
end)
RegisterNetEvent('IND-clothes:client:CreateFirstCharacter')
AddEventHandler('IND-clothes:client:CreateFirstCharacter', function()
    INDCore.Functions.GetPlayerData(function(pData)
...
RegisterNetEvent("IND-clothes:loadSkin")
AddEventHandler("IND-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('IND-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)
RegisterNetEvent('IND-clothing:client:loadPlayerClothing')
AddEventHandler('IND-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('IND-clothing:client:loadOutfit')
AddEventHandler('IND-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if INDCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        INDCore.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
RegisterNetEvent("IND-clothing:client:adjustfacewear")
AddEventHandler("IND-clothing:client:adjustfacewear",function(type)
    if INDCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
...
RegisterNetEvent('INDCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("IND-clothes:loadPlayerSkin")
    PlayerData = INDCore.Functions.GetPlayerData()
    loadStores()
--    INDCore.Shared.Jobs = exports['IND-jobs']:AddJobs()
end)
RegisterNetEvent('INDCore:Client:OnJobUpdate', function(JobInfo)
    PlayerData.job = JobInfo
end)
RegisterNetEvent('INDCore:Client:OnGangUpdate', function(GangInfo)
    PlayerData.gang = GangInfo
end)
RegisterNetEvent('IND-clothing:client:openOutfitMenu', function()
    INDCore.Functions.TriggerCallback('IND-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('IND-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNUICallback('TrackerError', function(_, cb)
    INDCore.Functions.Notify(Lang:t("notify.error_bracelet"), "error")
    cb('ok')
end)
RegisterNUICallback('saveOutfit', function(data, cb)
    local ped = PlayerPedId()
    local model = GetEntityModel(ped)
    TriggerServerEvent('IND-clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('IND-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('IND-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    INDCore.Functions.Notify(Lang:t('notify.info_deleteOutfit', {outfit = data.outfitName}))
    cb('ok')
end)
    ...
    if Config.UseTarget then
        CreateThread(function()
            for k, v in pairs(Config.Stores) do
                ...
                exports['IND-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                    name = v.shopType .. k,
                    debugPoly = false,
                    minZ = v.coords.z-1,
                    maxZ = v.coords.z+1,
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                elseif not INDCore.Shared.INDJobsStatus then
                    action = function()
                        customCamLocation = v.cameraLocation
                        local gradeLevel = PlayerData.job.grade.level
                        getOutfits(gradeLevel, Config.Outfits[v.requiredJob])
                    end
                else break end --this break is important if INDJobsStatus is true then the reste of the code in this loop does not need to run.

                exports['IND-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
            ...
            clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
                if isPointInside then
                    ...
                    if zoneName == 'surgeon' then
                        exports['IND-core']:DrawText('[E] - '..Lang:t("store.surgeon"), 'left')
                    elseif zoneName == 'clothing' then
                        exports['IND-core']:DrawText('[E] - '..Lang:t("store.clothing"), 'left')
                    elseif zoneName == 'barber' then
                        exports['IND-core']:DrawText('[E] - '..Lang:t("store.barber"), 'left')
                    elseif zoneName == 'outfit' then 
                        exports['IND-core']:DrawText('[E] - '..Lang:t("store.outfitchanger"), 'left')
                    end
                else
                    inZone = false
                    ...
                    if PlayerData.gang and PlayerData.gang.name or (not INDCore.Shared.INDJobsStatus and PlayerData.job.name) then
                        ...
                        clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
                            if isPointInside then
                                local zoneID = tonumber(INDCore.Shared.SplitStr(zone.name, "_")[2])
                                local job = Config.ClothingRooms[zoneID].isGang and PlayerData.gang.name or (not INDCore.Shared.INDJobsStatus and PlayerData.job.name)
                                if (job == Config.ClothingRooms[zoneID].requiredJob) then
                                    zoneName = zoneID
                                    inZone = true
                                    exports['IND-core']:DrawText('[E] - Clothing Shop', 'left')
                                end
                            else
                                inZone = false
                                exports['IND-core']:HideText()
                ...
                if inZone then
                    ...
                    elseif not INDCore.Shared.INDJobsStatus then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local gradeLevel = clothingRoom.isGang and PlayerData.gang.grade.level or (not INDCore.Shared.INDJobsStatus and PlayerData.job.grade.level)
