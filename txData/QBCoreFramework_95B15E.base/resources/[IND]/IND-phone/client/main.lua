...
local function findVehFromPlateAndLocate(plate)
    local gameVehicles = INDCore.Functions.GetVehicles()
    for i = 1, #gameVehicles do
        local vehicle = gameVehicles[i]
        if DoesEntityExist(vehicle) then
            if INDCore.Functions.GetPlate(vehicle) == plate then
...
local function LoadPhone()
    Wait(100)
    INDCore.Functions.TriggerCallback('IND-phone:server:GetPhoneData', function(pData)
        PlayerJob = INDCore.Functions.GetPlayerData().job
        PhoneData.PlayerData = INDCore.Functions.GetPlayerData()
...
local function OpenPhone()
    INDCore.Functions.TriggerCallback('IND-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            PhoneData.PlayerData = INDCore.Functions.GetPlayerData()
            ...
            INDCore.Functions.TriggerCallback('IND-garage:server:GetPlayerVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
        else
            INDCore.Functions.Notify("You don't have a phone", "error")
        end
    end)
end
...
local function CancelCall()
    TriggerServerEvent('IND-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('IND-phone:server:SetCallState', false)
...
local function CallContact(CallData, AnonymousCall)
    ...
    TriggerServerEvent('IND-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('IND-phone:server:SetCallState', true)
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('IND-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            INDCore.Functions.Notify("Action not available at the moment..", "error")
        end
    end
end)
...
RegisterNUICallback('ClearRecentAlerts', function(_, cb)
    TriggerServerEvent('IND-phone:server:SetPhoneAlerts', "phone", 0)
...
RegisterNUICallback('SetBackground', function(data, cb)
    ...
    TriggerServerEvent('IND-phone:server:SaveMetaData', PhoneData.MetaData)
    cb('ok')
end)
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('IND-phone:server:RemoveMail', MailId)
    cb('ok')
end)
...
RegisterNUICallback('AcceptMailButton', function(data, cb)
    ...
    TriggerServerEvent('IND-phone:server:ClearButtonData', data.mailId)
    cb('ok')
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('IND-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    INDCore.Functions.TriggerCallback('IND-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('PostAdvert', function(data, cb)
    TriggerServerEvent('IND-phone:server:AddAdvert', data.message, data.url)
    cb('ok')
end)

RegisterNUICallback("DeleteAdvert", function(_, cb)
    TriggerServerEvent("IND-phone:server:DeleteAdvert")
    cb('ok')
end)
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('IND-phone:server:SetPhoneAlerts', "whatsapp", newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    INDCore.Functions.TriggerCallback('IND-phone:server:PayInvoice', function(CanPay, Invoices)
        if CanPay then PhoneData.Invoices = Invoices end
        cb(CanPay)
    end, society, amount, invoiceId, senderCitizenId)
    TriggerServerEvent('IND-phone:server:BillingEmail', data, true)
end)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    local amount = data.amount
    local invoiceId = data.invoiceId
    INDCore.Functions.TriggerCallback('IND-phone:server:DeclineInvoice', function(_, Invoices)
        PhoneData.Invoices = Invoices
        cb('ok')
    end, society, amount, invoiceId)
    TriggerServerEvent('IND-phone:server:BillingEmail', data, false)
end)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('IND-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
    ...
    if #Hashtag <= 3 then
        for i = 2, #Hashtag, 1 do
            ...
            if Handle ~= nil or Handle ~= "" then
                ...
                TriggerServerEvent('IND-phone:server:UpdateHashtags', Handle, TweetMessage)
            end
        end
            ...
            if Handle ~= nil or Handle ~= "" then
                ...
                if (Firstname ~= nil and Firstname ~= "") and (Lastname ~= nil and Lastname ~= "") then
                    if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                        TriggerServerEvent('IND-phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                    end
                end
            end
        end
        ...
        TriggerServerEvent('IND-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
    else
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNUICallback('DeleteTweet',function(data, cb)
    TriggerServerEvent('IND-phone:server:DeleteTweet', data.id)
    cb('ok')
end)
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('IND-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerServerEvent('IND-phone:server:RemoveInstallation', data.app)
    cb("ok")
end)

RegisterNUICallback('GetTruckerData', function(_, cb)
    local TruckerMeta = INDCore.Functions.GetPlayerData().metadata["jobrep"]["trucker"]
    local TierData = exports['IND-trucker']:GetTier(TruckerMeta)
    cb(TierData)
end)
...
RegisterNUICallback('DeleteImage', function(image, cb)
    TriggerServerEvent('IND-phone:server:RemoveImageFromGallery',image)
    Wait(400)
    TriggerServerEvent('IND-phone:server:getImageFromGallery')
    cb(true)
end)
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        INDCore.Functions.Notify("Your vehicle has been marked", "success")
    else
        INDCore.Functions.Notify("This vehicle cannot be located", "error")
    end
    cb("ok")
end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('IND-phone:server:RemoveContact', Name, Number)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(_, cb)
    INDCore.Functions.TriggerCallback('IND-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data, cb)
    TriggerServerEvent('IND-lapraces:server:JoinRace', data.RaceData)
    cb('ok')
end)

RegisterNUICallback('LeaveRace', function(data, cb)
    TriggerServerEvent('IND-lapraces:server:LeaveRace', data.RaceData)
    cb('ok')
end)

RegisterNUICallback('StartRace', function(data, cb)
    TriggerServerEvent('IND-lapraces:server:StartRace', data.RaceData.RaceId)
    cb('ok')
end)

RegisterNUICallback('SetAlertWaypoint', function(data, cb)
    local coords = data.alert.coords
    INDCore.Functions.Notify('GPS Location set: '..data.alert.title)
...
RegisterNUICallback('FetchVehicleResults', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then
            for k, _ in pairs(result) do
                INDCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(_, cb)
    ...
    INDCore.Functions.TriggerCallback('IND-phone:server:ScanPlate', function(result)
        INDCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
	    if INDCore.Shared.Vehicles[vehname] ~= nil then
                result.label = INDCore.Shared.Vehicles[vehname]['name']
            else
...
RegisterNUICallback('GetRaces', function(_, cb)
    INDCore.Functions.TriggerCallback('IND-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('IND-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
    cb("ok")
end)

RegisterNUICallback('HasCreatedRace', function(_, cb)
    INDCore.Functions.TriggerCallback('IND-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(_, cb)
    local InRace = exports['IND-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['IND-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('IND-lapraces:server:CreateLapRace', data.TrackName)
    cb("ok")
end)

RegisterNUICallback('GetRacingLeaderboards', function(_, cb)
    INDCore.Functions.TriggerCallback('IND-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('IND-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
            INDCore.Functions.Notify('You\'re too far away from the race. GPS has been set to the race.', 'error', 5000)
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == "editor" then
        cb(exports['IND-lapraces']:IsInEditor())
    else
        cb(exports['IND-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(_, cb)
    INDCore.Functions.TriggerCallback('IND-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(_, cb)
    INDCore.Functions.TriggerCallback('IND-phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(_, cb)
    INDCore.Functions.TriggerCallback('IND-phone:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    INDCore.Functions.Notify("GPS has been set to " .. data.HouseData.HouseData.adress .. "!", "success")
    cb("ok")
end)

RegisterNUICallback('RemoveKeyholder', function(data, cb)
    TriggerServerEvent('IND-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    INDCore.Functions.TriggerCallback('IND-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    INDCore.Functions.Notify('GPS has been set!', 'success')
    cb("ok")
end)
...
RegisterNUICallback('GetCurrentLawyers', function(_, cb)
    INDCore.Functions.TriggerCallback('IND-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('ClearMentions', function(_ ,cb)
    ...
    TriggerServerEvent('IND-phone:server:SetPhoneAlerts', "twitter", 0)
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
    cb('ok')
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data, cb)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('IND-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
        cb('ok')
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('IND-phone:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        INDCore.Functions.TriggerCallback('IND-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('GetWhatsappChats', function(_, cb)
    INDCore.Functions.TriggerCallback('IND-phone:server:GetContactPictures', function(Chats)
        cb(Chats)
    end, PhoneData.Chats)
end)

RegisterNUICallback('CallContact', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-phone:server:GetCallState', function(CanCall, IsOnline, _)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('IND-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('IND-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    INDCore.Functions.TriggerCallback('IND-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
RegisterNUICallback("TakePhoto", function(_,cb)
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
            INDCore.Functions.TriggerCallback("IND-phone:server:GetWebhook",function(hook)
                if hook then
                    exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                        local image = json.decode(data)
                        DestroyMobilePhone()
                        CellCamActivate(false, false)
                        TriggerServerEvent('IND-phone:server:addImageToGallery', image.attachments[1].proxy_url)
                        Wait(400)
                        TriggerServerEvent('IND-phone:server:getImageFromGallery')
...
RegisterCommand('ping', function(_, args)
    if not args[1] then
        INDCore.Functions.Notify("You need to input a Player ID", "error")
    else
        TriggerServerEvent('IND-phone:server:sendPing', args[1])
    end
end)
...
RegisterNetEvent('IND-phone:client:TransferMoney', function(amount, newmoney)
...
-- RegisterNetEvent('IND-phone:client:UpdateTweetsDel', function(source, Tweets)
...
RegisterNetEvent('IND-phone:client:UpdateTweets', function(src, Tweets, NewTweetData, delete)
    PhoneData.Tweets = Tweets
    local MyPlayerId = PhoneData.PlayerData.source
...
RegisterNetEvent('IND-phone:client:RaceNotify', function(message)
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = "Racing",
...
RegisterNetEvent('IND-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('IND-phone:server:SetPhoneAlerts', "phone")
...
RegisterNetEvent("IND-phone-new:client:BankNotify", function(text)
    SendNUIMessage({
        action = "PhoneNotification",
        NotifyData = {
            title = "Bank",
...
RegisterNetEvent('IND-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = "Mail",
    ...
    TriggerServerEvent('IND-phone:server:SetPhoneAlerts', "mail")
end)

RegisterNetEvent('IND-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = "UpdateMails",
        Mails = NewMails
    })
...
RegisterNetEvent('IND-phone:client:UpdateAdvertsDel', function(Adverts)
...
RegisterNetEvent('IND-phone:client:UpdateAdverts', function(Adverts, LastAd)
    PhoneData.Adverts = Adverts
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
...
RegisterNetEvent('IND-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('IND-phone:server:sendNewMail', {
...
RegisterNetEvent('IND-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == "ongoing" then
        SendNUIMessage({
...
RegisterNetEvent('IND-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('IND-phone:server:AddRecentCall', "missed", CallData)
                    break
                end
                Wait(Config.RepeatTimeout)
            else
...
RegisterNetEvent('IND-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
	    ...
	    SendNUIMessage({
            ...
            TriggerServerEvent('IND-phone:server:SetPhoneAlerts', "whatsapp")
        end
    else
...
RegisterNetEvent('IND-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNetEvent('IND-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNetEvent('IND-phone:client:AddTransaction', function(_, _, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('IND-phone:server:AddTransaction', Data)
end)

RegisterNetEvent('IND-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications["phone"].Alerts = Config.PhoneApplications["phone"].Alerts + 1
    TriggerServerEvent('IND-phone:server:SetPhoneAlerts', "phone", Config.PhoneApplications["phone"].Alerts)
end)

RegisterNetEvent('IND-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNetEvent('IND-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = "ongoing"
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNetEvent('IND-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = INDCore.Functions.GetPlayerData().job
...
RegisterNetEvent('IND-phone:client:GiveContactDetails', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('IND-phone:server:GiveContactDetails', PlayerId)
    else
        INDCore.Functions.Notify("No one nearby!", "error")
    end
end)

RegisterNetEvent('IND-phone:client:UpdateLapraces', function()
    SendNUIMessage({
        action = "UpdateRacingApp",
    })
end)

RegisterNetEvent('IND-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNetEvent('IND-phone:refreshImages', function(images)
    PhoneData.Images = images
end)

RegisterNetEvent("IND-phone:client:CustomNotification", function(title, text, icon, color, timeout) -- Send a PhoneNotification to the phone from anywhere
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = title,
