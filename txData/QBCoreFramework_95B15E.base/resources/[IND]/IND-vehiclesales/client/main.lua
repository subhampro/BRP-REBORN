    ...
    if Zone then
        ...
        if vehicles then
            for i = 1, #vehicles, 1 do
                ...
                INDCore.Functions.SetVehicleProperties(occasionVehicles[Zone][i].car, json.decode(occasionVehicles[Zone][i].mods))
                ...
                if Config.UseTarget then
                    ...
                    EntityZones[i] = exports['IND-target']:AddTargetEntity(occasionVehicles[Zone][i].car, {
                        options = {
                            {
                                type = "client",
                                event = "IND-vehiclesales:client:OpenContract",
                                icon = "fas fa-car",
...
local function despawnOccasionsVehicles()
    ...
    for i = 1, #oSlot, 1 do
        ...
        if oldVehicle then
            INDCore.Functions.DeleteVehicle(oldVehicle)
        end

        if EntityZones[i] and Config.UseTarget then
            exports['IND-target']:RemoveZone(EntityZones[i])
        end
    end
    EntityZones = {}
end
...
local function openSellContract(bool)
    local pData = INDCore.Functions.GetPlayerData()
    ...
    SendNUIMessage({
        ...
        plate = INDCore.Functions.GetPlate(GetVehiclePedIsUsing(PlayerPedId()))
    })
end
...
local function sellVehicleWait(price)
    ...
    Wait(1500)
    DoScreenFadeIn(250)
    INDCore.Functions.Notify(Lang:t('success.car_up_for_sale', { value = price }), 'success')
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end

local function SellData(data, model)
    INDCore.Functions.TriggerCallback("IND-vehiclesales:server:CheckModelName",function(DataReturning)
        local vehicleData = {}
        vehicleData.ent = GetVehiclePedIsUsing(PlayerPedId())
        vehicleData.model = DataReturning
        vehicleData.plate = model
        vehicleData.mods = INDCore.Functions.GetVehicleProperties(vehicleData.ent)
        vehicleData.desc = data.desc
        TriggerServerEvent('IND-occasions:server:sellVehicle', data.price, vehicleData)
        ...
        while listen do
            if IsControlJustReleased(0, 38) then -- E
                if spot then
                    ...
                    TriggerEvent('IND-vehiclesales:client:OpenContract', data)
                else
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        listen = false
                        TriggerEvent('IND-occasions:client:MainMenu')
                        --TriggerEvent('IND-vehiclesales:client:SellVehicle')
                    else
                        INDCore.Functions.Notify(Lang:t("error.not_in_veh"), "error", 4500)
                    end
                end
            end
            Wait(0)
    ...
    for k, v in pairs(Config.Zones) do
        ...
        SellSpot:onPlayerInOut(function(isPointInside)
            if isPointInside and Zone ~= k then
                Zone = k
                INDCore.Functions.TriggerCallback('IND-occasions:server:getVehicles', function(vehicles)
...
RegisterNUICallback('sellVehicle', function(data, cb)
    local plate = INDCore.Functions.GetPlate(GetVehiclePedIsUsing(PlayerPedId())) --Getting the plate and sending to the function
    SellData(data,plate)
    cb('ok')
end)
...
RegisterNUICallback('buyVehicle', function(_, cb)
    TriggerServerEvent('IND-occasions:server:buyVehicle', CurrentVehicle)
    cb('ok')
end)

RegisterNUICallback('takeVehicleBack', function(_, cb)
    TriggerServerEvent('IND-occasions:server:ReturnVehicle', CurrentVehicle)
    cb('ok')
end)

-- Events

RegisterNetEvent('IND-occasions:client:BuyFinished', function(vehdata)
    ...
    INDCore.Functions.TriggerCallback('INDCore:Server:SpawnVehicle', function(netId)
        ...
        INDCore.Functions.Notify(Lang:t('success.vehicle_bought'), "success", 2500)
        TriggerEvent("vehiclekeys:client:SetOwner", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Wait(500)
        INDCore.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('IND-occasions:client:SellBackCar', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        INDCore.Functions.TriggerCallback('IND-garage:server:checkVehicleOwner', function(owned, balance)
            if owned then
                if balance < 1 then
                    TriggerServerEvent('IND-occasions:server:sellVehicleBack', vehicleData)
                    INDCore.Functions.DeleteVehicle(vehicle)
                else
                    INDCore.Functions.Notify(Lang:t('error.finish_payments'), 'error', 3500)
                end
            else
                INDCore.Functions.Notify(Lang:t('error.not_your_vehicle'), 'error', 3500)
            end
...
RegisterNetEvent('IND-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Wait(500)
    INDCore.Functions.TriggerCallback('INDCore:Server:SpawnVehicle', function(netId)
        ...
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleFuelLevel(veh, 100)
        INDCore.Functions.Notify(Lang:t('info.vehicle_returned'))
        TriggerEvent("vehiclekeys:client:SetOwner", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Wait(500)
        INDCore.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('IND-occasion:client:refreshVehicles', function()
    if Zone then
        INDCore.Functions.TriggerCallback('IND-occasions:server:getVehicles', function(vehicles)
            despawnOccasionsVehicles()
            spawnOccasionsVehicles(vehicles)
...
RegisterNetEvent('IND-vehiclesales:client:SellVehicle', function()
    local VehiclePlate = INDCore.Functions.GetPlate(GetVehiclePedIsIn(PlayerPedId()))
    INDCore.Functions.TriggerCallback('IND-garage:server:checkVehicleOwner', function(owned, balance)
        if owned then
            if balance < 1 then
                INDCore.Functions.TriggerCallback('IND-occasions:server:getVehicles', function(vehicles)
                    if vehicles == nil or #vehicles < #Config.Zones[Zone].VehicleSpots then
                        ...
                        INDCore.Functions.Notify(Lang:t('error.no_space_on_lot'), 'error', 3500)
                    end
                end)
            else
                INDCore.Functions.Notify(Lang:t('error.finish_payments'), 'error', 3500)
...
RegisterNetEvent('IND-vehiclesales:client:OpenContract', function(data)
    CurrentVehicle = occasionVehicles[Zone][data.Contract]
    if CurrentVehicle then
        INDCore.Functions.TriggerCallback('IND-occasions:server:getSellerInformation', function(info)
            if info then
        ...
        INDCore.Functions.Notify(Lang:t("error.not_for_sale"), 'error', 7500)
    end
end)

RegisterNetEvent('IND-occasions:client:MainMenu', function()
    local MainMenu = {
        {
            isMenuHeader = true,
        ...
        },
        {
            ...
            params = {
                event = 'IND-vehiclesales:client:SellVehicle',
            }
        },
        {
            ...
            params = {
                event = 'IND-occasions:client:SellBackCar',
            }
        }
    }

    exports['IND-menu']:openMenu(MainMenu)
end)
    ...
    for k, cars in pairs(Config.Zones) do
        ...
        SpawnZone[k]:onPlayerInOut(function(isPointInside)
            if isPointInside and IsPedInAnyVehicle(PlayerPedId(), false) then
                exports['IND-core']:DrawText(Lang:t("menu.interaction"), 'left')
                TextShown = true
                Listen4Control()
            else
                listen = false
            ...
            for k2, v in pairs(Config.Zones[k].VehicleSpots) do
                ...
                VehicleZones:onPlayerInOut(function(isPointInside)
                    if isPointInside and IsCarSpawned(k2) then
                        exports['IND-core']:DrawText(Lang:t("menu.view_contract_int"), 'left')
                        TextShown = true
                        Listen4Control(k2)
                    else
                        listen = false
