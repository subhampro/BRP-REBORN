...
--[[ -- Only used by the following "IND-banking:initiateTransfer"
...
RegisterNetEvent('IND-banking:initiateTransfer', function(_)
    --[[
    local _src = source
    local _startChar = INDCore.Functions.GetPlayer(_src)
    ...
    if (checkAccount) then
        ...
        if receiptName ~= false or receiptName ~= nil then
            local userOnline = exports.IND-base:checkOnline(cid)

            if userOnline ~= false then
                -- User is online so we can do a straght transfer
                local _targetUser = exports.IND-base:Source(userOnline)
                if acType == "Current" then
                    ...
                    TriggerClientEvent('IND-banking:openBankScreen', _src)
                    TriggerClientEvent('IND-banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('IND-banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
INDCore.Functions.CreateCallback('IND-banking:getBankingInformation', function(source, cb)
...
RegisterNetEvent('IND-banking:createBankCard', function(pin)
    local src = source
    ...
    TriggerClientEvent('IND-banking:openBankScreen', src)
    TriggerClientEvent('IND-banking:successAlert', src, Lang:t('success.debit_card'))

    TriggerEvent('IND-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** successfully ordered a debit card")
end)

RegisterNetEvent('IND-banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = INDCore.Functions.GetPlayer(src)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('IND-banking:openBankScreen', src)
            TriggerClientEvent('IND-banking:successAlert', src, Lang:t('success.cash_deposit', {value = amount}))
            TriggerEvent('IND-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash deposit of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('IND-banking:toggleCard', function(toggle)
    local src = source
    local xPlayer = INDCore.Functions.GetPlayer(src)
...
RegisterNetEvent('IND-banking:doQuickWithdraw', function(amount, _)
    local src = source
    local xPlayer = INDCore.Functions.GetPlayer(src)
    if not xPlayer then return end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then
            TriggerClientEvent('IND-banking:openBankScreen', src)
            TriggerClientEvent('IND-banking:successAlert', src, Lang:t('success.cash_withdrawal', {value = amount}))
            TriggerEvent('IND-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash withdrawal of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('IND-banking:updatePin', function(currentBankCard, newPin)
    if newPin ~= nil then
        local src = source
        local xPlayer = INDCore.Functions.GetPlayer(src)
        if not xPlayer then return end
        ...
        MySQL.update('UPDATE bank_cards SET cardPin = ? WHERE record_id = ?', {
            ...
            if result == 1 then
                TriggerClientEvent('IND-banking:openBankScreen', src)
                TriggerClientEvent('IND-banking:successAlert', src, Lang:t('success.updated_pin'))
            else
                TriggerClientEvent('INDCore:Notify', src, 'Error updating pin', "error")
            end
        end)
    end
end)

RegisterNetEvent('IND-banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = INDCore.Functions.GetPlayer(src)
    if not xPlayer then return end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('IND-banking:openBankScreen', src)
        TriggerClientEvent('IND-banking:successAlert', src, Lang:t('success.savings_deposit', {value = tostring(amount)}))
        TriggerEvent('IND-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings deposit of $"..tostring(amount).." successfully..")
    end
end)

RegisterNetEvent('IND-banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = INDCore.Functions.GetPlayer(src)
    if not xPlayer then return end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        TriggerClientEvent('IND-banking:openBankScreen', src)
        TriggerClientEvent('IND-banking:successAlert', src, Lang:t('success.savings_withdrawal', {value = tostring(amount)}))
        TriggerEvent('IND-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings withdrawal of $"..tostring(amount).." successfully.")
    end
end)

RegisterNetEvent('IND-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = INDCore.Functions.GetPlayer(src)
    local success = createSavingsAccount(xPlayer.PlayerData.citizenid)
    repeat Wait(0) until success ~= nil
    TriggerClientEvent('IND-banking:openBankScreen', src)
    TriggerClientEvent('IND-banking:successAlert', src, Lang:t('success.opened_savings'))
    TriggerEvent('IND-log:server:CreateLog', 'banking', 'Banking', "lightgreen", "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** opened a savings account")
end)


INDCore.Commands.Add('givecash', Lang:t('command.givecash'), {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Amount'}}, true, function(source, args)
  local src = source
	...
	if id and amount then
		local xPlayer = INDCore.Functions.GetPlayer(src)
		local xReciv = INDCore.Functions.GetPlayer(id)
                    ...
                    if amount > 0 then
                        if xPlayer.Functions.RemoveMoney('cash', amount) then
                            if xReciv.Functions.AddMoney('cash', amount) then
                                TriggerClientEvent('INDCore:Notify', src, Lang:t('success.give_cash',{id = tostring(id), cash = tostring(amount)}), "success")
                                TriggerClientEvent('INDCore:Notify', id, Lang:t('success.received_cash',{id = tostring(src), cash = tostring(amount)}), "success")
                                TriggerClientEvent("payanimation", src)
                            else
                                -- Return player cash
                                xPlayer.Functions.AddMoney('cash', amount)
                                TriggerClientEvent('INDCore:Notify', src, Lang:t('error.not_give'), "error")
                            end
                        else
                            TriggerClientEvent('INDCore:Notify', src, Lang:t('error.not_enough'), "error")
                        end
                    else
                        TriggerClientEvent('INDCore:Notify', src, Lang:t('error.invalid_amount'), "error")
                    end
				else
					TriggerClientEvent('INDCore:Notify', src, Lang:t('error.too_far_away'), "error")
				end
			else
				TriggerClientEvent('INDCore:Notify', src, Lang:t('error.dead'), "error")
			end
		else
			TriggerClientEvent('INDCore:Notify', src, Lang:t('error.wrong_id'), "error")
		end
	else
		TriggerClientEvent('INDCore:Notify', src, Lang:t('error.givecash'), "error")
	end
end)
