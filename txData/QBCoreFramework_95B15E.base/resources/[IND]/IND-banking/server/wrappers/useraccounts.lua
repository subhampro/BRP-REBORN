        ...
        if not self.cardActive then
            ...
            MySQL.update('UPDATE bank_cards SET cardnumber = ?, cardPin = ?, cardDecrypted = ?, cardActive = ?, cardLocked = ?, cardType = ? WHERE citizenid = ? AND record_id = ?', {
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('IND-banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
                    local xPlayer = INDCore.Functions.GetPlayer(self.source)
...
RegisterNetEvent('IND-banking:server:registerCurrentAccount', function(cid)
...
RegisterNetEvent('IND-banking:server:registerSavingsAccount', function(cid)
