...
local function ShowEntranceHeaderMenu()
    ...
    exports['IND-menu']:showHeader(headerMenu)
end
...
local function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Wait(250)
    INDCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            INDCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['IND-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                Wait(100)
                TriggerServerEvent('IND-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('IND-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('IND-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
            TriggerServerEvent("apartments:server:setCurrentApartment", CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('IND-interior:client:SetNewState', true)
            else
                TriggerEvent('IND-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('IND-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_open", 0.1)
    openHouseAnim()
    TriggerServerEvent("IND-apartments:returnBucket")
    DoScreenFadeOut(500)
    ...
    exports['IND-interior']:DespawnInterior(HouseObj, function()
        ...
        TriggerServerEvent('IND-apartments:server:SetInsideMeta', CurrentApartment, false)
...
local function SetClosestApartment()
    ...
    if current ~= ClosestHouse and LocalPlayer.state.isLoggedIn and not InApartment then
        ...
        INDCore.Functions.TriggerCallback('apartments:IsOwner', function(result)
            IsOwned = result
            DeleteApartmentsEntranceTargets()
            DeleteInApartmentTargets()
        end, ClosestHouse)
...
function MenuOwners()
    INDCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            INDCore.Functions.Notify(Lang:t('error.nobody_home'), "error", 3500)
            CloseMenuFull()
        else
            local apartmentMenu = {
                {
            ...
            exports['IND-menu']:openMenu(apartmentMenu)
        end
    end, ClosestHouse)
end
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    INDCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('IND-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('IND-spawn:client:openUI', true)
            TriggerEvent("apartments:client:SetHomeBlip", result.type)
        else
            if Apartments.Starting then
                TriggerEvent('IND-spawn:client:setupSpawns', cData, true, Apartments.Locations)
                TriggerEvent('IND-spawn:client:openUI', true)
            else
                TriggerEvent('IND-spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('IND-spawn:client:openUI', true)
...
RegisterNetEvent('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if RangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            INDCore.Functions.Notify(Lang:t('error.to_far_from_door'))
            return
        end
    end
...
RegisterNetEvent('IND-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
RegisterNetEvent('apartments:client:RingDoor', function(player, _)
    ...
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "doorbell", 0.1)
    INDCore.Functions.Notify(Lang:t('info.at_the_door'))
end)
