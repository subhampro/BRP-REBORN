...
INDCore.Functions.CreateUseableItem("tunerlaptop", function(source)
    TriggerClientEvent('IND-tunerchip:client:openChip', source)
end)

RegisterNetEvent('IND-tunerchip:server:TuneStatus', function(plate, bool)
    if bool then
...
INDCore.Functions.CreateCallback('IND-tunerchip:server:HasChip', function(source, cb, data)
    local src = source
...
INDCore.Functions.CreateCallback('IND-tunerchip:server:GetStatus', function(_, cb, plate)
    cb(tunedVehicles[plate])
end)

INDCore.Functions.CreateUseableItem("nitrous", function(source)
