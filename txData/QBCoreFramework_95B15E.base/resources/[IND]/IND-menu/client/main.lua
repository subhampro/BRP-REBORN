...
local function openMenu(data, sort, skipFirst)
	...
	for _,v in pairs(data) do
		if v["icon"] then
			local img = "IND-inventory/html/"
			if INDCore.Shared.Items[tostring(v["icon"])] then
				if not string.find(INDCore.Shared.Items[tostring(v["icon"])].image, "images/") then
					img = img.."images/"
				end
				v["icon"] = img..INDCore.Shared.Items[tostring(v["icon"])].image
			end
		end
	end
...
RegisterNetEvent('IND-menu:client:openMenu', function(data, sort, skipFirst)
    openMenu(data, sort, skipFirst)
end)

RegisterNetEvent('IND-menu:client:closeMenu', function()
    closeMenu()
end)
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isQBCommand then
                    TriggerServerEvent('INDCore:CallCommand', data.params.event, data.params.args)
...
RegisterNUICallback('closeMenu', function(_, cb)
    ...
    SetNuiFocus(false)
    cb('ok')
    TriggerEvent("IND-menu:client:menuClosed")
end)
