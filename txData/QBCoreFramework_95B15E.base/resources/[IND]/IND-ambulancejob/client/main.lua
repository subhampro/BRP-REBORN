...
local function DoLimbAlert()
    if not isDead and not InLaststand then
        if #injured > 0 then
            ...
            INDCore.Functions.Notify(limbDamageMsg, "primary")
        end
    end
end
...
local function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        INDCore.Functions.Notify(Lang:t('info.bleed_alert', {bleedstate = Config.BleedingStates[tonumber(isBleeding)].label}), "error")
    end
end
...
local function CheckWeaponDamage(ped)
    ...
    for k, v in pairs(INDCore.Shared.Weapons) do
        if HasPedBeenDamagedByWeapon(ped, k, 0) then
            detected = true
...
RegisterNetEvent('hospital:client:ambulanceAlert', function(coords, text)
    ...
    INDCore.Functions.Notify({text = text, caption = street1name.. ' ' ..street2name}, 'ambulance')
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    emsNotified = false
    INDCore.Functions.Notify(Lang:t('info.healthy'))
end)
...
RegisterNetEvent('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    INDCore.Functions.Notify(Lang:t('success.wounds_healed'), 'success')
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function ()
        Wait(5)
        if isRevive then
            INDCore.Functions.Notify(Lang:t('success.being_helped'), 'success')
...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    ...
    if exports["IND-policejob"]:IsHandcuffed() then
...
RegisterNetEvent('hospital:client:SendBillEmail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        if INDCore.Functions.GetPlayerData().charinfo.gender == 1 then
            gender = Lang:t('info.mrs')
        end
        local charinfo = INDCore.Functions.GetPlayerData().charinfo
...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            exports['IND-core']:DrawText(Lang:t('text.bed_out'))
        ...
        while listen do
            if IsControlJustPressed(0, 38) then
                ...
                if variable == "checkin" then
                   TriggerEvent('IND-ambulancejob:checkin')
                    listen = false
                elseif variable == "beds" then
                    TriggerEvent('IND-ambulancejob:beds')
                    listen = false
                end
            end
            Wait(1)
...
RegisterNetEvent('IND-ambulancejob:checkin', function()
    if doctorCount >= Config.MinimalDoctors then
        TriggerServerEvent("hospital:server:SendDoctorAlert")
    else
        TriggerEvent('animations:client:EmoteCommandStart', {"notepad"})
        INDCore.Functions.Progressbar("hospital_checkin", Lang:t('progress.checking_in'), 2000, false, true, {
            ...
            if bedId then
                ...
                INDCore.Functions.Notify(Lang:t('error.beds_taken'), "error")
            end
        end, function() -- Cancel
            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
...
RegisterNetEvent('IND-ambulancejob:beds', function()
...
if Config.UseTarget then
    CreateThread(function()
        for k, v in pairs(Config.Locations["checking"]) do
            exports['IND-target']:AddBoxZone("checking"..k, vector3(v.x, v.y, v.z), 3.5, 2, {
                name = "checking"..k,
                heading = -72,
                debugPoly = false,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        ...
                        event = "IND-ambulancejob:checkin",
                        label = "Check In",
                    }
                },
                distance = 1.5
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            exports['IND-target']:AddBoxZone("beds"..k,  v.coords, 2.5, 2.3, {
                name = "beds"..k,
                heading = -20,
                debugPoly = false,
                minZ = v.coords.z - 1,
    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["checking"]) do
            ...
            checkingCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    if doctorCount >= Config.MinimalDoctors then
                        exports['IND-core']:DrawText(Lang:t('text.call_doc'),'left')
                        CheckInControls("checkin")
                    else
                        exports['IND-core']:DrawText(Lang:t('text.check_in'), 'left')
                        CheckInControls("checkin")
                    end
                else
                    listen = false
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            ...
            bedCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    exports['IND-core']:DrawText(Lang:t('text.lie_bed'), 'left')
