...
RegisterNetEvent('prison:server:SetJailStatus', function(jailTime)
    ...
    if jailTime > 0 then
        if Player.PlayerData.job.name ~= "unemployed" then
            ...
            TriggerClientEvent('INDCore:Notify', src, Lang:t("info.lost_job"))
        end
    else
        GotItems[source] = nil
    end
...
RegisterNetEvent('prison:server:CheckChance', function()
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, INDCore.Shared.Items['phone'], 'add')
    TriggerClientEvent('INDCore:Notify', src, Lang:t('success.found_phone'), 'success')
    GotItems[src] = true
end)

INDCore.Functions.CreateCallback('prison:server:IsAlarmActive', function(_, cb)
    cb(AlarmActivated)
end)
