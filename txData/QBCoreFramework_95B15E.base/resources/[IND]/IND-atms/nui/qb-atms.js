...
function closeBanking() {
    $.post("https://IND-atms/NUIFocusOff", JSON.stringify({}));
    currentLimit = null;
    clientPin = null;
    cardSelected = null;
    clientCid = null;
...
$( function() {
        ...
        if ($('#pinContainer').is(':visible')) {
            if (key.which >= 48 && key.which <= 57) { // 48 - 57 (0-9 keys)
                ...
                if (currentVal == clientPin) {
                    ...
                    $('#pinCode').val('');
                    $.post("https://IND-atms/loadBankingAccount", JSON.stringify({
                        cid: clientCid,
                        cardnumber: cardNumb
                    }));
                } else {
    ...
    $(document).on('click','[data-action=useCard]',function(){
        $.post("https://IND-atms/playATMAnim", JSON.stringify({}));
    ...
    $(document).on('click','[data-action=removeCard]',function(){
        var selectedCard = $(this).data('card')
        $.post("https://IND-atms/removeCard", JSON.stringify({
    ...
    document.querySelectorAll('[data-withdrawal]').forEach(function (element) {
        element.addEventListener('click', (evt) => {
            ...
            $.post("https://IND-atms/doATMWithdraw", JSON.stringify({
