...
RegisterNetEvent('IND-commandbinding:client:openUI', function()
    openBindingMenu()
end)
...
for k, v in pairs(availableKeys) do
    RegisterCommand(v[1], function()
        if LocalPlayer.state.isLoggedIn and not keyPressed and GetLastInputMethod(0) then
            local keyMeta = INDCore.Functions.GetPlayerData().metadata["commandbinds"]
            local args
            if next(keyMeta) ~= nil then
                if keyMeta[v[2]]["command"] ~= "" then
                    ...
                    TriggerServerEvent('INDCore:CallCommand', keyMeta[v[2]]["command"], args)
                    keyPressed = true
                    Wait(1000)
                    keyPressed = false
                else
                    INDCore.Functions.Notify('There is still nothing ['..v[2]..'] bound, /binds to bind a command', 'primary', 4000)
                end
            end
        end
    end, false)
...
RegisterNUICallback('save', function(data, cb)
    ...
    INDCore.Functions.Notify('Command bindings have been saved!', 'success')
    TriggerServerEvent('IND-commandbinding:server:setKeyMeta', keyData)
    cb('ok')
end)
