...
INDCore.Functions.CreateCallback('IND-vehicletuning:server:GetDrivingDistances', function(_, cb)
    cb(VehicleDrivingDistance)
end)

INDCore.Functions.CreateCallback('IND-vehicletuning:server:IsVehicleOwned', function(_, cb, plate)
    local retval = false
    local result = MySQL.scalar.await('SELECT 1 from player_vehicles WHERE plate = ?', {plate})
    if result then
        retval = true
...
INDCore.Functions.CreateCallback('IND-vehicletuning:server:GetAttachedVehicle', function(_, cb)
    cb(Config.Plates)
end)

INDCore.Functions.CreateCallback('IND-vehicletuning:server:IsMechanicAvailable', function(_, cb)
    local amount = 0
...
INDCore.Functions.CreateCallback('IND-vehicletuning:server:GetStatus', function(_, cb, plate)
...
RegisterNetEvent('IND-vehicletuning:server:SaveVehicleProps', function(vehicleProps)
...
RegisterNetEvent('IND-vehicletuning:server:UpdateDrivingDistance', function(amount, plate)
    VehicleDrivingDistance[plate] = amount
    TriggerClientEvent('IND-vehicletuning:client:UpdateDrivingDistance', -1, VehicleDrivingDistance[plate], plate)
...
RegisterNetEvent('IND-vehicletuning:server:LoadStatus', function(veh, plate)
...
RegisterNetEvent('IND-vehicletuning:server:SetPartLevel', function(plate, part, level)
    if VehicleStatus[plate] ~= nil then
        VehicleStatus[plate][part] = level
        TriggerClientEvent("vehiclemod:client:setVehicleStatus", -1, plate, VehicleStatus[plate])
    end
...
RegisterNetEvent('IND-vehicletuning:server:SetAttachedVehicle', function(veh, k)
    if veh ~= false then
        Config.Plates[k].AttachedVehicle = veh
        TriggerClientEvent('IND-vehicletuning:client:SetAttachedVehicle', -1, veh, k)
    else
        Config.Plates[k].AttachedVehicle = nil
        TriggerClientEvent('IND-vehicletuning:client:SetAttachedVehicle', -1, false, k)
    end
end)

RegisterNetEvent('IND-vehicletuning:server:CheckForItems', function(part)
    local src = source
    ...
    if RepairPart ~= nil then
        if RepairPart.amount >= Config.RepairCostAmount[part].costs then
            TriggerClientEvent('IND-vehicletuning:client:RepaireeePart', src, part)
            ...
            for _ = 1, Config.RepairCostAmount[part].costs, 1 do
                TriggerClientEvent('inventory:client:ItemBox', src,
                    INDCore.Shared.Items[Config.RepairCostAmount[part].item], "remove")
                Wait(500)
            end
        else
            TriggerClientEvent('INDCore:Notify', src, Lang:t('notifications.not_enough') .. INDCore.Shared.Items[Config.RepairCostAmount[part].item]["label"] .. " (min. " ..
                    Config.RepairCostAmount[part].costs .. "x)", "error")
        end
    else
        TriggerClientEvent('INDCore:Notify', src, Lang:t('notifications.not_have') ..
            INDCore.Shared.Items[Config.RepairCostAmount[part].item]["label"], "error")
    end
end)

RegisterNetEvent('IND-mechanicjob:server:removePart', function(part, amount)
...
INDCore.Commands.Add("setvehiclestatus", "Set Vehicle Status", {{
    name = "part",
...
INDCore.Commands.Add("setmechanic", "Give Someone The Mechanic job", {{
    name = "id",
    ...
    if IsAuthorized(Player.PlayerData.citizenid) then
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                ...
                TriggerClientEvent('INDCore:Notify', TargetData.PlayerData.source,
                    "You Were Hired As An Autocare Employee!")
                TriggerClientEvent('INDCore:Notify', source, "You have (" .. TargetData.PlayerData.charinfo.firstname ..
                    ") Hired As An Autocare Employee!")
            end
        else
            TriggerClientEvent('INDCore:Notify', source, "You Must Provide A Player ID!")
        end
    else
        TriggerClientEvent('INDCore:Notify', source, "You Cannot Do This!", "error")
    end
end)

INDCore.Commands.Add("firemechanic", "Fire A Mechanic", {{
    name = "id",
    help = "ID Of The Player"
}}, false, function(source, args)
    local Player = INDCore.Functions.GetPlayer(source)
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                if TargetData.PlayerData.job.name == "mechanic" then
                    ...
                    TriggerClientEvent('INDCore:Notify', source,
                        "You have (" .. TargetData.PlayerData.charinfo.firstname .. ") Fired As Autocare Employee!")
                else
                    TriggerClientEvent('INDCore:Notify', source, "Youre Not An Employee of Autocare!", "error")
                end
            end
        else
            TriggerClientEvent('INDCore:Notify', source, "You Must Provide A Player ID!", "error")
        end
    else
        TriggerClientEvent('INDCore:Notify', source, "You Cannot Do This!", "error")
    end
