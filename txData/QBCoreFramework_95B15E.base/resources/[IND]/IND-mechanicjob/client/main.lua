...
local function DeleteTarget(id)
    if Config.UseTarget then
        exports['IND-target']:RemoveZone(id)
    else
...
local function RegisterDutyTarget()
    ...
    if Config.UseTarget then
        exports['IND-target']:AddBoxZone(dutyTargetBoxID, coords, 1.5, 1.5, {
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['IND-core']:DrawText("[E] " .. label, 'left')
            else
...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        exports['IND-target']:AddBoxZone(stashTargetBoxID, coords, 1.5, 1.5, {
            ...
            options = {{
                type = "client",
                event = "IND-mechanicjob:client:target:OpenStash",
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['IND-core']:DrawText(Lang:t('labels.o_stash'), 'left')
            else
                exports['IND-core']:HideText()
            end
...
local function RegisterGarageZone()
    ...
    vehicleZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            ...
            if inVehicle then
                exports['IND-core']:DrawText(Lang:t('labels.h_vehicle'), 'left')
            else
                exports['IND-core']:DrawText(Lang:t('labels.g_vehicle'), 'left')
            end
        else
            exports['IND-core']:HideText()
        end
    ...
    plateZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            if plate.AttachedVehicle then
                exports['IND-core']:DrawText(Lang:t('labels.o_menu'), 'left')
            else
                if IsPedInAnyVehicle(PlayerPedId()) then
                    exports['IND-core']:DrawText(Lang:t('labels.work_v'), 'left')
                end
            end
        else
            exports['IND-core']:HideText()
...
local function SendStatusMessage(statusList)
    if statusList ~= nil then
        TriggerEvent('chat:addMessage', {
            ...
            args = {Lang:t('labels.veh_status'), round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..INDCore.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..INDCore.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..INDCore.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..INDCore.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..INDCore.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..INDCore.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..INDCore.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
    ...
    local openMenu = {
        {
            ...
            params = {
                event = "IND-mechanicjob:client:UnattachVehicle",
            }
        }, {
            header = Lang:t('lift_menu.header_stats'),
            txt = Lang:t('lift_menu.desc_stats'),
            params = {
                event = "IND-mechanicjob:client:CheckStatus",
                args = {
                    number = 1,
                }
            }
            ...
            params = {
                event = "IND-mechanicjob:client:PartsMenu",
                args = {
                    number = 1,
                }
            }
            ...
            txt = "",
            params = {
                event = "IND-mechanicjob:client:target:CloseMenu",
            }
        },
    }
    exports['IND-menu']:openMenu(openMenu)
end

local function PartsMenu()
    local plate = INDCore.Functions.GetPlate(Config.Plates[ClosestPlate].AttachedVehicle)
    if VehicleStatus[plate] ~= nil then
        local vehicleMenu = {
            {
                header = "Status",
        ...
        for k,v in pairs(Config.ValuesLabels) do
            if math.ceil(VehicleStatus[plate][k]) ~= Config.MaxStatusValues[k] then
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    params = {
                        event = "IND-mechanicjob:client:PartMenu",
                        args = {
                            name = v,
                            parts = k
                        }
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    txt = Lang:t('parts_menu.status') .. percentage .. ".0% / 100.0%",
                    params = {
                        event = "IND-mechanicjob:client:NoDamage",
                    }
                }
            end
        end
...
local function PartMenu(data)
    ...
    local TestMenu1 = {
        ...
        {
            header = ""..partName.."",
            txt = Lang:t('parts_menu.repair_op')..INDCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x",
            params = {
                event = "IND-mechanicjob:client:RepairPart",
                args = {
                    part = part,
                }
            }
    ...
    exports['IND-menu']:openMenu(TestMenu1)
end
...
local function NoDamage()
    ...
    exports['IND-menu']:openMenu(noDamage)
end
...
local function UnattachVehicle()
    ...
    TriggerServerEvent('IND-vehicletuning:server:SetAttachedVehicle', false, ClosestPlate)
...
local function VehicleList()
    ...
    for k,v in pairs(Config.Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "Vehicle: "..v.."",
            params = {
                event = "IND-mechanicjob:client:SpawnListVehicle",
                args = {
                    headername = v,
                    spawnName = k
                }
...
local function RepairPart(part)
    ...
    local indx = 0
    local countitem = 0
    INDCore.Functions.TriggerCallback('IND-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            INDCore.Functions.Progressbar("repair_part", Lang:t('labels.progress_bar') ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                TriggerEvent('IND-vehicletuning:client:RepaireeePart', part)
                TriggerServerEvent('IND-inventory:server:SaveStashItems', "mechanicstash", StashItems)
                SetTimeout(250, function()
                    PartsMenu()
                end)
            end, function()
                INDCore.Functions.Notify(Lang:t('notifications.rep_canceled'), "error")
            end)
        else
            INDCore.Functions.Notify(Lang:t('notifications.not_materials'), 'error')
        end
    end, "mechanicstash")
end
...
RegisterNetEvent("IND-mechanicjob:client:UnattachVehicle",function()
    UnattachVehicle()
end)

RegisterNetEvent("IND-mechanicjob:client:PartsMenu",function()
    PartsMenu()
end)

RegisterNetEvent("IND-mechanicjob:client:PartMenu",function(data)
    PartMenu(data)
end)

RegisterNetEvent("IND-mechanicjob:client:NoDamage",function()
    NoDamage()
end)

RegisterNetEvent("IND-mechanicjob:client:CheckStatus",function()
    CheckStatus()
end)

RegisterNetEvent("IND-mechanicjob:client:SpawnListVehicle",function(data)
    local vehicleSpawnName = data.spawnName
    SpawnListVehicle(vehicleSpawnName)
end)

RegisterNetEvent("IND-mechanicjob:client:RepairPart",function(data)
...
RegisterNetEvent('INDCore:Client:OnPlayerLoaded', function()
    INDCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.onduty then
            if PlayerJob.type == 'mechanic' then
                TriggerServerEvent("INDCore:ToggleDuty")
            end
        end
    end)
    INDCore.Functions.TriggerCallback('IND-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    INDCore.Functions.TriggerCallback('IND-vehicletuning:server:GetDrivingDistances', function(retval)
...
RegisterNetEvent('INDCore:Client:SetDuty', function(duty)
    onDuty = duty
...
RegisterNetEvent('IND-vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
...
RegisterNetEvent('IND-vehicletuning:client:RepaireeePart', function(part)
    ...
    INDCore.Functions.Notify(Lang:t('notifications.partrep', {value = Config.ValuesLabels[part]}))
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then
                        ...
                        INDCore.Functions.Notify(Lang:t('notifications.uknown'), "error")
                    end
                else
                    INDCore.Functions.Notify(Lang:t('notifications.not_valid'), "error")
                end
            else
                INDCore.Functions.Notify(Lang:t('notifications.not_close'), "error")
            end
        else
            INDCore.Functions.Notify(Lang:t('notifications.veh_first'), "error")
        end
    else
        INDCore.Functions.Notify(Lang:t('notifications.outside'), "error")
    end
end)
...
RegisterNetEvent('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            INDCore.Functions.Notify(Lang:t('notifications.wrong_seat'), "error")
        end
    else
        INDCore.Functions.Notify(Lang:t('notifications.not_vehicle'), "error")
    end
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                        ...
                        INDCore.Functions.Progressbar("repair_advanced", Lang:t('notifications.progress_bar'), lockpickTime, false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            if part == "body" then
                                ...
                                TriggerServerEvent("IND-mechanicjob:server:removePart", part, needAmount)
                                TriggerEvent("inventory:client:ItemBox", INDCore.Shared.Items[Config.RepairCost[part]], "remove")
                            elseif part ~= "engine" then
                                TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                TriggerServerEvent("IND-mechanicjob:server:removePart", part, level)
                                TriggerEvent("inventory:client:ItemBox", INDCore.Shared.Items[Config.RepairCost[part]], "remove")
                            end
                        end, function() -- Cancel
                            openingDoor = false
                            ClearPedTasks(PlayerPedId())
                            INDCore.Functions.Notify(Lang:t('notifications.process_canceled'), "error")
                        end)
                    else
                        INDCore.Functions.Notify(Lang:t('notifications.not_part'), "error")
                    end
                else
                    INDCore.Functions.Notify(Lang:t('notifications.not_valid'), "error")
                end
...
RegisterNetEvent('IND-mechanicjob:client:target:OpenStash', function ()
...
RegisterNetEvent('IND-mechanicjob:client:target:CloseMenu', function()
    DestroyVehiclePlateZone(ClosestPlate)
    RegisterVehiclePlateZone(ClosestPlate, Config.Plates[ClosestPlate])

    TriggerEvent('IND-menu:client:closeMenu')
end)
        ...
        if PlayerJob.type == 'mechanic' then
            ...
            if onDuty then
                if isInsideStashZone then
                    wait = 0
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent("IND-mechanicjob:client:target:OpenStash")
                    end
                end
                ...
                if isInsideVehiclePlateZone then
                    ...
                    if attachedVehicle then
                        ...
                        if IsControlJustPressed(0, 38) and IsPedInAnyVehicle(PlayerPedId()) then
                            ...
                            Wait(500)
                            DoScreenFadeIn(150)
                            TriggerServerEvent('IND-vehicletuning:server:SetAttachedVehicle', veh, ClosestPlate)
