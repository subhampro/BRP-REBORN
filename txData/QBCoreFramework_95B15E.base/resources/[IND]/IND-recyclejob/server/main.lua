...
RegisterNetEvent('IND-recycle:server:getItem', function()
  local src = source
  ...
  for _ = 1, math.random(1, Config.MaxItemsReceived), 1 do
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, INDCore.Shared.Items[randItem], 'add')
    Wait(500)
  end
  ...
  if chance < 7 then
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, INDCore.Shared.Items[Config.ChanceItem], 'add')
  end
  ...
  if luck == odd then
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, INDCore.Shared.Items[Config.LuckyItem], 'add')
  end
end)
