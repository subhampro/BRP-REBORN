                ...
                if dist < 20 then
                    ...
                    if dist < 5 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            TriggerServerEvent('IND-taxi:server:NpcPay', meterData.currentFare)
                            meterActive = false
                            SendNUIMessage({
                                action = "resetMeter"
                            })
                            INDCore.Functions.Notify(Lang:t("info.person_was_dropped_off"), 'success')
                            if NpcData.DeliveryBlip ~= nil then
                                RemoveBlip(NpcData.DeliveryBlip)
                            end
...
function TaxiGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = v.label,
            params = {
                event = "IND-taxi:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("IND-taxi:client:TakeVehicle", function(data)
    ...
    if SpawnPoint then
        ...
        if CanSpawn then
            ...
            INDCore.Functions.Notify(Lang:t("info.no_spawn_point"), "error")
        end
    else
        INDCore.Functions.Notify(Lang:t("info.no_spawn_point"), 'error')
        return
...
-- Events
RegisterNetEvent('IND-taxi:client:DoTaxiNpc', function()
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            INDCore.Functions.Notify(Lang:t("info.npc_on_gps"), 'success')
           ...
           if not Config.UseTarget then
                    ...
                    if dist < 20 then
                        ...
                        if dist < 5 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                INDCore.Functions.Notify(Lang:t("info.go_to_location"))
                                if NpcData.NpcBlip ~= nil then
                                    RemoveBlip(NpcData.NpcBlip)
                                end
                                GetDeliveryLocation()
            ...
            INDCore.Functions.Notify(Lang:t("error.already_mission"))
        end
    else
        INDCore.Functions.Notify(Lang:t("error.not_in_taxi"))
    end
end)

RegisterNetEvent('IND-taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            INDCore.Functions.Notify(Lang:t("error.missing_meter"), 'error')
        end
    else
        INDCore.Functions.Notify(Lang:t("error.no_vehicle"), 'error')
    end
end)

RegisterNetEvent('IND-taxi:client:enableMeter', function()
    if meterIsOpen then
        SendNUIMessage({
            action = "toggleMeter"
        })
    else
        INDCore.Functions.Notify(Lang:t("error.not_active_meter"), 'error')
    end
end)

RegisterNetEvent('IND-taxi:client:toggleMuis', function()
    Wait(400)
    if meterIsOpen then
        ...
        INDCore.Functions.Notify(Lang:t("error.no_meter_sight"), 'error')
    end
end)
...
RegisterNetEvent('IND-taxijob:client:requestcab', function()
    TaxiGarage()
end)
    ...
    while true do
        if not Config.UseTarget then
            ...
            if LocalPlayer.state.isLoggedIn then
                local Player = INDCore.Functions.GetPlayerData()
        ...
        exports['IND-target']:SpawnPed({
            ...
            target = {
                options = {
                    {
                        type = "client",
                        event = "IND-taxijob:client:requestcab",
    ...
    zone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsidePickupZone and not NpcData.NpcTaken then
                ...
                exports['IND-core']:DrawText(Lang:t("info.call_npc"), Config.DefaultTextLocation)
                callNpcPoly()
            end
        else
    ...
    delieveryZone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsideDropZone and NpcData.NpcTaken then
                ...
                exports['IND-core']:DrawText(Lang:t("info.drop_off_npc"), Config.DefaultTextLocation)
                dropNpcPoly()
            end
        else
...
function setupCabParkingLocation()
    ...
    taxiParking:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside and not Notified and Config.UseTarget then
            if whitelistedVehicle() then
                exports['IND-core']:DrawText(Lang:t("info.vehicle_parking"), Config.DefaultTextLocation)
                Notified = true
        ...
        if isPlayerInsideZone then
            if IsControlJustReleased(0, 38) then
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    if meterIsOpen then
                        TriggerEvent('IND-taxi:client:toggleMeter')
                        meterActive = false
                    end
                    TaskLeaveVehicle(PlayerPedId(), vehicle, 0)
                    Wait(2000) -- 2 second delay just to ensure the player is out of the vehicle
                    DeleteVehicle(vehicle)
                    INDCore.Functions.Notify(Lang:t("info.taxi_returned"), 'success')
                end
            end
        end
        Wait(1)
...
-- switched boss menu from IND-bossmenu to taxijob
CreateThread(function()
    while true do
        local sleep = 1000
