...
local PlayerData = INDCore.Functions.GetPlayerData() -- Just for resource restart (same as event handler)
...
local function connecttoradio(channel)
    ...
    if SplitStr(tostring(channel), ".")[2] ~= nil and SplitStr(tostring(channel), ".")[2] ~= "" then
        INDCore.Functions.Notify(Config.messages['joined_to_radio'] ..channel.. ' MHz', 'success')
    else
        INDCore.Functions.Notify(Config.messages['joined_to_radio'] ..channel.. '.00 MHz', 'success')
    end
end
...
local function leaveradio()
    ...
    INDCore.Functions.Notify(Config.messages['you_leave'] , 'error')
end
...
RegisterNetEvent('IND-radio:use', function()
    toggleRadio(not radioMenu)
end)

RegisterNetEvent('IND-radio:onRadioDrop', function()
            ...
            if rchannel ~= RadioChannel then
                if Config.RestrictedChannels[rchannel] ~= nil then
                    if Config.RestrictedChannels[rchannel][PlayerData.job.name] and PlayerData.job.onduty then
                        ...
                        INDCore.Functions.Notify(Config.messages['restricted_channel_error'], 'error')
                    end
                else
                    connecttoradio(rchannel)
                end
            else
                INDCore.Functions.Notify(Config.messages['you_on_radio'] , 'error')
            end
        else
            INDCore.Functions.Notify(Config.messages['invalid_radio'] , 'error')
        end
    else
        INDCore.Functions.Notify(Config.messages['invalid_radio'] , 'error')
    end
...
RegisterNUICallback('leaveRadio', function(_, cb)
    if RadioChannel == 0 then
        INDCore.Functions.Notify(Config.messages['not_on_radio'], 'error')
    else
        leaveradio()
    end
    cb("ok")
...
RegisterNUICallback("volumeUp", function(_, cb)
	if RadioVolume <= 95 then
		...
		INDCore.Functions.Notify(Config.messages["volume_radio"] .. RadioVolume, "success")
		exports["pma-voice"]:setRadioVolume(RadioVolume)
	else
		INDCore.Functions.Notify(Config.messages["decrease_radio_volume"], "error")
	end
    cb('ok')
end)
...
RegisterNUICallback("volumeDown", function(_, cb)
	if RadioVolume >= 10 then
		...
		INDCore.Functions.Notify(Config.messages["increase_radio_volume"], "error")
	end
    cb('ok')
end)
...
RegisterNUICallback("increaseradiochannel", function(_, cb)
    ...
    INDCore.Functions.Notify(Config.messages["increase_decrease_radio_channel"] .. newChannel, "success")
    cb("ok")
end)
