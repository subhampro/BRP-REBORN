local INDCore = exports[Config.corename]:GetCoreObject()
local INDPhone = {}
local AppAlerts = {}
...
local function GetOnlineStatus(number)
    local Target = INDCore.Functions.GetPlayerByPhone(number)
    local retval = false
    if Target ~= nil then
        retval = true
    end
...
function INDPhone.AddMentionedTweet(citizenid, TweetData)
...
function INDPhone.SetPhoneAlerts(citizenid, app, alerts)
...
local function sendNewMailToOffline(citizenid, mailData)
    ...
    if Player then
        ...
        if mailData.button == nil then
            ...
            TriggerClientEvent('IND-phone:client:NewMailNotify', src, mailData)
        else
        ...
        SetTimeout(200, function()
            ...
            TriggerClientEvent('IND-phone:client:UpdateMails', src, mails)
        end)
    else
        if mailData.button == nil then
...
INDCore.Functions.CreateCallback('IND-phone:server:GetCallState', function(_, cb, ContactData)
    local Target = INDCore.Functions.GetPlayerByPhone(ContactData.number)
    if Target ~= nil then
...
INDCore.Functions.CreateCallback('IND-phone:server:GetPhoneData', function(source, cb)
    local src = source
    ...
    if Player ~= nil then
        ...
        if invoices[1] ~= nil then
            for _, v in pairs(invoices) do
                local Ply = INDCore.Functions.GetPlayerByCitizenId(v.sender)
                if Ply ~= nil then
        ...
        if garageresult[1] ~= nil then
            for _, v in pairs(garageresult) do
                ...
                if (INDCore.Shared.Vehicles[vehicleModel] ~= nil) and (Config.Garages[v.garage] ~= nil) then
                    v.garage = Config.Garages[v.garage].label
                    v.vehicle = INDCore.Shared.Vehicles[vehicleModel].name
                    v.brand = INDCore.Shared.Vehicles[vehicleModel].brand
                end
...
INDCore.Functions.CreateCallback('IND-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Invoices = {}
    local Ply = INDCore.Functions.GetPlayer(source)
    local SenderPly = INDCore.Functions.GetPlayerByCitizenId(sendercitizenid)
    ...
    exports['IND-phone']:sendNewMailToOffline(sendercitizenid, invoiceMailData)
    exports['IND-management']:AddMoney(society, amount)
...
INDCore.Functions.CreateCallback('IND-phone:server:DeclineInvoice', function(source, cb, _, _, invoiceId)
    local Invoices = {}
    local Ply = INDCore.Functions.GetPlayer(source)
    MySQL.query('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
    local invoices = MySQL.query.await('SELECT * FROM phone_invoices WHERE citizenid = ?', {Ply.PlayerData.citizenid})
...
INDCore.Functions.CreateCallback('IND-phone:server:GetContactPictures', function(_, cb, Chats)
...
INDCore.Functions.CreateCallback('IND-phone:server:GetContactPicture', function(_, cb, Chat)
...
INDCore.Functions.CreateCallback('IND-phone:server:GetPicture', function(_, cb, number)
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', {query})
    if result[1] ~= nil then
...
INDCore.Functions.CreateCallback('IND-phone:server:FetchResult', function(_, cb, search)
...
INDCore.Functions.CreateCallback('IND-phone:server:GetVehicleSearchResults', function(_, cb, search)
    search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
    ...
    if result[1] ~= nil then
        for k, _ in pairs(result) do
            ...
            if player[1] ~= nil then
                ...
                local vehicleInfo = INDCore.Shared.Vehicles[result[k].vehicle]
...
INDCore.Functions.CreateCallback('IND-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData
    if plate ~= nil then
        ...
        TriggerClientEvent('INDCore:Notify', src, 'No Vehicle Nearby', 'error')
        cb(nil)
    end
end)

INDCore.Functions.CreateCallback('IND-phone:server:HasPhone', function(source, cb)
...
INDCore.Functions.CreateCallback('IND-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        ...
        local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', {query})
        if result[1] ~= nil then
            local Reciever = INDCore.Functions.GetPlayerByCitizenId(result[1].citizenid)
...
INDCore.Functions.CreateCallback('IND-phone:server:GetCurrentLawyers', function(_, cb)
    local Lawyers = {}
...
INDCore.Functions.CreateCallback("IND-phone:server:GetWebhook",function(_,cb)
	if WebHook ~= "" then
		cb(WebHook)
	else
...
RegisterNetEvent('IND-phone:server:AddAdvert', function(msg, url)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('IND-phone:client:UpdateAdverts', -1, Adverts, "@" .. Player.PlayerData.charinfo.firstname .. "" .. Player.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('IND-phone:server:DeleteAdvert', function()
    local Player = INDCore.Functions.GetPlayer(source)
    local citizenid = Player.PlayerData.citizenid
    Adverts[citizenid] = nil
    TriggerClientEvent('IND-phone:client:UpdateAdvertsDel', -1, Adverts)
end)

RegisterNetEvent('IND-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterNetEvent('IND-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
...
RegisterNetEvent('IND-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', {Player.PlayerData.citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0})
...
RegisterNetEvent('IND-phone:server:sendNewEventMail', function(citizenid, mailData)
    local Player = INDCore.Functions.GetPlayerByCitizenId(citizenid)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', {citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0})
    else
    ...
    SetTimeout(200, function()
        ...
        TriggerClientEvent('IND-phone:client:UpdateMails', Player.PlayerData.source, mails)
    end)
end)

RegisterNetEvent('IND-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
...
RegisterNetEvent('IND-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for _, v in pairs(INDCore.Functions.GetPlayers()) do
        local Player = INDCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                INDPhone.SetPhoneAlerts(Player.PlayerData.citizenid, "twitter")
                INDPhone.AddMentionedTweet(Player.PlayerData.citizenid, TweetMessage)
                TriggerClientEvent('IND-phone:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]["twitter"])
            else
                ...
                if result[1] ~= nil then
                    ...
                    INDPhone.SetPhoneAlerts(MentionedTarget, "twitter")
                    INDPhone.AddMentionedTweet(MentionedTarget, TweetMessage)
                end
            end
        end
    end
...
RegisterNetEvent('IND-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = INDCore.Functions.GetPlayer(src)
    local Target = INDCore.Functions.GetPlayerByPhone(TargetData.number)
    if Target ~= nil then
        TriggerClientEvent('IND-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

RegisterNetEvent('IND-phone:server:BillingEmail', function(data, paid)
    for _, v in pairs(INDCore.Functions.GetPlayers()) do
        local target = INDCore.Functions.GetPlayer(v)
        if target.PlayerData.job.name == data.society then
            if paid then
                local name = '' .. INDCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. INDCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('IND-phone:client:BillingEmail', target.PlayerData.source, data, true, name)
            else
                local name = '' .. INDCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. INDCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('IND-phone:client:BillingEmail', target.PlayerData.source, data, false, name)
            end
        end
    end
end)

RegisterNetEvent('IND-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('IND-phone:client:UpdateHashtags', -1, Handle, messageData)
end)

RegisterNetEvent('IND-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = INDCore.Functions.GetPlayer(src).citizenid
    INDPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterNetEvent('IND-phone:server:DeleteTweet', function(tweetId)
    local Player = INDCore.Functions.GetPlayer(source)
    local delete = false
    local TID = tweetId
    ...
    if delete then
        ...
        TriggerClientEvent('IND-phone:client:UpdateTweets', -1, TWData, nil, true)
    end
end)

RegisterNetEvent('IND-phone:server:UpdateTweets', function(NewTweets, TweetData)
    local src = source
    if Config.Linux then
        ...
        TriggerClientEvent('IND-phone:client:UpdateTweets', -1, src, NewTweets, TweetData, false)
    else
        MySQL.insert('INSERT INTO phone_tweets (citizenid, firstName, lastName, message, date, url, picture, tweetid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
            TweetData.citizenid,
            TweetData.firstName,
...
RegisterNetEvent('IND-phone:server:TransferMoney', function(iban, amount)
    local src = source
    local sender = INDCore.Functions.GetPlayer(src)
    ...
    if result[1] ~= nil then
        ...
        if reciever ~= nil then
            ...
            if PhoneItem ~= nil then
                TriggerClientEvent('IND-phone:client:TransferMoney', reciever.PlayerData.source, amount,
        ...
        TriggerClientEvent('INDCore:Notify', src, "This account number doesn't exist!", "error")
    end
end)

RegisterNetEvent('IND-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, _)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    MySQL.update(
...
RegisterNetEvent('IND-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
...
RegisterNetEvent('IND-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
...
RegisterNetEvent('IND-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, _)
    local src = source
    local SenderData = INDCore.Functions.GetPlayer(src)
    local query = '%' .. ChatNumber .. '%'
    local Player = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', {query})
    if Player[1] ~= nil then
        local TargetData = INDCore.Functions.GetPlayerByCitizenId(Player[1].citizenid)
        if TargetData ~= nil then
            ...
            if Chat[1] ~= nil then
                ...
                TriggerClientEvent('IND-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
            else
                -- Insert for target
                ...
                -- Send notification & Update messages for target
                TriggerClientEvent('IND-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
            end
        else
            local Chat = MySQL.query.await('SELECT * FROM phone_messages WHERE citizenid = ? AND number = ?', {SenderData.PlayerData.citizenid, ChatNumber})
            if Chat[1] ~= nil then
...
RegisterNetEvent('IND-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = INDCore.Functions.GetPlayer(src)
    local Hour = os.date("%H")
    local Minute = os.date("%M")
    local label = Hour .. ":" .. Minute
    TriggerClientEvent('IND-phone:client:AddRecentCall', src, data, label, type)
    local Trgt = INDCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('IND-phone:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterNetEvent('IND-phone:server:CancelCall', function(ContactData)
    local Ply = INDCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('IND-phone:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('IND-phone:server:AnswerCall', function(CallData)
    local Ply = INDCore.Functions.GetPlayerByPhone(CallData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('IND-phone:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('IND-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
...
RegisterNetEvent('IND-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('IND-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterNetEvent('IND-phone:server:AddTransaction', function(data)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
...
RegisterNetEvent('IND-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    ...
    -- TriggerClientEvent('IND-phone:RefreshPhone', src)
end)

RegisterNetEvent('IND-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
...
RegisterNetEvent('IND-phone:server:addImageToGallery', function(image)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    MySQL.insert('INSERT INTO phone_gallery (`citizenid`, `image`) VALUES (?, ?)',{Player.PlayerData.citizenid,image})
end)

RegisterNetEvent('IND-phone:server:getImageFromGallery', function()
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    local images = MySQL.query.await('SELECT * FROM phone_gallery WHERE citizenid = ? ORDER BY `date` DESC',{Player.PlayerData.citizenid})
    TriggerClientEvent('IND-phone:refreshImages', src, images)
end)

RegisterNetEvent('IND-phone:server:RemoveImageFromGallery', function(data)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
...
RegisterNetEvent('IND-phone:server:sendPing', function(data)
    local src = source
    if src == data then
        TriggerClientEvent("INDCore:Notify", src, "You cannot ping yourself", "error")
    end
end)

-- Command

INDCore.Commands.Add("setmetadata", "Set Player Metadata (God Only)", {}, false, function(source, args)
    local Player = INDCore.Functions.GetPlayer(source)
    if args[1] then
...
INDCore.Commands.Add('bill', 'Bill A Player', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Fine Amount'}}, false, function(source, args)
    local biller = INDCore.Functions.GetPlayer(source)
    local billed = INDCore.Functions.GetPlayer(tonumber(args[1]))
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('IND-phone:RefreshPhone', billed.PlayerData.source)
                    TriggerClientEvent('INDCore:Notify', source, 'Invoice Successfully Sent', 'success')
                    TriggerClientEvent('INDCore:Notify', billed.PlayerData.source, 'New Invoice Received')
                else
                    TriggerClientEvent('INDCore:Notify', source, 'Must Be A Valid Amount Above 0', 'error')
                end
            else
                TriggerClientEvent('INDCore:Notify', source, 'You Cannot Bill Yourself', 'error')
            end
        else
            TriggerClientEvent('INDCore:Notify', source, 'Player Not Online', 'error')
        end
    else
        TriggerClientEvent('INDCore:Notify', source, 'No Access', 'error')
    end
end)
...
RegisterNetEvent(Config.pings ..'server:acceptping', function()
    ...
    if Pings[src] ~= nil then
        TriggerClientEvent(Config.pings ..'client:AcceptPing', src, Pings[src], INDCore.Functions.GetPlayer(Pings[src].sender))
        TriggerClientEvent('INDCore:Notify', Pings[src].sender, Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname.." accepted your ping request!")
        Pings[src] = nil
    else
        TriggerClientEvent('INDCore:Notify', src, "You have no ping...", "error")
    end
end)
...
RegisterNetEvent(Config.pings ..'server:denyping', function()
    ...
    local Player = INDCore.Functions.GetPlayer(src)
    if Pings[src] ~= nil then
        TriggerClientEvent('INDCore:Notify', Pings[src].sender, "Your ping request has been rejected...", "error")
        TriggerClientEvent('INDCore:Notify', src, "You turned down the ping...", "success")
        Pings[src] = nil
    else
        TriggerClientEvent('INDCore:Notify', src, "You have no ping...", "error")
    end
...
RegisterNetEvent(Config.pings ..'server:SendPing', function(id)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    local Target = INDCore.Functions.GetPlayer(id)
    ...
    if Target ~= nil then
        ...
        if OtherItem ~= nil then
            TriggerClientEvent('INDCore:Notify', src, "You have requested the location of "..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname)
            Pings[id] = {
                coords = coords,
                sender = src,
            }
            TriggerClientEvent('INDCore:Notify', id, "You have received a ping request from "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname..". Use the app to allow or reject!")
            TriggerClientEvent('IND-phone:ping:client:UiUppers', id, true)
        else
            TriggerClientEvent('INDCore:Notify', src, "Could not send ping...", "error")
        end
    else
        TriggerClientEvent('INDCore:Notify', src, "This person is not in the city...", "error")
    end
end)
...
RegisterNetEvent('IND-phone:server:SendBillForPlayer_bill', function(data)
    local src = source
    local biller = INDCore.Functions.GetPlayer(src)
    local billed = INDCore.Functions.GetPlayer(tonumber(data.ID))
    ...
    if billed then
            if amount and amount > 0 then
                ...
                TriggerClientEvent('INDCore:Notify', src, 'bill successfully sent!', 'success')
                TriggerClientEvent('INDCore:Notify', billed.PlayerData.source, 'New bill Received')
                Wait(1)
                TriggerClientEvent('IND-phone:RefreshPhoneForbill', billed.PlayerData.source)
            else
                TriggerClientEvent('INDCore:Notify', src, 'Must be a valid amount above 0', 'error')
            end
    else
        TriggerClientEvent('INDCore:Notify', src, 'Player not Online', 'error')
    end
end)

INDCore.Functions.CreateCallback('IND-phone:server:GetHasBills_bill', function(source, cb)
    local src = source
    local Ply = INDCore.Functions.GetPlayer(src)
...
RegisterNetEvent('IND-phone:server:debit_AcceptBillForPay', function(data)
    local src = source
    local Ply = INDCore.Functions.GetPlayer(src)
    local OtherPly = INDCore.Functions.GetPlayerByCitizenId(data.CSN)
    ...
    if OtherPly then
        if Ply.PlayerData.money.bank then
            ...
            Wait(1)
            TriggerClientEvent('IND-phone:RefreshPhoneForbill', OtherPly.PlayerData.source)
        else
            TriggerClientEvent('INDCore:Notify', src, 'You don\'t have enough money...', 'error')
        end
    else
        TriggerClientEvent('INDCore:Notify', src, 'Player not Online', 'error')
    end
