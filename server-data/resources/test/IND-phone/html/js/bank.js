...
$(document).on('click', '.bank-app-account', function(e){
    ...
    IND.Phone.Notifications.Add("fas fa-university", "SBI BANK", "Account number. copied!", "#badc58", 1750);
});
...
IND.Phone.Functions.DoBankOpen = function() {
    IND.Phone.Data.PlayerData.money.bank = (IND.Phone.Data.PlayerData.money.bank).toFixed();
    $(".bank-app-account-number").val(IND.Phone.Data.PlayerData.charinfo.account);
    $(".bank-app-account-balance").html("&#36; "+IND.Phone.Data.PlayerData.money.bank);
    $(".bank-app-account-balance").data('balance', IND.Phone.Data.PlayerData.money.bank);
...
$(document).on('click', '.bank-app-account-actions', function(e){
    IND.Phone.Animations.TopSlideDown(".bank-app-transfer", 400, 0);
});

$(document).on('click', '#cancel-transfer', function(e){
    e.preventDefault();

    IND.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
});
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
            $.post('https://ef-phone/CanTransferMoney', JSON.stringify({
                sendTo: iban,
                amountOf: amount,
            }), function(data){
                if (data.TransferedMoney) {
                    ...
                    IND.Phone.Notifications.Add("fas fa-university", "SBI BANK", "You have transfered &#36; "+amount+"!", "#badc58", 1500);
                } else {
                    IND.Phone.Notifications.Add("fas fa-university", "SBI BANK", "You don't have enough balance!", "#badc58", 1500);
                }
                IND.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        IND.Phone.Notifications.Add("fas fa-university", "SBI BANK", "Fill out all fields!", "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://ef-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                IND.Phone.Notifications.Add("fas fa-university", "SBI BANK", "You have paid &#36;"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('https://ef-phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        society: InvoiceData.society,
        invoiceId: InvoiceData.id,
...
IND.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
...
IND.Phone.Functions.LoadContactsWithNumber = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            var ContactColor = IND.Phone.ContactColors[RandomNumber];
...
$(document).on('click', '.bank-app-my-contacts-list-back', function(e){
    ...
    IND.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});

$(document).on('click', '.bank-transfer-mycontacts-icon', function(e){
    e.preventDefault();

    IND.Phone.Animations.TopSlideDown(".bank-app-my-contacts", 400, 0);
});
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        IND.Phone.Notifications.Add("fas fa-university", "SBI BANK", "There is no bank account attached to this number!", "#badc58", 2500);
    }
    IND.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});