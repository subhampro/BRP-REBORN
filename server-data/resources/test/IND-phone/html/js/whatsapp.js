...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    IND.Phone.Functions.SetupChatMessages(ChatData);
...
IND.Phone.Functions.GetLastMessage = function(messages) {
...
IND.Phone.Functions.LoadWhatsappChats = function(chats) {
    $(".whatsapp-chats").html("");
    $.each(chats, function(i, chat){
        ...
        var LastMessage = IND.Phone.Functions.GetLastMessage(chat.messages);
...
IND.Phone.Functions.ReloadWhatsappAlerts = function(chats) {
    $.each(chats, function(i, chat){
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread);
            $(".unread-chat-id-"+i).css({"display":"block"});
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        ...
        IND.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "You can't send a empty message!", "#25D366", 1750);
    }
});
...
IND.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
    if (cData) {
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                var Sender = "me";
                if (message.sender !== IND.Phone.Data.PlayerData.citizenid) { Sender = "other"; }
                var MessageElement
