...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    IND.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});
...
IND.Phone.Notifications.LoadTweets = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var TwtMessage = IND.Phone.Functions.FormatTwitterMessage(clean);
            ...
            // if (Tweet.citizenid === IND.Phone.Data.PlayerData.citizenid){
...
IND.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    $('#tweet-new-url').val("");
    if (Tweets.length > 0) {
...
IND.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('https://ef-phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            IND.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    IND.Phone.Notifications.Add("fab fa-twitter", "Twitter", $(elem).data('mentiontag')+ " copied!", "rgb(27, 149, 224)", 1250);
    document.execCommand("copy");
    $temp.remove();
}

IND.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
...
IND.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    IND.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
