...
---
		if dist <= 20.0 then
		DrawMarker(27, startX, startY, startZ-0.97, 0, 0, 0, 0, 0, 0, 0.90, 0.90, 0.90, 255, 102, 0, 200, 0, 0, 0, 0)
		else
		Citizen.Wait(1500)
		end
		
		if dist <= 2.5 then
		DrawText3D(startX, startY, startZ, "~r~[E]~w~ To Catch chickens")
		end
--
		if dist <= 0.5 then
			if IsControlJustPressed(0, Keys['E']) then -- "E"
			TriggerServerEvent("IND-chickenjob:startChicken")
			LapChicken()
			end			
		end
	end
...
Citizen.CreateThread(function()
	...
	Citizen.Wait(0)
		...
		if dist <= 25.0 then
		DrawMarker(27, portionX, portionY, portionZ-0.97, 0, 0, 0, 0, 0, 0, 0.90, 0.90, 0.90, 255, 102, 0, 200, 0, 0, 0, 0)
		DrawMarker(27, portionX2, portionY2, portionZ2-0.97, 0, 0, 0, 0, 0, 0, 0.90, 0.90, 0.90, 255, 102, 0, 200, 0, 0, 0, 0)
		DrawMarker(27, packingX, packingY, packingZ-0.97, 0, 0, 0, 0, 0, 0, 0.90, 0.90, 0.90, 255, 102, 0, 200, 0, 0, 0, 0)
		DrawMarker(27, packingX2, packingY2, packingZ2-0.97, 0, 0, 0, 0, 0, 0, 0.90, 0.90, 0.90, 255, 102, 0, 200, 0, 0, 0, 0)
		else
		Citizen.Wait(1500)
		end
		
		if dist <= 2.5 then
		DrawText3D(portionX, portionY, portionZ, "~r~[E]~w~ To portion the chicken")
		end
		...
		if dist2 <= 2.5 then
		DrawText3D(portionX2, portionY2, portionZ2, "~r~[E]~w~ To portion the chicken")
		end
		...
		DrawText3D(packingX, packingY, packingZ, "~r~[E]~w~ To pack chicken")
		elseif distP <= 2.5 and packs == 1 then
		DrawText3D(packingX, packingY, packingZ, "~r~[G]~w~ To stop packing")
		DrawText3D(packingX, packingY, packingZ+0.1, "~r~[E]~w~ To keep packing")
		end
		...
		DrawText3D(packingX2, packingY2, packingZ2, "~r~[E]~w~ To pack chicken")
		elseif distP2 <= 2.5 and packs == 1 then
		DrawText3D(packingX2, packingY2, packingZ2, "~r~[G]~w~ To stop packing")
		DrawText3D(packingX2, packingY2, packingZ2+0.1, "~r~[E]~w~ To keep packing")
		end
...
Citizen.CreateThread(function()
	...
	if dist <= 20.0 then
	--DrawMarker(0, sellX, sellY, sellZ-0.96, 0, 0, 0, 0, 0, 0, 2.20, 2.20, 2.20, 255, 102, 0, 200, 0, 0, 0, 0)
	else
	Citizen.Wait(1000)
	end
...
if share == true then
	...
	andsplashed = 0
	share = false
	INDCore.Functions.Notify("Take Alived Chiken To Process Area ..", "primary")
	TriggerServerEvent("IND-chickenjob:getNewChicken")
	end
...
function hewassplashed()
	...
	if chanceofsplashes <= 60 then
			INDCore.Functions.Notify("You managed to catch 1 chicken!", "success")
			if Caught1 == 1 then
		...
		INDCore.Functions.Notify("The chicken escaped your arms!", "error")
	end
end
-------------------DEVELOPED BY MrRobot-----------------------------------------

function packingg(stanowisko)
	local inventory =  INDCore.Functions.GetPlayerData()
	
	INDCore.Functions.TriggerCallback('INDCore:HasItem', function(result)
		if result then
			...
			INDCore.Functions.Progressbar("wash-", "Packing..", 15000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			}, {}, {}, {}, function()
			TriggerServerEvent("IND-chickenjob:getpackedChicken",2)
			INDCore.Functions.Notify("Keep packing the chicken or go to the vehicle and store it.", "primary")
		...
		INDCore.Functions.Notify("You have nothing to pack!", "error")
		end
...
function packed(stanowisko)
	...
	LoadDict('anim@heists@box_carry@')
		...
		INDCore.Functions.Notify("You stopped packing!", "error")
...
function portioning(position)
	...
	INDCore.Functions.TriggerCallback('INDCore:HasItem', function(result)
		if result then
			...
			INDCore.Functions.Progressbar("Cut-", "Slaughtering..", 20000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			...
			INDCore.Functions.Notify("Now Pack slaughtered chicken!", "primary")
			TriggerServerEvent("IND-chickenjob:getcutChicken", 1)
			FreezeEntityPosition(GetPlayerPed(-1),false)
			...
			INDCore.Functions.Notify("You dont have any chickens!", "error")
		end
	end, 'alivechicken')
end
...
function packedsell()
	...
	INDCore.Functions.TriggerCallback('INDCore:HasItem', function(result)
		if result then
			...
			INDCore.Functions.Progressbar("Cut-", "Selling..", 15000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			}, {}, {}, {}, function()
				TriggerServerEvent("IND-chickenjob:sell",3)
				ClearPedTasks(GetPlayerPed(-1))
				DeleteEntity(prop)
			end, function() -- Cancel
			...
			INDCore.Functions.Notify("You have nothing to sell!", "error")
		end
