...
CreateThread(function()
    while true do
        Wait(3600000)
        dailyWithdraws = {}
        TriggerClientEvent('INDCore:Notify', -1, "Daily Withdraw Limit Reset", "success")
    end
end)
...
RegisterCommand('atm', function(source)
    ...
    if visas ~= nil and masters ~= nil then
        for _, v in pairs(visas) do
            ...
            local xCH = INDCore.Functions.GetPlayerByCitizenId(cardHolder)
            if xCH ~= nil then
    ...
    TriggerClientEvent('IND-atms:client:loadATM', src, cards)
end)
end
-- Event

RegisterNetEvent('IND-atms:server:enteratm',function ()
    local src = source
    local xPlayer = INDCore.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('mastercard')
...
RegisterNetEvent('IND-atms:server:doAccountWithdraw', function(data)
    if data ~= nil then
        local src = source
        local xPlayer = INDCore.Functions.GetPlayer(src)
        ...
        if tonumber(data.amount) <= 0 then 
            return TriggerClientEvent('INDCore:Notify', src, "Amount should be greater than 0", "error")
        end   
        ...
        if dailyWith < Config.DailyLimit then
            ...
            if xCH ~= nil then
                ...
                if bankCount > 0 then
                    ...
                    TriggerClientEvent('INDCore:Notify', src, "Withdraw $" .. data.amount .. ' from credit card. Daily Withdraws: ' .. dailyWithdraws[cardHolder], "success")
                else
                    TriggerClientEvent('INDCore:Notify', src, "Not Enough Money", "error")
                end
            ...
            TriggerClientEvent('IND-atms:client:updateBankInformation', src, banking)
        else
            TriggerClientEvent('INDCore:Notify', src, "You have reached the daily limit", "error")
        end
    end
end)
...
INDCore.Functions.CreateCallback('IND-debitcard:server:requestCards', function(source, cb)
    local src = source
    local xPlayer = INDCore.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('mastercard')
...
INDCore.Functions.CreateCallback('IND-debitcard:server:deleteCard', function(source, cb, data)
...
INDCore.Functions.CreateCallback('IND-atms:server:loadBankAccount', function(source, cb, cid, cardnumber)
    local src = source
    local xPlayer = INDCore.Functions.GetPlayer(src)
