...
local function IsWhitelisted(CitizenId)
    ...
    local Perms = INDCore.Functions.GetPermission(Player.PlayerData.source)
...
RegisterNetEvent('IND-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('IND-phone:client:RaceNotify', src, 'You have won the WR from ' .. RaceData.RaceName ..
    ...
    TriggerClientEvent('IND-lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, Player)
    ...
    TriggerClientEvent('IND-phone:client:UpdateLapraces', -1)
end)

RegisterNetEvent('IND-lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('IND-lapraces:client:StartRaceEditor', source, RaceName)
        else
            TriggerClientEvent('INDCore:Notify', source, 'There is already a race with this name.', 'error')
        end
    else
        TriggerClientEvent('INDCore:Notify', source, 'You have not been authorized to race\'s to create.', 'error')
    end
end)

RegisterNetEvent('IND-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('INDCore:Notify', src, 'You were the only one in the race, the race had ended', 'error')
            TriggerClientEvent('IND-lapraces:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('IND-lapraces:client:JoinRace', src, Races[RaceId], AvailableRaces[AvailableKey].Laps)
    TriggerClientEvent('IND-phone:client:UpdateLapraces', -1)
    local creatorsource = INDCore.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData
                              .source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('IND-phone:client:RaceNotify', creatorsource,
            string.sub(Player.PlayerData.charinfo.firstname, 1, 1) .. '. ' .. Player.PlayerData.charinfo.lastname ..
                ' the race has been joined!')
    end
end)

RegisterNetEvent('IND-lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    local RaceName
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('INDCore:Notify', src, 'You were the only one in the race.The race had ended.', 'error')
        TriggerClientEvent('IND-lapraces:client:LeaveRace', src, Races[RaceId])
...
RegisterNetEvent('IND-lapraces:server:SetupRace', function(RaceId, Laps)
            ...
            if not Races[RaceId].Started then
                ...
                SetTimeout(5 * 60 * 1000, function()
                    if Races[RaceId].Waiting then
                        local AvailableKey = GetOpenedRaceKey(RaceId)
                        for cid, _ in pairs(Races[RaceId].Racers) do
                            local RacerData = INDCore.Functions.GetPlayerByCitizenId(cid)
                            if RacerData ~= nil then
                                TriggerClientEvent('IND-lapraces:client:LeaveRace', RacerData.PlayerData.source,
                                    Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
                ...
                TriggerClientEvent('INDCore:Notify', source, 'The race is already running', 'error')
            end
        else
            TriggerClientEvent('INDCore:Notify', source, 'The race is already running', 'error')
        end
    else
        TriggerClientEvent('INDCore:Notify', source, 'This race does not exist :(', 'error')
    end
end)

RegisterNetEvent('IND-lapraces:server:CancelRace', function(raceId)
    local src = source
    local Player = INDCore.Functions.GetPlayer(source)
    local AvailableKey = GetOpenedRaceKey(raceId)

    TriggerClientEvent('INDCore:Notify', src, 'Stopping the race: ' .. raceId, 'error')
    ...
    if AvailableKey ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == Player.PlayerData.citizenid then
            for cid, _ in pairs(Races[raceId].Racers) do
                local RacerData = INDCore.Functions.GetPlayerByCitizenId(cid)
                if RacerData ~= nil then
                    TriggerClientEvent('IND-lapraces:client:LeaveRace', RacerData.PlayerData.source, Races[raceId])
                end
            end
        ...
        TriggerClientEvent('INDCore:Notify', src, 'Race not open: ' .. raceId, 'error')
    end
end)

RegisterNetEvent('IND-lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterNetEvent('IND-lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('IND-lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterNetEvent('IND-lapraces:server:StartRace', function(RaceId)
    local src = source
    local MyPlayer = INDCore.Functions.GetPlayer(src)
    local AvailableKey = GetOpenedRaceKey(RaceId)
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            for CitizenId, _ in pairs(Races[RaceId].Racers) do
                local Player = INDCore.Functions.GetPlayerByCitizenId(CitizenId)
                if Player ~= nil then
                    TriggerClientEvent('IND-lapraces:client:RaceCountdown', Player.PlayerData.source)
                end
            end
            TriggerClientEvent('IND-phone:client:UpdateLapraces', -1)
        else
            TriggerClientEvent('INDCore:Notify', src, 'You are not the creator of the race..', 'error')
        end
    else
...
RegisterNetEvent('IND-lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
...
INDCore.Functions.CreateCallback('IND-lapraces:server:GetRacingLeaderboards', function(_, cb)
    cb(Races)
end)

INDCore.Functions.CreateCallback('IND-lapraces:server:GetRaces', function(_, cb)
    cb(AvailableRaces)
end)

INDCore.Functions.CreateCallback('IND-lapraces:server:GetListedRaces', function(_, cb)
    cb(Races)
end)

INDCore.Functions.CreateCallback('IND-lapraces:server:GetRacingData', function(_, cb, RaceId)
    cb(Races[RaceId])
end)

INDCore.Functions.CreateCallback('IND-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(INDCore.Functions.GetPlayer(source).PlayerData.citizenid))
end)

INDCore.Functions.CreateCallback('IND-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(IsWhitelisted(INDCore.Functions.GetPlayer(source).PlayerData.citizenid), IsNameAvailable(TrackName))
end)

INDCore.Functions.CreateCallback('IND-lapraces:server:CanRaceSetup', function(_, cb)
    cb(Config.RaceSetupAllowed)
end)

INDCore.Functions.CreateCallback('IND-lapraces:server:GetTrackData', function(_, cb, RaceId)
...
INDCore.Commands.Add("cancelrace", "Cancel going race..", {}, false, function(source, args)
    local Player = INDCore.Functions.GetPlayer(source)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        ...
        if RaceName ~= nil then
            ...
            if Races[RaceId].Started then
                ...
                TriggerClientEvent('INDCore:Notify', source, 'This race has not started yet.', 'error')
            end
        end
    else
        TriggerClientEvent('INDCore:Notify', source, 'You have not been authorized to do this.', 'error')
    end
end)

INDCore.Commands.Add("togglesetup", "Turn on / off racing setup", {}, false, function(source, _)
    local Player = INDCore.Functions.GetPlayer(source)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        Config.RaceSetupAllowed = not Config.RaceSetupAllowed
        if not Config.RaceSetupAllowed then
            TriggerClientEvent('INDCore:Notify', source, 'No more races can be created!', 'error')
        else
            TriggerClientEvent('INDCore:Notify', source, 'Races can be created again!', 'success')
        end
    else
        TriggerClientEvent('INDCore:Notify', source, 'You have not been authorized to do this.', 'error')
    end
