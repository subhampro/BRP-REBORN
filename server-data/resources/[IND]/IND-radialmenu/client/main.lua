...
PlayerData = INDCore.Functions.GetPlayerData() -- Setting this for when you restart the resource in game
    ...
    if Vehicle ~= 0 then
        ...
        if not IsVehicleOnAllWheels(Vehicle) then
            VehicleMenu.items[#VehicleMenu.items+1] = {
                ...
                icon = 'car-burst',
                type = 'client',
                event = 'IND-radialmenu:flipVehicle',
                shouldClose = true
            }
        end
        ...
        if IsPedInAnyVehicle(ped) then
            ...
            for i = 1, AmountOfSeats do
                ...
                VehicleMenu.items[seatIndex].items[newIndex] = {
                    ...
                    icon = 'caret-up',
                    type = 'client',
                    event = 'IND-radialmenu:client:ChangeSeat',
                    shouldClose = false,
                }
            end
        end
...
local function controlToggle(bool)
    for i = 1, #controlsToToggle,1 do
        if bool then
            exports['IND-smallresources']:addDisableControls(controlsToToggle[i])
        else
            exports['IND-smallresources']:removeDisableControls(controlsToToggle[i])
        end
    end
end
...
local function setRadialState(bool, sendMessage, delay)
    ...
    if Config.UseWhilstWalking then
        ...
        if bool then
            TriggerEvent('IND-radialmenu:client:onRadialmenuOpen')
            SetupRadialMenu()
        else
            TriggerEvent('IND-radialmenu:client:onRadialmenuClose')
        end
        SetNuiFocus(bool, bool)
    end
...
RegisterNetEvent('IND-radialmenu:client:noPlayers', function()
    INDCore.Functions.Notify(Lang:t("error.no_people_nearby"), 'error', 2500)
end)

RegisterNetEvent('IND-radialmenu:client:openDoor', function(data)
    ...
    if closestVehicle ~= 0 then
        if closestVehicle ~= GetVehiclePedIsIn(ped) then
            local plate = INDCore.Functions.GetPlate(closestVehicle)
            if GetVehicleDoorAngleRatio(closestVehicle, door) > 0.0 then
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('IND-radialmenu:trunk:server:Door', false, plate, door)
                else
                    SetVehicleDoorShut(closestVehicle, door, false)
                end
            else
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('IND-radialmenu:trunk:server:Door', true, plate, door)
                else
        ...
        INDCore.Functions.Notify(Lang:t("error.no_vehicle_found"), 'error', 2500)
    end
end)

RegisterNetEvent('IND-radialmenu:client:setExtra', function(data)
    local string = data.id
        ...
        if GetPedInVehicleSeat(veh, -1) == ped then
            ...
            if DoesExtraExist(veh, extra) then
                if IsVehicleExtraTurnedOn(veh, extra) then
                    ...
                    INDCore.Functions.Notify(Lang:t("error.extra_deactivated", {extra = extra}), 'error', 2500)
                else
                    SetVehicleExtra(veh, extra, 0)
                    INDCore.Functions.Notify(Lang:t("success.extra_activated", {extra = extra}), 'success', 2500)
                end
            else
                INDCore.Functions.Notify(Lang:t("error.extra_not_present", {extra = extra}), 'error', 2500)
            end
        else
            INDCore.Functions.Notify(Lang:t("error.not_driver"), 'error', 2500)
        end
    end
end)

RegisterNetEvent('IND-radialmenu:trunk:client:Door', function(plate, door, open)
    local veh = GetVehiclePedIsIn(PlayerPedId())
    if veh ~= 0 then
        local pl = INDCore.Functions.GetPlate(veh)
        if pl == plate then
            if open then
...
RegisterNetEvent('IND-radialmenu:client:ChangeSeat', function(data)
    local Veh = GetVehiclePedIsIn(PlayerPedId())
    local IsSeatFree = IsVehicleSeatFree(Veh, data.id)
    local speed = GetEntitySpeed(Veh)
    local HasHarnass = exports['IND-smallresources']:HasHarness()
    if not HasHarnass then
        local kmh = speed * 3.6
        if IsSeatFree then
            if kmh <= 100.0 then
                SetPedIntoVehicle(PlayerPedId(), Veh, data.id)
                INDCore.Functions.Notify(Lang:t("info.switched_seats", {seat = data.title}))
            else
                INDCore.Functions.Notify(Lang:t("error.vehicle_driving_fast"), 'error')
            end
        else
            INDCore.Functions.Notify(Lang:t("error.seat_occupied"), 'error')
        end
    else
        INDCore.Functions.Notify(Lang:t("error.race_harness_on"), 'error')
    end
end)

RegisterNetEvent('IND-radialmenu:flipVehicle', function()
    TriggerEvent('animations:client:EmoteCommandStart', {"mechanic"})
    INDCore.Functions.Progressbar("pick_grape", Lang:t("progress.flipping_car"), Config.Fliptime, false, true, {
        ...
        INDCore.Functions.Notify(Lang:t("task.cancel_task"), "error")
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
    end)
end)
...
RegisterNUICallback('selectItem', function(inData, cb)
    ...
    if data and found then
        ...
        elseif data.type == 'qbcommand' then
            TriggerServerEvent('INDCore:CallCommand', data.event, data)
        end
    end
    cb('ok')
end)
