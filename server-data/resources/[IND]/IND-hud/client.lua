...
local function loadSettings(settings)
    ...
    INDCore.Functions.Notify(Lang:t("notify.hud_settings_loaded"), "success")
    Wait(1000)
...
local function restartHud()
    ...
    INDCore.Functions.Notify(Lang:t("notify.hud_restart"), "error")
    ...
    Wait(2600)
    INDCore.Functions.Notify(Lang:t("notify.hud_start"), "success")
end
...
RegisterNetEvent("hud:client:resetStorage", function()
    ...
    INDCore.Functions.TriggerCallback('hud:server:getMenu', function(menu) loadSettings(menu); SetResourceKvp('hudSettings', json.encode(menu)) end)
end)
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if Menu.isToggleMapShapeChecked == "square" then
        ...
        if Menu.isMapNotifChecked then
            INDCore.Functions.Notify(Lang:t("notify.load_square_map"))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            INDCore.Functions.Notify(Lang:t("notify.loaded_square_map"))
        end
    elseif Menu.isToggleMapShapeChecked == "circle" then
        ...
        if Menu.isMapNotifChecked then
            INDCore.Functions.Notify(Lang:t("notify.load_circle_map"))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            INDCore.Functions.Notify(Lang:t("notify.loaded_circle_map"))
        end
    end
end)
...
RegisterNUICallback('cinematicMode', function(_, cb)
    ...
    if Menu.isCinematicModeChecked then
        ...
        if Menu.isCinematicNotifChecked then
            INDCore.Functions.Notify(Lang:t("notify.cinematic_off"), 'error')
        end
        DisplayRadar(1)
    else
        CinematicShow(true)
        ...
        if Menu.isCinematicNotifChecked then
            INDCore.Functions.Notify(Lang:t("notify.cinematic_on"))
        end
    end
    TriggerEvent("hud:client:playHudChecklistSound")
    saveSettings()
...
RegisterNetEvent('hud:client:UpdateNeeds', function(newHunger, newThirst) -- Triggered in IND-core
    hunger = newHunger
    thirst = newThirst
end)
...
RegisterNetEvent("IND-admin:client:ToggleDevmode", function()
    dev = not dev
end)
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                if exports['LegacyFuel']:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        ...
                        INDCore.Functions.Notify(Lang:t("notify.low_fuel"), "error")
