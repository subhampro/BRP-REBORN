...
local function enterRobberyHouse(house)
    ...
    local data = exports['IND-interior']:CreateHouseRobbery(coords)
...
local function lockpickFinish(success)
    ClearPedTasks(PlayerPedId())
    if success then
        TriggerServerEvent('IND-houserobbery:server:enterHouse', closestHouse)
        INDCore.Functions.Notify(Lang:t("success.worked"), "success", 2500)
    else
        if usingAdvanced then
            if math.random(1, 100) <= Config.ChanceToBreakAdvancedLockPick then
                TriggerServerEvent("IND-houserobbery:server:removeAdvancedLockpick")
            ...
            if math.random(1, 100) <= Config.ChanceToBreakLockPick then
                TriggerServerEvent("IND-houserobbery:server:removeLockpick")
                TriggerEvent('inventory:client:ItemBox', INDCore.Shared.Items["lockpick"], "remove")
            end
        end

        INDCore.Functions.Notify(Lang:t("error.didnt_work"), "error", 2500)
    end
end
...
local function searchCabin(cabin)
    ...
    TriggerServerEvent('IND-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if succeededAttempts + 1 >= neededAttempts then
            ClearPedTasks(PlayerPedId())
            TriggerServerEvent('IND-houserobbery:server:searchFurniture', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('IND-houserobbery:server:SetBusyState', cabin, currentHouse, false)
            succeededAttempts = 0
        ...
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('IND-houserobbery:server:SetBusyState', cabin, currentHouse, false)
        INDCore.Functions.Notify(Lang:t("error.process_cancelled"), "error", 3500)
        succeededAttempts = 0
        FreezeEntityPosition(ped, false)
        SetTimeout(500, function()
            IsLockpicking = false
...
RegisterNetEvent('INDCore:Client:OnPlayerLoaded', function()
    INDCore.Functions.TriggerCallback('IND-houserobbery:server:GetHouseConfig', function(HouseConfig)
        Config.Houses = HouseConfig
    end)
end)

RegisterNetEvent('IND-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('IND-houserobbery:client:enterHouse', function(house)
    enterRobberyHouse(house)
end)

RegisterNetEvent('IND-houserobbery:client:setHouseState', function(house, state)
    Config.Houses[house]["opened"] = state
end)

RegisterNetEvent('IND-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('IND-houserobbery:client:SetBusyState', function(cabin, house, bool)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    if Config.UseClockHours then
        if GetClockHours() < Config.MinimumTime or GetClockHours() > Config.MaximumTime then
            INDCore.Functions.Notify(Lang:t("error.not_allowed_time"), "error", 3500)
            return
        end
    end
        ...
        if CurrentCops >= Config.PoliceOnDutyRequired then
            if not Config.Houses[closestHouse]["opened"] then
                if not usingAdvanced then
                    if Config.RequireScrewdriver and not INDCore.Functions.HasItem("screwdriverset") then
                        INDCore.Functions.Notify(Lang:t("error.missing_something"), "error", 3500)
                        return
                    end
                end
                ...
                INDCore.Functions.Notify(Lang:t("error.door_open"), "error", 3500)
            end
        else
            INDCore.Functions.Notify(Lang:t("error.not_enough_police"), "error", 3500)
        end
    end
end)
...
CreateThread(function()
    Wait(500)
    local requiredItems = {
        [1] = {name = INDCore.Shared.Items["advancedlockpick"]["name"], image = INDCore.Shared.Items["advancedlockpick"]["image"]},
        [2] = {name = INDCore.Shared.Items["screwdriverset"]["name"], image = INDCore.Shared.Items["screwdriverset"]["image"]},
    }
