...
-- This is the handler for the cop count, you can change this to anything you want as this is by default the IND-policejob event
...
function Config.DoorlockAction(doorId, setLocked)
    TriggerServerEvent('IND-doorlock:server:updateState', doorId, setLocked, false, false, true, false, false)
end
...
function Config.OnHackDone(success, bank)
    ...
    TriggerServerEvent('IND-bankrobbery:server:setBankState', bank)
end
...
function Config.OnEvidence(pos, chance)
    if math.random(1, 100) > chance or INDCore.Functions.IsWearingGloves() then return end
