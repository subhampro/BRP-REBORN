...
local function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('IND-hotdogjob:server:UpdateReputation', Quality)
        if Config.MyLevel == 1 then
            INDCore.Functions.Notify(Lang:t("success.made_hotdog", {value = Config.Stock[Quality].Label}), "success")
            ...
            if Luck == LuckyNumber then
                INDCore.Functions.Notify(Lang:t("success.made_luck_hotdog", {value = LuckyAmount, value2 = Config.Stock[Quality].Label}), "success")
        ...
        INDCore.Functions.Notify(Lang:t("error.no_more", {value = Config.Stock[Quality].Label}), "error")
    end
    PreparingFood = false
end
...
local function StartHotdogMinigame()
    PrepareAnim()
    TriggerEvent('IND-keyminigame:show')
    TriggerEvent('IND-keyminigame:start', FinishMinigame)
end
...
local function StartWorking()
    INDCore.Functions.TriggerCallback('IND-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            if Config.UseTarget then
                ...
                exports['IND-target']:AddTargetEntity(StandObject, {
                    options = {
                        {
                            icon = "fas fa-hand",
                            ...
                            type = 'client',
                            event = 'IND-hotdogjob:client:ToggleSell',
                            canInteract = function()
                                return IsWorking
                            end
                        }
                ...
                UpdateUI()
                INDCore.Functions.Notify(Lang:t("success.deposit", {deposit = Config.StandDeposit}), 'success')
            else
                local SpawnCoords = Config.Locations["spawn"].coords
                IsWorking = true
                LoadModel("prop_hotdogstand_01")
            ...
            INDCore.Functions.Notify(Lang:t("error.no_money"), 'error')
        end
    end)
end
...
local function StopWorking()
    if DoesEntityExist(StandObject) then
        INDCore.Functions.TriggerCallback('IND-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                DeleteObject(StandObject)
                ClearPedTasksImmediately(PlayerPedId())
                IsWorking = false
                ...
                INDCore.Functions.Notify(Lang:t("success.deposit_returned", {deposit = Config.StandDeposit}), 'success')
            else
                INDCore.Functions.Notify(Lang:t("error.deposit_notreturned"), 'error')
            end
        end)
    else
        INDCore.Functions.Notify(Lang:t("error.no_stand_found"), 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsUIActive = false
...
local function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
        ...
        if PlayerDist > 10.0 then
            ...
            INDCore.Functions.Notify(Lang:t("error.too_far"), 'error')
            break
        end
        Wait(100)
    end
            ...
            if SellingData.Hotdog ~= nil then
                ...
                if Config.UseTarget then
                    if not zoneMade then
                        zoneMade = true
                        exports['IND-target']:AddEntityZone('sellingDogPed', ped, {
                            name = 'sellingDogPed',
                            debugPoly = false,
                        }, {
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        INDCore.Functions.Notify(Lang:t("success.sold_hotdogs", {value = HotdogsForSale, value2 = (HotdogsForSale * SellingPrice)}), 'success')
                                        TriggerServerEvent('IND-hotdogjob:server:Sell', pedCoords, HotdogsForSale, SellingPrice)
                                        SellingData.HasTarget = false
                                        LoadAnim('mp_common')
                                        ...
                                        SellingData.Hotdog = nil
                                        exports['IND-target']:RemoveZone('sellingDogPed')
                                        zoneMade = false
                                    end,
                                },
                                {
                                    ...
                                    label = 'Decline offer',
                                    action = function(entity)
                                        INDCore.Functions.Notify(Lang:t("error.cust_refused"), 'error')
                                        SellingData.HasTarget = false
                                        FreezeEntityPosition(entity, false)
                                        SetPedKeepTask(entity, false)
                                        SetEntityAsNoLongerNeeded(entity)
                ...
                SellingData.HasTarget = false
                SellingData.Hotdog = nil
                INDCore.Functions.Notify(Lang:t("error.no_dogs"), 'error')
                break
            end
        else
            SellingData.HasTarget = false
...
local function ToggleSell()
    ...
    if StandObject ~= nil then
                ...
                while true do
                    if SellingData.Enabled then
                        ...
                        if not SellingData.HasTarget then
                            ...
                            local closestPed, closestDistance = INDCore.Functions.GetClosestPed(coords, PlayerPeds)
        ...
        INDCore.Functions.Notify(Lang:t("error.no_stand"), 'error')
    end
end
...
RegisterNetEvent('IND-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('IND-hotdogjob:client:ToggleSell', function()
...
RegisterNetEvent('IND-hotdogjob:staff:DeletStand', function()
    local ped = PlayerPedId()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Wait(100)
            DeleteEntity(Object)
            INDCore.Functions.Notify(Lang:t("info.admin_removed", "primary"))
        end
    end
end)
...
CreateThread(function()
    if Config.UseTarget then
        exports['IND-target']:AddBoxZone('hotdog_start', vector3(Config.Locations["take"].coords.x, Config.Locations["take"].coords.y, Config.Locations["take"].coords.z), 1, 1, {
            name = 'hotdog_start',
            debugPoly = false,
        ...
        hotdogStart:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                if PlayerData.job.name == 'hotdog' then
                    if not IsWorking then
                        exports['IND-core']:DrawText(Lang:t("info.start_working"), 'left')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then
                        ...
                        exports['IND-core']:DrawText(Lang:t("info.stop_working"), 'left')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then
