...
local function MenuGarage()
    ...
    for k in pairs(Config.Vehicles) do
        towMenu[#towMenu+1] = {
            ...
            params = {
                event = "IND-tow:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
    ...
    exports['IND-menu']:openMenu(towMenu)
end
...
local function CreateZone(type, number)
    ...
    elseif type == "towspots" then
        event = "IND-tow:client:SpawnNPCVehicle"
    ...
    if Config.UseTarget and type == "main" then
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('IND-tow:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('IND-tow:client:Vehicle')
                elseif type == "towspots" then
                    TriggerEvent('IND-tow:client:SpawnNPCVehicle')
                end
            end
        end)
        if type == "vehicle" then
            ...
            zoneComboV:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    TriggerEvent('IND-tow:client:ShowMarker', true)
                else
                    TriggerEvent('IND-tow:client:ShowMarker', false)
                end
            end)
        elseif type == "towspots" then
...
local function deliverVehicle(vehicle)
    ...
    INDCore.Functions.Notify(Lang:t("mission.delivered_vehicle"), "success")
    INDCore.Functions.Notify(Lang:t("mission.get_new_vehicle"))
...
RegisterNetEvent('IND-tow:client:SpawnVehicle', function()
...
RegisterNetEvent('jobs:client:ToggleNpc', function()
    if INDCore.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then
            INDCore.Functions.Notify(Lang:t("error.finish_work"), "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
RegisterNetEvent('IND-tow:client:TowVehicle', function()
    ...
    if isTowVehicle(vehicle) then
            ...
            if not IsPedInAnyVehicle(PlayerPedId()) then
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        INDCore.Functions.Progressbar("towing_vehicle", Lang:t("mission.towing_vehicle"), 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                INDCore.Functions.Notify(Lang:t("mission.goto_depot"), "primary", 5000)
                                ...
                                TriggerServerEvent('IND-tow:server:nano', vehNetID)
                                --remove zone
                                CurrentLocation.zoneCombo:destroy()
                            end
                            INDCore.Functions.Notify(Lang:t("mission.vehicle_towed"), "success")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                            INDCore.Functions.Notify(Lang:t("error.failed"), "error")
                        end)
                    end
                end
            end
        else
            INDCore.Functions.Progressbar("untowing_vehicle", Lang:t("mission.untowing_vehicle"), 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                drawDropOff = false
                INDCore.Functions.Notify(Lang:t("mission.vehicle_takenoff"), "success")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                INDCore.Functions.Notify(Lang:t("error.failed"), "error")
            end)
        ...
        INDCore.Functions.Notify(Lang:t("error.not_towing_vehicle"), "error")
    end
end)

RegisterNetEvent('IND-tow:client:TakeOutVehicle', function(data)
    local coords = Config.Locations["vehicle"].coords
    ...
    if #(pos - coords) <= 5 then
        ...
        TriggerServerEvent('IND-tow:server:DoBail', true, vehicleInfo)
        selectedVeh = vehicleInfo
    else
        INDCore.Functions.Notify(Lang:t("error.too_far_away"), 'error')
    end
end)

RegisterNetEvent('IND-tow:client:Vehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
    if not CurrentTow then
        if vehicle and isTowVehicle(vehicle) then
            DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            TriggerServerEvent('IND-tow:server:DoBail', false)
        else
            MenuGarage()
        end
    else
...
RegisterNetEvent('IND-tow:client:PaySlip', function()
    if JobsDone > 0 then
        RemoveBlip(CurrentBlip)
        TriggerServerEvent("IND-tow:server:11101110", JobsDone)
        JobsDone = 0
        NpcOn = false
    else
...
RegisterNetEvent('IND-tow:client:SpawnNPCVehicle', function()
...
RegisterNetEvent('IND-tow:client:ShowMarker', function(active)
    if PlayerJob.name == "tow" then
        showMarker = active
    end
end)
