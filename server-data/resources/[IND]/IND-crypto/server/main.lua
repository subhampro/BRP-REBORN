...
local function RefreshCrypto()
    ...
    if result ~= nil and result[1] ~= nil then
        ...
        if result[1].history ~= nil then
            ...
            TriggerClientEvent('IND-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, json.decode(result[1].history))
        else
            TriggerClientEvent('IND-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, nil)
        end
    end
end
...
INDCore.Commands.Add("setcryptoworth", "Set crypto value", {{name="crypto", help="Name of the crypto currency"}, {name="Value", help="New value of the crypto currency"}}, false, function(source, args)
    local src = source
    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                TriggerClientEvent('INDCore:Notify', src, "You have changed the value of "..Crypto.Labels[crypto].." from: $"..Crypto.Worth[crypto].." to: $"..NewWorth.." ("..ChangeLabel.." "..PercentageChange.."%)")
                Crypto.Worth[crypto] = NewWorth
                TriggerClientEvent('IND-crypto:client:UpdateCryptoWorth', -1, crypto, NewWorth)
                ...
                TriggerClientEvent('INDCore:Notify', src, Lang:t('text.you_have_not_given_a_new_value', {crypto = Crypto.Worth[crypto]}))
            end
        else
            TriggerClientEvent('INDCore:Notify', src, Lang:t('text.this_crypto_does_not_exist'))
        end
    else
        TriggerClientEvent('INDCore:Notify', src, Lang:t('text.you_have_not_provided_crypto_available_qbit'))
    end
end, "admin")

INDCore.Commands.Add("checkcryptoworth", "", {}, false, function(source)
    local src = source
    TriggerClientEvent('INDCore:Notify', src, Lang:t('text.the_qbit_has_a_value_of', {crypto = Crypto.Worth["qbit"]}))
end)

INDCore.Commands.Add("crypto", "", {}, false, function(source)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    local MyPocket = math.ceil(Player.PlayerData.money.crypto * Crypto.Worth["qbit"])

    TriggerClientEvent('INDCore:Notify', src, Lang:t('text.you_have_with_a_value_of',{playerPlayerDataMoneyCrypto = Player.PlayerData.money.crypto,mypocket = MyPocket}))
end)

-- Events

RegisterServerEvent('IND-crypto:server:FetchWorth', function()
    for name,_ in pairs(Crypto.Worth) do
        ...
        if result[1] ~= nil then
            ...
            if result[1].history ~= nil then
                ...
                TriggerClientEvent('IND-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, json.decode(result[1].history))
            else
                TriggerClientEvent('IND-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, nil)
            end
        end
    end
end)

RegisterServerEvent('IND-crypto:server:ExchangeFail', function()
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, INDCore.Shared.Items["cryptostick"], "remove")
        TriggerClientEvent('INDCore:Notify', src, Lang:t('error.cryptostick_malfunctioned'), 'error')
    end
end)

RegisterServerEvent('IND-crypto:server:Rebooting', function(state, percentage)
    Crypto.Exchange.RebootInfo.state = state
    Crypto.Exchange.RebootInfo.percentage = percentage
end)

RegisterServerEvent('IND-crypto:server:GetRebootState', function()
    local src = source
    TriggerClientEvent('IND-crypto:client:GetRebootState', src, Crypto.Exchange.RebootInfo)
end)

RegisterServerEvent('IND-crypto:server:SyncReboot', function()
    TriggerClientEvent('IND-crypto:client:SyncReboot', -1)
end)

RegisterServerEvent('IND-crypto:server:ExchangeSuccess', function(LuckChance)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    local ItemData = Player.Functions.GetItemByName("cryptostick")
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('INDCore:Notify', src, Lang:t('success.you_have_exchanged_your_cryptostick_for',{amount = Amount}), "success", 3500)
        TriggerClientEvent('inventory:client:ItemBox', src, INDCore.Shared.Items["cryptostick"], "remove")
        TriggerClientEvent('IND-phone:client:AddTransaction', src, Player, {},  Lang:t('credit.there_are_amount_credited',{amount = Amount}), "Credit")
    end
end)

-- Callbacks

INDCore.Functions.CreateCallback('IND-crypto:server:HasSticky', function(source, cb)
...
INDCore.Functions.CreateCallback('IND-crypto:server:GetCryptoData', function(source, cb, name)
    local Player = INDCore.Functions.GetPlayer(source)
    local CryptoData = {
...
INDCore.Functions.CreateCallback('IND-crypto:server:BuyCrypto', function(source, cb, data)
    local Player = INDCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.bank >= total_price then
        ...
        TriggerClientEvent('IND-phone:client:AddTransaction', source, Player, data,  Lang:t('credit.you_have_qbit_purchased',{dataCoins = tonumber(data.Coins)}), "Credit")
...
INDCore.Functions.CreateCallback('IND-crypto:server:SellCrypto', function(source, cb, data)
    local Player = INDCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        TriggerClientEvent('IND-phone:client:AddTransaction', source, Player, data, Lang:t('debit.you_have_sold',{dataCoins = tonumber(data.Coins)}), "Debit")
...
INDCore.Functions.CreateCallback('IND-crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            Player.Functions.RemoveMoney('crypto', tonumber(data.Coins))
            TriggerClientEvent('IND-phone:client:AddTransaction', source, Player, data, "You have "..tonumber(data.Coins).." Qbit('s) transferred!", "Debit")
            local Target = INDCore.Functions.GetPlayerByCitizenId(result[1].citizenid)

            if Target ~= nil then
                Target.Functions.AddMoney('crypto', tonumber(data.Coins))
                TriggerClientEvent('IND-phone:client:AddTransaction', Target.PlayerData.source, Player, data, "There are "..tonumber(data.Coins).." Qbit('s) credited!", "Credit")
            else
