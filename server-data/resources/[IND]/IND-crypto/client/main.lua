...
local requiredItems = {[1] = {name = INDCore.Shared.Items["cryptostick"]["name"], image = INDCore.Shared.Items["cryptostick"]["image"]}}
...
local function ExchangeSuccess()
	TriggerServerEvent('IND-crypto:server:ExchangeSuccess', math.random(1, 10))
end
...
local function ExchangeFail()
	...
	if RemoveChance == LosingNumber then
		TriggerServerEvent('IND-crypto:server:ExchangeFail')
		TriggerServerEvent('IND-crypto:server:SyncReboot')
	end
end
	...
	CreateThread(function()
		while Crypto.Exchange.RebootInfo.state do
			if (Crypto.Exchange.RebootInfo.percentage + 1) <= 100 then
				...
				TriggerServerEvent('IND-crypto:server:Rebooting', true, Crypto.Exchange.RebootInfo.percentage)
			else
				Crypto.Exchange.RebootInfo.percentage = 0
				Crypto.Exchange.RebootInfo.state = false
				TriggerServerEvent('IND-crypto:server:Rebooting', false, 0)
			end
			Wait(1200)
		end
	end)
				...
				if dist < 1.5 then
					if not Crypto.Exchange.RebootInfo.state then
						...
						if IsControlJustPressed(0, 38) then
							INDCore.Functions.TriggerCallback('IND-crypto:server:HasSticky', function(HasItem)
								if HasItem then
									TriggerEvent("mhacking:show")
									TriggerEvent("mhacking:start", math.random(4, 6), 45, HackingSuccess)
								else
									INDCore.Functions.Notify(Lang:t('error.you_dont_have_a_cryptostick'), 'error')
								end
							end)
						end
					else
...
RegisterNetEvent('IND-crypto:client:SyncReboot', function()
	Crypto.Exchange.RebootInfo.state = true
	SystemCrashCooldown()
end)

RegisterNetEvent('INDCore:Client:OnPlayerLoaded', function()
	TriggerServerEvent('IND-crypto:server:FetchWorth')
	TriggerServerEvent('IND-crypto:server:GetRebootState')
end)

RegisterNetEvent('IND-crypto:client:UpdateCryptoWorth', function(crypto, amount, history)
...
RegisterNetEvent('IND-crypto:client:GetRebootState', function(RebootInfo)
