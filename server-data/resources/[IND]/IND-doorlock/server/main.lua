...
local function isAuthorized(Player, door, usedLockpick)
	...
	if Config.AdminAccess and INDCore.Functions.HasPermission(Player.PlayerData.source, Config.AdminPermission) then
...
INDCore.Functions.CreateCallback('IND-doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

INDCore.Functions.CreateCallback('IND-doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('IND-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('IND-doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		if Config.DoorStates[doorID] == nil then Config.DoorStates[doorID] = locked elseif Config.DoorStates[doorID] ~= locked then Config.DoorStates[doorID] = nil end
		TriggerClientEvent('IND-doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('IND-doorlock:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	if not INDCore.Functions.HasPermission(src, Config.CommandPermission) and not IsPlayerAceAllowed(src, 'command') then
		if Config.Warnings then
			showWarning(Lang:t("general.warn_no_permission_newdoor", {player = GetPlayerName(src), license = INDCore.Functions.GetIdentifier(src, 'license'), source = src}))
		end
		return
	end
	...
	TriggerClientEvent('IND-doorlock:client:newDoorAdded', -1, configData, identifier, src)
end)
...
RegisterNetEvent('IND-doorlock:server:removeLockpick', function(type)
	local Player = INDCore.Functions.GetPlayer(source)
...
INDCore.Commands.Add('newdoor', Lang:t("general.newdoor_command_description"), {}, false, function(source)
	TriggerClientEvent('IND-doorlock:client:addNewDoor', source)
end, Config.CommandPermission)

INDCore.Commands.Add('doordebug', Lang:t("general.doordebug_command_description"), {}, false, function(source)
	TriggerClientEvent('IND-doorlock:client:ToggleDoorDebug', source)
end, Config.CommandPermission)
