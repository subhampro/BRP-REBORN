...
local function lockpickFinish(success)
	if success then
		INDCore.Functions.Notify(Lang:t("success.lockpick_success"), 'success', 2500)
		...
		Wait(1800)
		TriggerServerEvent('IND-doorlock:server:updateState', closestDoor.id, false, false, true, false) -- Broadcast new state of the door to everyone
	else
		INDCore.Functions.Notify(Lang:t("error.lockpick_fail"), 'error', 2500)
		if math.random(1,100) <= 17 then
			if usingAdvanced then
				TriggerServerEvent("IND-doorlock:server:removeLockpick", "advancedlockpick")
				TriggerEvent('inventory:client:ItemBox', INDCore.Shared.Items["advancedlockpick"], "remove")
			else
				TriggerServerEvent("IND-doorlock:server:removeLockpick", "lockpick")
				TriggerEvent('inventory:client:ItemBox', INDCore.Shared.Items["lockpick"], "remove")
			end
		end
	end
end
...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		INDCore.Functions.TriggerCallback('IND-doorlock:server:checkItems', function(result)
			p:resolve(result)
...
function SetupDoors()
	local p = promise.new()
	INDCore.Functions.TriggerCallback('IND-doorlock:server:setupDoors', function(result)
		p:resolve(result)
	end)
...
RegisterNetEvent('IND-doorlock:client:setState', function(serverId, doorID, state, src, enableSounds, enableAnimation)
...
RegisterNetEvent('IND-doorlock:client:addNewDoor', function()
	canContinue = false
	hideNUI()
	...
	if Config.DoorList[identifier] then
		INDCore.Functions.Notify((Lang:t("error.door_identifier_exists")):format(identifier), 'error')
		canContinue = true
		return
	end
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		if not model or model == 0 then INDCore.Functions.Notify(Lang:t("error.door_not_found"), 'error') canContinue = true return end
		result = DoorSystemFindExistingDoor(coords.x, coords.y, coords.z, model)
		if result then INDCore.Functions.Notify(Lang:t("error.door_registered"), 'error') canContinue = true return end
		...
		TriggerServerEvent('IND-doorlock:server:saveNewDoor', doorData, false)
		canContinue = true
	else
		...
		if not model[1] or model[1] == 0 or not model[2] or model[2] == 0 then INDCore.Functions.Notify(Lang:t("error.door_not_found"), 'error') return end
		if entity[1] == entity[2] then INDCore.Functions.Notify(Lang:t("error.same_entity"), 'error') canContinue = true return end
		doorData.doorHash = {}
		for i = 1, 2 do
		...
		doorData.model = model
		doorData.heading = heading
		TriggerServerEvent('IND-doorlock:server:saveNewDoor', doorData, true)
		canContinue = true
	end
end)

RegisterNetEvent('IND-doorlock:client:newDoorAdded', function(data, id, creatorSrc)
	Config.DoorList[id] = data
	TriggerEvent('IND-doorlock:client:setState', creatorSrc, id, data.locked, false, true, true)
end)

RegisterNetEvent('IND-doorlock:client:ToggleDoorDebug', function()
...
RegisterCommand('toggledoorlock', function()
	...
	TriggerServerEvent('IND-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
...
RegisterCommand('remotetriggerdoor', function()
	...
	TriggerServerEvent('IND-doorlock:server:updateState', nearestDoor.id, not nearestDoor.data.locked, NetworkGetNetworkIdFromEntity(playerPed), false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
