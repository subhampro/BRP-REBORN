...
RegisterNetEvent('IND-fitbit:use', function()
    openWatch()
end)
...
RegisterNUICallback('setFoodWarning', function(data, cb)
    ...
    TriggerServerEvent('IND-fitbit:server:setValue', 'food', foodValue)
    INDCore.Functions.Notify(Lang:t('success.hunger_set', {hungervalue = foodValue}), 'success')
    cb('ok')
end)

RegisterNUICallback('setThirstWarning', function(data, cb)
    local thirstValue = tonumber(data.value)
    TriggerServerEvent('IND-fitbit:server:setValue', 'thirst', thirstValue)
    INDCore.Functions.Notify(Lang:t('success.thirst_set', {thirstvalue = thirstValue}), 'success')
    cb('ok')
end)