...
RegisterNetEvent('IND-apartments:server:SetInsideMeta', function(house, insideId, bool, isVisiting)
    local src = source
    ...
    if bool then
        ...
        INDCore.Functions.SetPlayerBucket(src, tonumber(routeId))
    else
        ...
        Player.Functions.SetMetaData("inside", insideMeta)
        INDCore.Functions.SetPlayerBucket(src, 0)
    end
end)

RegisterNetEvent('IND-apartments:returnBucket', function()
    local src = source
...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('INDCore:Notify', src, Lang:t('success.receive_apart').." ("..label..")")
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('INDCore:Notify', src, Lang:t('success.changed_apart'))
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
INDCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(_, cb, apartment)
    local apartments = {}
...
INDCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(_, cb, apartmentId)
    local retval = 0
...
INDCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(_, cb, apartment)
...
INDCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
INDCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = INDCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = MySQL.query.await('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
INDCore.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
    local src = source
    local Player = INDCore.Functions.GetPlayer(src)
