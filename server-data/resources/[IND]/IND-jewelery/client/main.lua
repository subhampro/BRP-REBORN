...
local function smashVitrine(k)
    ...
    INDCore.Functions.TriggerCallback('IND-jewellery:server:getCops', function(cops)
        if cops >= Config.RequiredCops then
            ...
            elseif math.random(1, 100) <= 5 and IsWearingGloves() then
                TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
                INDCore.Functions.Notify(Lang:t('error.fingerprints'), "error")
            end
            smashing = true
            INDCore.Functions.Progressbar("smash_vitrine", Lang:t('info.progressbar'), Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
            }, {}, {}, {}, function() -- Done
                TriggerServerEvent('IND-jewellery:server:vitrineReward', k)
                TriggerServerEvent('IND-jewellery:server:setTimeout')
                TriggerServerEvent('police:server:policeAlert', 'Robbery in progress')
                smashing = false
                TaskPlayAnim(ped, animDict, "exit", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            end, function() -- Cancel
                TriggerServerEvent('IND-jewellery:server:setVitrineState', "isBusy", false, k)
                smashing = false
                TaskPlayAnim(ped, animDict, "exit", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            end)
            TriggerServerEvent('IND-jewellery:server:setVitrineState', "isBusy", true, k)
            ...
            INDCore.Functions.Notify(Lang:t('error.minimum_police', {value = Config.RequiredCops}), 'error')
        end
    end)
end
...
RegisterNetEvent('INDCore:Client:OnPlayerLoaded', function()
	INDCore.Functions.TriggerCallback('IND-jewellery:server:getVitrineState', function(result)
		Config.Locations = result
	end)
end)

RegisterNetEvent('IND-jewellery:client:setVitrineState', function(stateType, state, k)
                ...
                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                    exports['IND-core']:KeyPressed()
                        if validWeapon() then
                            ...
                            INDCore.Functions.Notify(Lang:t('error.wrong_weapon'), 'error')
                        end
                    else
                        exports['IND-core']:DrawText(Lang:t('general.drawtextui_broken'), 'left')
                    end
                end
            Wait(1)
        end
...
CreateThread(function()
    if Config.UseTarget then
        for k, v in pairs(Config.Locations) do
            exports["IND-target"]:AddBoxZone("jewelstore" .. k, v.coords, 1, 1, {
                name = "jewelstore" .. k,
                heading = 40,
                minZ = v.coords.z - 1,
                maxZ = v.coords.z + 1,
        ...
        for k, v in pairs(Config.Locations) do
            ...
            boxZone:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    Listen4Control(k)
                    exports['IND-core']:DrawText(Lang:t('general.drawtextui_grab'), 'left')
                else
                    listen = false
