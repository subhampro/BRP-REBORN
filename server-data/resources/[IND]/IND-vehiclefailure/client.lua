...
local function DamageRandomComponent()
    ...
    exports['IND-mechanicjob']:SetVehicleStatus(INDCore.Functions.GetPlate(vehicle), randomComponent, exports['IND-mechanicjob']:GetVehicleStatus(INDCore.Functions.GetPlate(vehicle), randomComponent) - randomDamage)
end

local function CleanVehicle(veh)
    local ped = PlayerPedId()
    TaskStartScenarioInPlace(ped, "WORLD_HUMAN_MAID_CLEAN", 0, true)
    INDCore.Functions.Progressbar("cleaning_vehicle", Lang:t("progress.clean_veh"), math.random(10000, 20000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        INDCore.Functions.Notify(Lang:t("success.cleaned_veh"))
        SetVehicleDirtLevel(veh, 0.1)
        SetVehicleUndriveable(veh, false)
        WashDecalsFromVehicle(veh, 1.0)
        TriggerServerEvent('IND-vehiclefailure:server:removewashingkit', veh)
        TriggerEvent('inventory:client:ItemBox', INDCore.Shared.Items["cleaningkit"], "remove")
        ClearAllPedProps(ped)
        ClearPedTasks(ped)
    end, function() -- Cancel
        INDCore.Functions.Notify(Lang:t("error.failed_notification"), "error")
        ClearAllPedProps(ped)
        ClearPedTasks(ped)
    end)
end
...
local function RepairVehicleFull(veh)
    ...
    INDCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.repair_veh"), math.random(20000, 30000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        INDCore.Functions.Notify(Lang:t("success.repaired_veh"))
        SetVehicleEngineHealth(veh, 1000.0)
        SetVehicleEngineOn(veh, true, false)
        for i = 0, 5 do
            SetVehicleTyreFixed(veh, i)
            TriggerEvent('IND-vehiclefailure:client:TyreSync', veh, i)
        end
        if (IsBackEngine(GetEntityModel(veh))) then
        ...
        TriggerServerEvent('IND-vehiclefailure:removeItem', "advancedrepairkit")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
        INDCore.Functions.Notify(Lang:t("error.failed_notification"), "error")
        if (IsBackEngine(GetEntityModel(veh))) then
...
local function RepairVehicle(veh)
    ...
    INDCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.repair_veh"), math.random(10000, 20000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerServerEvent('IND-vehiclefailure:removeItem', "repairkit")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
        INDCore.Functions.Notify(Lang:t("error.failed_notification"), "error")
        if (IsBackEngine(GetEntityModel(veh))) then
...
RegisterNetEvent('IND-vehiclefailure:client:RepairVehicle', function()
    local veh = INDCore.Functions.GetClosestVehicle()
    ...
    if veh ~= nil and veh ~= 0 and engineHealth < 500 then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            ...
            if #(pos - vehpos) > 4.9 then
                INDCore.Functions.Notify(Lang:t("error.out_range_veh"), "error")
            else
                INDCore.Functions.Notify(Lang:t("error.inside_veh"), "error")
            end
        end
    else
        if veh == nil or veh == 0 then
            INDCore.Functions.Notify(Lang:t("error.not_near_veh"), "error")
        else
            INDCore.Functions.Notify(Lang:t("error.healthy_veh"), "error")
        end
    end
end)

RegisterNetEvent('IND-vehiclefailure:client:SyncWash', function(veh)
    SetVehicleDirtLevel(veh, 0.1)
    SetVehicleUndriveable(veh, false)
    WashDecalsFromVehicle(veh, 1.0)
end)

RegisterNetEvent('IND-vehiclefailure:client:CleanVehicle', function()
    local veh = INDCore.Functions.GetClosestVehicle()
...
RegisterNetEvent('IND-vehiclefailure:client:RepairVehicleFull', function()
    local veh = INDCore.Functions.GetClosestVehicle()
    if veh ~= nil and veh ~= 0 then
        local ped = PlayerPedId()
        local pos = GetEntityCoords(ped)
...
RegisterNetEvent('IND-vehiclefailure:client:TyreSync', function(veh, tyre)
...
RegisterNetEvent('iens:repaira', function()
    ...
    INDCore.Functions.Notify(Lang:t("error.inside_veh_req"))
end)

RegisterNetEvent('iens:besked', function()
    INDCore.Functions.Notify(Lang:t("error.roadside_avail"))
end)

RegisterNetEvent('iens:notAllowed', function()
    INDCore.Functions.Notify(Lang:t("error.no_permission"))
end)
    ...
    if isPedDrivingAVehicle() then
        ...
        if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
            if GetVehicleOilLevel(vehicle) > 0 then
                ...
                INDCore.Functions.Notify(Lang:t(('fix_message_%s'):format(fixMessagePos)))
                fixMessagePos = fixMessagePos + 1
                if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
            else
                INDCore.Functions.Notify(Lang:t("error.veh_damaged"))
            end
        else
            INDCore.Functions.Notify(Lang:t(('nofix_message_%s'):format(noFixMessagePos)))
                    ...
                    if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
                        ...
                        if speed >= 1.0 then
                            ...
                            if brake > 127 then
                                ...
                                --exports['IND-vehicletuning']:SetVehicleStatus(INDCore.Functions.GetPlate(vehicle), "brakes", exports['IND-vehicletuning']:GetVehicleStatus(INDCore.Functions.GetPlate(vehicle), "brakes") - 0.01)
                            end
