...
local function LoadInventory(source, citizenid)
	...
	for _, item in pairs(inventory) do
		if item then
			local itemInfo = INDCore.Shared.Items[item.name:lower()]
			if itemInfo then
...
local function AddItem(source, item, amount, slot, info)
	...
	local itemInfo = INDCore.Shared.Items[item:lower()]
	if not itemInfo and not Player.Offline then
		INDCore.Functions.Notify(source, "Item does not exist", 'error')
		return false
	end
	...
	if itemInfo['type'] == 'weapon' then
		info.serie = info.serie or tostring(INDCore.Shared.RandomInt(2) .. INDCore.Shared.RandomStr(3) .. INDCore.Shared.RandomInt(1) .. INDCore.Shared.RandomStr(2) .. INDCore.Shared.RandomInt(3) .. INDCore.Shared.RandomStr(4))
	...
	if (totalWeight + (itemInfo['weight'] * amount)) <= Config.MaxInventoryWeight then
		if (slot and Player.PlayerData.items[slot]) and (Player.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
			...
			TriggerEvent('IND-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** got item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[slot].amount)
		...
		elseif itemInfo['unique'] or (not slot or slot == nil) or itemInfo['type'] == 'weapon' then
			for i = 1, Config.MaxInventorySlots, 1 do
				if Player.PlayerData.items[i] == nil then
					...
					TriggerEvent('IND-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** got item: [slot:' .. i .. '], itemname: ' .. Player.PlayerData.items[i].name .. ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[i].amount)
	...
	elseif not Player.Offline then
		INDCore.Functions.Notify(source, "Inventory too full", 'error')
	end
	return false
end
	...
	if slot then
		if Player.PlayerData.items[slot].amount > amount then
			...
			if not Player.Offline then
				TriggerEvent('IND-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[slot].amount)
			end
		...
		elseif Player.PlayerData.items[slot].amount == amount then
			...
			TriggerEvent('IND-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. slot .. '], itemname: ' .. item .. ', removed amount: ' .. amount .. ', item removed')
		...
		for _, _slot in pairs(slots) do
			if Player.PlayerData.items[_slot].amount > amountToRemove then
				...
				if not Player.Offline then
					TriggerEvent('IND-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. Player.PlayerData.items[_slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[_slot].amount)
				end
			...
			elseif Player.PlayerData.items[_slot].amount == amountToRemove then
				...
				TriggerEvent('IND-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. item .. ', removed amount: ' .. amount .. ', item removed')
...
local function ClearInventory(source, filterItems)
	...
	TriggerEvent('IND-log:server:CreateLog', 'playerinventory', 'ClearInventory', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** inventory cleared')
end
...
local function SetInventory(source, items)
	...
	TriggerEvent('IND-log:server:CreateLog', 'playerinventory', 'SetInventory', 'blue', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** items set: ' .. json.encode(items))
end
...
local function CreateUsableItem(itemName, data)
	INDCore.Functions.CreateUseableItem(itemName, data)
end
...
local function GetUsableItem(itemName)
	return INDCore.Functions.CanUseItem(itemName)
end

exports("GetUsableItem", GetUsableItem)

---Use an item from the INDCore.UsableItems table if a callback is present
...
local function AddToStash(stashId, slot, otherslot, itemName, amount, info)
	amount = tonumber(amount) or 1
	local ItemData = INDCore.Shared.Items[itemName]
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
		...
		local itemInfo = INDCore.Shared.Items[itemData.name:lower()]
		...
		TriggerEvent("IND-log:server:CreateLog", "drop", "New Item Drop", "red", "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) dropped new item; name: **"..itemData.name.."**, amount: **" .. itemAmount .. "**")
		...
		INDCore.Functions.Notify(source, Lang:t("notify.missitem"), "error")
	end
end
...
local function OpenInventory(name, id, other, origin)
	...
	if ply.state.inv_busy then
		return INDCore.Functions.Notify(src, Lang:t("notify.noaccess"), 'error')
	end
	if name and id then
		local secondInv = {}
		if name == "stash" then
			if Stashes[id] then
				if Stashes[id].isOpen then
					local Target = INDCore.Functions.GetPlayer(Stashes[id].isOpen)
					if Target then
		...
		elseif name == "trunk" then
			if Trunks[id] then
				if Trunks[id].isOpen then
					local Target = INDCore.Functions.GetPlayer(Trunks[id].isOpen)
					if Target then
			...
			if (Trunks[id] and Trunks[id].isOpen) or (INDCore.Shared.SplitStr(id, "PLZI")[2] and (Player.PlayerData.job.name ~= "police" or Player.PlayerData.job.type ~= "leo")) then
				secondInv.name = "none-inv"
		...
		elseif name == "glovebox" then
			if Gloveboxes[id] then
				if Gloveboxes[id].isOpen then
					local Target = INDCore.Functions.GetPlayer(Gloveboxes[id].isOpen)
					if Target then
		...
		elseif name == "otherplayer" then
			local OtherPlayer = INDCore.Functions.GetPlayer(tonumber(id))
			if OtherPlayer then
			...
			if Drops[id] then
				if Drops[id].isOpen then
					local Target = INDCore.Functions.GetPlayer(Drops[id].isOpen)
					if Target then
		...
		TriggerClientEvent("IND-inventory:client:closeinv", id)
...
AddEventHandler('INDCore:Server:PlayerLoaded', function(Player)
	INDCore.Functions.AddPlayerMethod(Player.PlayerData.source, "AddItem", function(item, amount, slot, info)
		return AddItem(Player.PlayerData.source, item, amount, slot, info)
	end)

	INDCore.Functions.AddPlayerMethod(Player.PlayerData.source, "RemoveItem", function(item, amount, slot)
		return RemoveItem(Player.PlayerData.source, item, amount, slot)
	end)

	INDCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemBySlot", function(slot)
		return GetItemBySlot(Player.PlayerData.source, slot)
	end)

	INDCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemByName", function(item)
		return GetItemByName(Player.PlayerData.source, item)
	end)

	INDCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemsByName", function(item)
		return GetItemsByName(Player.PlayerData.source, item)
	end)

	INDCore.Functions.AddPlayerMethod(Player.PlayerData.source, "ClearInventory", function(filterItems)
		ClearInventory(Player.PlayerData.source, filterItems)
	end)

	INDCore.Functions.AddPlayerMethod(Player.PlayerData.source, "SetInventory", function(items)
...
AddEventHandler('onResourceStart', function(resourceName)
	...
	for k in pairs(Players) do
		INDCore.Functions.AddPlayerMethod(k, "AddItem", function(item, amount, slot, info)
			return AddItem(k, item, amount, slot, info)
		end)

		INDCore.Functions.AddPlayerMethod(k, "RemoveItem", function(item, amount, slot)
			return RemoveItem(k, item, amount, slot)
		end)

		INDCore.Functions.AddPlayerMethod(k, "GetItemBySlot", function(slot)
			return GetItemBySlot(k, slot)
		end)

		INDCore.Functions.AddPlayerMethod(k, "GetItemByName", function(item)
			return GetItemByName(k, item)
		end)

		INDCore.Functions.AddPlayerMethod(k, "GetItemsByName", function(item)
			return GetItemsByName(k, item)
		end)

		INDCore.Functions.AddPlayerMethod(k, "ClearInventory", function(filterItems)
			ClearInventory(k, filterItems)
		end)

		INDCore.Functions.AddPlayerMethod(k, "SetInventory", function(items)
			SetInventory(k, items)
		end)
	end
end)

RegisterNetEvent('INDCore:Server:UpdateObject', function()
...
RegisterNetEvent('inventory:server:combineItem', function(item, fromItem, toItem)
	...
	local recipe = INDCore.Shared.Items[toItem.name].combinable
...
--	print('inventory:server:OpenInventory is deprecated use exports[\'IND-inventory\']:OpenInventory() instead.')
	local src = source
	local ply = Player(src)
	local Player = INDCore.Functions.GetPlayer(src)
	if ply.state.inv_busy then
...
RegisterNetEvent('inventory:server:UseItemSlot', function(slot)
	...
	if not itemData then return end
	local itemInfo = INDCore.Shared.Items[itemData.name]
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if (fromInventory == "player" or fromInventory == "hotbar") and (INDCore.Shared.SplitStr(toInventory, "-")[1] == "itemshop" or toInventory == "crafting") then
		return
	end
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			elseif INDCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				local playerId = tonumber(INDCore.Shared.SplitStr(toInventory, "-")[2])
				...
				if toItemData then
					--Player.PlayerData.items[fromSlot] = toItemData
					local itemInfo = INDCore.Shared.Items[toItemData.name:lower()]
					toAmount = tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						RemoveItem(playerId, itemInfo["name"], toAmount, fromSlot)
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("IND-log:server:CreateLog", "robbing", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** with player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
					end
				else
					local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("IND-log:server:CreateLog", "robbing", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** to player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
				end
				local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
				AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info)
			elseif INDCore.Shared.SplitStr(toInventory, "-")[1] == "trunk" then
				local plate = INDCore.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("IND-log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("IND-log:server:CreateLog", "trunk", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
				AddToTrunk(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif INDCore.Shared.SplitStr(toInventory, "-")[1] == "glovebox" then
				local plate = INDCore.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("IND-log:server:CreateLog", "glovebox", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("IND-log:server:CreateLog", "glovebox", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
				AddToGlovebox(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif INDCore.Shared.SplitStr(toInventory, "-")[1] == "stash" then
				local stashId = INDCore.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("IND-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
					end
				else
					local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("IND-log:server:CreateLog", "stash", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
				end
				local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
				AddToStash(stashId, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif INDCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				-- Traphouse
				local traphouseId = INDCore.Shared.SplitStr(toInventory, "_")[2]
				local toItemData = exports['IND-traphouse']:GetInventoryData(traphouseId, toSlot)
				local IsItemValid = exports['IND-traphouse']:CanItemBeSaled(fromItemData.name:lower())
				if IsItemValid then
					RemoveItem(src, fromItemData.name, fromAmount, fromSlot)
					TriggerClientEvent("inventory:client:CheckWeapon", src, fromItemData.name)
					if toItemData  then
						...
						toAmount = tonumber(toAmount) or toItemData.amount
						if toItemData.name ~= fromItemData.name then
							exports['IND-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount)
							AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
							TriggerEvent("IND-log:server:CreateLog", "traphouse", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
						end
					else
						local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
						TriggerEvent("IND-log:server:CreateLog", "traphouse", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
					end
					local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
					exports['IND-traphouse']:AddHouseItem(traphouseId, toSlot, itemInfo["name"], fromAmount, fromItemData.info, src)
				else
					INDCore.Functions.Notify(src, Lang:t("notify.nosell"), 'error')
				end
			else
				-- drop
				...
				if toInventory == nil or toInventory == 0 then
					...
					if toItemData then
						...
						if toItemData.name ~= fromItemData.name then
							...
							TriggerEvent("IND-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
						end
					else
						local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
						TriggerEvent("IND-log:server:CreateLog", "drop", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
					end
					local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
			...
			INDCore.Functions.Notify(src, Lang:t("notify.missitem"), "error")
		end
	elseif INDCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		local playerId = tonumber(INDCore.Shared.SplitStr(fromInventory, "-")[2])
		local OtherPlayer = INDCore.Functions.GetPlayer(playerId)
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					itemInfo = INDCore.Shared.Items[toItemData.name:lower()]
					toAmount = tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						RemoveItem(src, toItemData.name, toAmount, toSlot)
				...
				itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
				AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info)
			end
		else
			INDCore.Functions.Notify(src, Lang:t("notify.itemexist"), "error")
		end
	elseif INDCore.Shared.SplitStr(fromInventory, "-")[1] == "trunk" then
		local plate = INDCore.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("IND-log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("IND-log:server:CreateLog", "trunk", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("IND-log:server:CreateLog", "trunk", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Trunks[plate].items[toSlot]
	...
	elseif INDCore.Shared.SplitStr(fromInventory, "-")[1] == "glovebox" then
		local plate = INDCore.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("IND-log:server:CreateLog", "glovebox", "Swapped", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src..")* swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("IND-log:server:CreateLog", "glovebox", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("IND-log:server:CreateLog", "glovebox", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Gloveboxes[plate].items[toSlot]
	...
	elseif INDCore.Shared.SplitStr(fromInventory, "-")[1] == "stash" then
		local stashId = INDCore.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("IND-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. stashId .. "*")
					else
						TriggerEvent("IND-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. stashId .. "*")
					end
				else
					TriggerEvent("IND-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. stashId .. "*")
				end
	...
	elseif INDCore.Shared.SplitStr(fromInventory, "-")[1] == "traphouse" then
		local traphouseId = INDCore.Shared.SplitStr(fromInventory, "-")[2]
		local fromItemData = exports['IND-traphouse']:GetInventoryData(traphouseId, fromSlot)
		fromAmount = tonumber(fromAmount) or fromItemData.amount
		if fromItemData and fromItemData.amount >= fromAmount then
			local itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
			if toInventory == "player" or toInventory == "hotbar" then
				local toItemData = GetItemBySlot(src, toSlot)
				exports['IND-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], fromAmount)
				if toItemData then
					itemInfo = INDCore.Shared.Items[toItemData.name:lower()]
					toAmount = tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						RemoveItem(src, toItemData.name, toAmount, toSlot)
						exports['IND-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
						TriggerEvent("IND-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. traphouseId .. "*")
					else
						TriggerEvent("IND-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. traphouseId .. "*")
					end
				else
					TriggerEvent("IND-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. traphouseId .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = exports['IND-traphouse']:GetInventoryData(traphouseId, toSlot)
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						itemInfo = INDCore.Shared.Items[toItemData.name:lower()]
						exports['IND-traphouse']:RemoveHouseItem(traphouseId, toSlot, itemInfo["name"], toAmount)
						exports['IND-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
					end
				end
				itemInfo = INDCore.Shared.Items[fromItemData.name:lower()]
			...
			INDCore.Functions.Notify(src, "Item doesn't exist??", "error")
		end
	elseif INDCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		local shopType = INDCore.Shared.SplitStr(fromInventory, "-")[2]
		local itemData = ShopItems[shopType].items[fromSlot]
		local itemInfo = INDCore.Shared.Items[itemData.name:lower()]
		local bankBalance = Player.PlayerData.money["bank"]
		local price = tonumber((itemData.price*fromAmount))

		if INDCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if INDCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				price = tonumber(itemData.price)
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					itemData.info.serie = tostring(INDCore.Shared.RandomInt(2) .. INDCore.Shared.RandomStr(3) .. INDCore.Shared.RandomInt(1) .. INDCore.Shared.RandomStr(2) .. INDCore.Shared.RandomInt(3) .. INDCore.Shared.RandomStr(4))
					itemData.info.quality = 100
					AddItem(src, itemData.name, 1, toSlot, itemData.info)
					TriggerClientEvent('IND-drugs:client:updateDealerItems', src, itemData, 1)
					INDCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("IND-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
				else
					INDCore.Functions.Notify(src, Lang:t("notify.notencash"), "error")
				end
			else
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
					TriggerClientEvent('IND-drugs:client:updateDealerItems', src, itemData, fromAmount)
					INDCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("IND-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. "  for $"..price)
				else
					INDCore.Functions.Notify(src, "You don't have enough cash..", "error")
				end
			end
		elseif INDCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
				...
				AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
				TriggerClientEvent('IND-shops:client:UpdateShop', src, INDCore.Shared.SplitStr(shopType, "_")[2], itemData, fromAmount)
				INDCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
				TriggerEvent("IND-log:server:CreateLog", "shops", "Shop item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			INDCore.Functions.Notify(src, Lang:t("notify.noitem"), "error")
		end
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("IND-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** - dropid: *" .. fromInventory .. "*")
					else
						TriggerEvent("IND-log:server:CreateLog", "drop", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** - from dropid: *" .. fromInventory .. "*")
					end
				else
					TriggerEvent("IND-log:server:CreateLog", "drop", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** -  dropid: *" .. fromInventory .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				toInventory = tonumber(toInventory)
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
    ...
    local OtherPlayer = INDCore.Functions.GetPlayer(target)
    local dist = #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target)))
	if Player == OtherPlayer then return INDCore.Functions.Notify(src, Lang:t("notify.gsitem")) end
	if dist > 2 then return INDCore.Functions.Notify(src, Lang:t("notify.tftgitem")) end
	local item = GetItemBySlot(src, slot)
	if not item then INDCore.Functions.Notify(src, Lang:t("notify.infound")); return end
	if item.name ~= name then INDCore.Functions.Notify(src, Lang:t("notify.iifound")); return end
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info) then
				TriggerClientEvent('inventory:client:ItemBox',target, INDCore.Shared.Items[item.name], "add")
				INDCore.Functions.Notify(target, Lang:t("notify.gitemrec")..amount..' '..item.label..Lang:t("notify.gitemfrom")..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
				TriggerClientEvent('inventory:client:ItemBox',src, INDCore.Shared.Items[item.name], "remove")
				INDCore.Functions.Notify(src, Lang:t("notify.gitemyg") .. OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.. " " .. amount .. " " .. item.label .."!")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('IND-inventory:client:giveAnim', src)
				TriggerClientEvent('IND-inventory:client:giveAnim', target)
			else
				AddItem(src, item.name, amount, item.slot, item.info)
				INDCore.Functions.Notify(src, Lang:t("notify.gitinvfull"), "error")
				INDCore.Functions.Notify(target, Lang:t("notify.giymif"), "error")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
			end
		else
			INDCore.Functions.Notify(src, Lang:t("notify.gitydhei"), "error")
		end
	else
		INDCore.Functions.Notify(src, Lang:t("notify.gitydhitt"))
	end
end)
...
INDCore.Functions.CreateCallback('IND-inventory:server:GetStashItems', function(_, cb, stashId)
	cb(GetStashItems(stashId))
end)

INDCore.Functions.CreateCallback('inventory:server:GetCurrentDrops', function(_, cb)
	cb(Drops)
end)

INDCore.Functions.CreateCallback('INDCore:HasItem', function(source, cb, items, amount)
	print("^3INDCore:HasItem is deprecated, please use INDCore.Functions.HasItem, it can be used on both server- and client-side and uses the same arguments.^0")
    local retval = false
    local Player = INDCore.Functions.GetPlayer(source)
...
INDCore.Commands.Add("resetinv", "Reset Inventory (Admin Only)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
	...
	if invType and invId then
		...
		elseif invType == "stash" then
			...
			INDCore.Functions.Notify(source,  Lang:t("notify.navt"), "error")
		end
	else
		INDCore.Functions.Notify(source,  Lang:t("notify.anfoc"), "error")
	end
end, "admin")

INDCore.Commands.Add("rob", "Rob Player", {}, false, function(source, _)
	TriggerClientEvent("police:client:RobPlayer", source)
end)

INDCore.Commands.Add("giveitem", "Give An Item (Admin Only)", {{name="id", help="Player ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, false, function(source, args)
	local id = tonumber(args[1])
	local Player = INDCore.Functions.GetPlayer(id)
	local amount = tonumber(args[3]) or 1
	local itemData = INDCore.Shared.Items[tostring(args[2]):lower()]
	if Player then
			if itemData then
				-- check iteminfo
				local info = {}
				...
				elseif itemData["type"] == "weapon" then
					amount = 1
					info.serie = tostring(INDCore.Shared.RandomInt(2) .. INDCore.Shared.RandomStr(3) .. INDCore.Shared.RandomInt(1) .. INDCore.Shared.RandomStr(2) .. INDCore.Shared.RandomInt(3) .. INDCore.Shared.RandomStr(4))
					info.quality = 100
				...
				elseif itemData["name"] == "labkey" then
					info.lab = exports["IND-methlab"]:GenerateRandomLab()
				elseif itemData["name"] == "printerdocument" then
					info.url = "https://cdn.discordapp.com/attachments/870094209783308299/870104331142189126/Logo_-_Display_Picture_-_Stylized_-_Red.png"
				end

				if AddItem(id, itemData["name"], amount, false, info) then
					INDCore.Functions.Notify(source, Lang:t("notify.yhg") ..GetPlayerName(id).." "..amount.." "..itemData["name"].. "", "success")
				else
					INDCore.Functions.Notify(source,  Lang:t("notify.cgitem"), "error")
				end
			else
				INDCore.Functions.Notify(source,  Lang:t("notify.idne"), "error")
			end
	else
		INDCore.Functions.Notify(source,  Lang:t("notify.pdne"), "error")
	end
end, "admin")

INDCore.Commands.Add("randomitems", "Give Random Items (God Only)", {}, false, function(source, _)
	local filteredItems = {}
	for k, v in pairs(INDCore.Shared.Items) do
		if INDCore.Shared.Items[k]["type"] ~= "weapon" then
	...
	for _ = 1, 10, 1 do
		...
		if AddItem(source, randitem["name"], amount) then
			TriggerClientEvent('inventory:client:ItemBox', source, INDCore.Shared.Items[randitem["name"]], 'add')
            Wait(500)
		end
	end
end, "god")

INDCore.Commands.Add('clearinv', 'Clear Players Inventory (Admin Only)', { { name = 'id', help = 'Player ID' } }, false, function(source, args)
    local playerId = args[1] ~= '' and tonumber(args[1]) or source
    local Player = INDCore.Functions.GetPlayer(playerId)
    if Player then
        ClearInventory(playerId)
    else
        INDCore.Functions.Notify(source, "Player not online", 'error')
    end
end, 'admin')
