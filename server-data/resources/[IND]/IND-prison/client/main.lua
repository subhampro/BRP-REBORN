INDCore = exports['IND-core']:GetCoreObject() -- Used Globally
inJail = false
jailTime = 0
currentJob = nil
CellsBlip = nil
...
local function ApplyClothes()
	...
	if DoesEntityExist(playerPed) then
		Citizen.CreateThread(function()
			...
			if gender == 0 then
				TriggerEvent('IND-clothing:client:loadOutfit', Config.Uniforms.male)
			else
				TriggerEvent('IND-clothing:client:loadOutfit', Config.Uniforms.female)
			end
		end)
	end
end
...
RegisterNetEvent('INDCore:Client:OnPlayerLoaded', function()
	...
	INDCore.Functions.TriggerCallback('prison:server:IsAlarmActive', function(active)
		if active then
	...
	exports['IND-target']:AddTargetEntity(freedom_ped, {
		options = {
			{
				type = "client",
	...
	exports['IND-target']:AddTargetEntity(canteen_ped, {
		options = {
			{
				type = "client",
...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	if invokingResource and invokingResource ~= 'IND-policejob' and invokingResource ~= 'IND-ambulancejob' and invokingResource ~= GetCurrentResourceName() then
		-- Use INDCore.Debug here for a quick and easy way to print to the console to grab your attention with this message
		INDCore.Debug({('Player with source %s tried to execute prison:client:Enter manually or from another resource which is not authorized to call this, invokedResource: %s'):format(GetPlayerServerId(PlayerId()), invokingResource)})
		return
	end

	INDCore.Functions.Notify( Lang:t("error.injail", {Time = time}), "error")
	...
	Wait(2000)
	DoScreenFadeIn(1000)
	INDCore.Functions.Notify( Lang:t("error.do_some_work", {currentjob = Config.Jobs[currentJob] }), "error")
end)

RegisterNetEvent('prison:client:Leave', function()
	if jailTime > 0 then
		INDCore.Functions.Notify( Lang:t("info.timeleft", {JAILTIME = jailTime}))
	else
		jailTime = 0
		...
		RemoveBlip(ShopBlip)
		ShopBlip = nil
		INDCore.Functions.Notify(Lang:t("success.free_"))
		DoScreenFadeOut(500)
		while not IsScreenFadedOut() do
			Wait(10)
		end
	...
	while true do
		...
		if jailTime > 0 and inJail then
			...
			if jailTime > 0 and inJail then
				...
				if jailTime <= 0 then
					jailTime = 0
					INDCore.Functions.Notify(Lang:t("success.timesup"), "success", 10000)
				end
				TriggerServerEvent("prison:server:SetJailStatus", jailTime)
			end
		end
	...
	if not Config.UseTarget then
		...
		freedom:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['IND-core']:DrawText('[E] Check Time', 'left')
			else
				exports['IND-core']:HideText()
			end
		end)
		...
		canteen:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['IND-core']:DrawText('[E] Open Canteen', 'left')
			else
				exports['IND-core']:HideText()
			end
		end)
