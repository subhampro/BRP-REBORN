...
RegisterNUICallback('save', function(data, cb)
    INDCore.Functions.TriggerCallback('IND-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = PlayerPedId()
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            INDCore.Functions.Notify(Lang:t("error.tunerchip_vehicle_tuned"), 'error')

            TriggerServerEvent('IND-tunerchip:server:TuneStatus', INDCore.Functions.GetPlate(veh), true)
        end
        cb('ok')
    end, data)
end)

RegisterNetEvent('IND-tunerchip:client:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    if vehModel ~= 0 then
        INDCore.Functions.TriggerCallback('IND-tunerchip:server:GetStatus', function(status)
            if status then
                INDCore.Functions.Notify(Lang:t("success.this_vehicle_has_been_tuned"), 'success')
            else
                INDCore.Functions.Notify(Lang:t("error.this_vehicle_has_not_been_tuned"), 'error')
            end
        end, plate)
    else
        INDCore.Functions.Notify(Lang:t("error.no_vehicle_nearby"), 'error')
    end
end)

RegisterNUICallback('checkItem', function(data, cb)
    local retval = false
    local result = INDCore.Functions.HasItem(data.item)
    if result then
        retval = true
    end
    cb(retval)
...
RegisterNUICallback('reset', function(_, cb)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    INDCore.Functions.Notify(Lang:t("error.tunerchip_vehicle_has_been_reset"), 'error')
    cb("ok")
end)

RegisterNetEvent('IND-tunerchip:client:openChip', function()
    local ped = PlayerPedId()
    local inVehicle = IsPedInAnyVehicle(ped)

    if inVehicle then
        INDCore.Functions.Progressbar("connect_laptop", Lang:t("error.tunerchip_vehicle_has_been_reset"), 2000, false, true, {
        ...
        INDCore.Functions.Notify(Lang:t("error.you_are_not_in_a_vehicle"), "error")
    end
end)
