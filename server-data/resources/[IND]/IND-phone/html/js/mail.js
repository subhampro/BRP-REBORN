...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://IND-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://IND-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
$(document).on('click','#new-advert-photo',function(e){
    e.preventDefault();
    $.post('https://IND-phone/TakePhoto',function(url){
        if(url){
...
$(document).on('click','#adv-delete',function(e){
    e.preventDefault();
    $.post('https://IND-phone/DeleteAdvert', function(){
        setTimeout(function(){
