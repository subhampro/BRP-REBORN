...
local function hintToDisplay(text)
	exports['IND-core']:DrawText(text)
end
...
RegisterNetEvent('IND-armoredtruckheist:client:911alert', function()
	if PoliceAlert == 0 then
		...
		TriggerServerEvent("IND-armoredtruckheist:server:callCops", streetLabel, transCoords)
		PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
		PoliceAlert = 1
	end
end)

RegisterNetEvent('IND-armoredtruckheist:client:robberyCall', function(streetLabel, coords)
	if PlayerJob.name == "police" then
		local store = "Armored Truck"
		PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
		TriggerEvent('IND-policealerts:client:AddPoliceAlert', {
			timeOut = 10000,
			...
			callSign = INDCore.Functions.GetPlayerData().metadata["callsign"],
		})
		local transG = 250
		...
		while MissionStart == 1 do
			...
			if dist <= 55.0  then
				...
				if warning == 0 then
					warning = 1
					stopAndBeAngry()
					INDCore.Functions.Notify("Get rid of the guards before you place the bomb.", "error")
					AlertPolice()
				end
				...
				if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
					if BlowBackdoor == 0 then
						...
						if IsControlJustPressed(0, 47) then
							...
							TriggerEvent("IND-armoredtruckheist:client:911alert")
							hideLastHint()
							sleep = 500
						end
					end
	...
	if IsVehicleStopped(transport) then
		if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
			if not IsEntityInWater(PlayerPedId()) then
				...
				hideLastHint()
				INDCore.Functions.Notify('The load will be detonated in '..Config.TimeToBlow ..' seconds.', "error")
				...
				BlownUp = 1
				lootable = 1
				INDCore.Functions.Notify('You can start collecting cash.', "success")
				RemoveBlip(TruckBlip)
			else
				INDCore.Functions.Notify('Get out of the water', "error")
			end
		else
			INDCore.Functions.Notify('The vehicle must be empty to place the load', "error")
		end
	else
		INDCore.Functions.Notify('You can not rob a vehicle that is moving.', "error")
	end
end
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	INDCore.Functions.Notify('You are packing cash into a bag', "success")
