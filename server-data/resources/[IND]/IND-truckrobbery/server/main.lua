...
RegisterServerEvent('AttackTransport:akceptujto', function()
	...
	local xPlayer = INDCore.Functions.GetPlayer(_source)
	local accountMoney = xPlayer.PlayerData.money["bank"]
	if ActiveMission == 0 then
		if accountMoney < Config.ActivationCost then
			TriggerClientEvent('INDCore:Notify', _source, "You need " .. Config.Currency .. "" ..Config.ActivationCost.. " in the bank to accept the mission")
		else
			...
			if copsOnDuty >= Config.ActivePolice then
				...
				TriggerClientEvent('INDCore:Notify', _source, 'Need at least '..Config.ActivePolice.. ' police to activate the mission.')
			end
		end
	else
		TriggerClientEvent('INDCore:Notify', _source, 'Someone is already carrying out this mission')
	end
end)

RegisterServerEvent('IND-armoredtruckheist:server:callCops', function(streetLabel, coords)
    -- local place = "Armored Truck"
    -- local msg = "The Alarm has been activated from a "..place.. " at " ..streetLabel
	-- Why is this unused?
    TriggerClientEvent("IND-armoredtruckheist:client:robberyCall", -1, streetLabel, coords)
end)
...
RegisterServerEvent('AttackTransport:graczZrobilnapad', function()
	...
	TriggerClientEvent('inventory:client:ItemBox', _source, INDCore.Shared.Items['markedbills'], "add")

	local chance = math.random(1, 100)
	TriggerClientEvent('INDCore:Notify', _source, 'You took '..bags..' bags of cash from the van')

	if chance >= 95 then
		xPlayer.Functions.AddItem('security_card_01', 1)
		TriggerClientEvent('inventory:client:ItemBox', _source, INDCore.Shared.Items['security_card_01'], "add")
	end
	Wait(2500)
end)
