...
CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('IND-admin:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('IND-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        INDCore.Functions.Notify(Lang:t("success.blips_activated"), "success")
    else
        ShowBlips = false
        INDCore.Functions.Notify(Lang:t("error.blips_deactivated"), "error")
    end
end)

RegisterNetEvent('IND-admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        INDCore.Functions.Notify(Lang:t("success.names_activated"), "success")
    else
        ShowNames = false
        INDCore.Functions.Notify(Lang:t("error.names_deactivated"), "error")
    end
end)

RegisterNetEvent('IND-admin:client:Show', function(players)
