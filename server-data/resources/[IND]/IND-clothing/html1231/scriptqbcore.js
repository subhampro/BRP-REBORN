...
QBClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://IND-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e) {
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://IND-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://IND-clothing/setCurrentPed', JSON.stringify({ ped: newValue }), function(model) {
...
$(document).on('change', '.item-number', function() {
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://IND-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e) {
    ...
    if (selectedCam == null) {
        ...
        $.post('https://IND-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch (event.keyCode) {
        case 68: // D
            $.post('https://IND-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://IND-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
$(document).on('click', "#save-menu", function(e) {
    e.preventDefault();
    QBClothing.Close();
    $.post('https://IND-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e) {
    e.preventDefault();
    QBClothing.Close();
    $.post('https://IND-clothing/resetOutfit');
});
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e) {
    ...
    $.post('https://IND-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e) {
    ...
    $.post('https://IND-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
$(document).on('click', '#save-outfit-save', function(e) {
    ...
    $.post('https://IND-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e) {
    ...
    $.post('https://IND-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
