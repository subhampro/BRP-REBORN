...
function TakeOutVehicle(vehicleInfo)
    ...
    INDCore.Functions.TriggerCallback('INDCore:Server:SpawnVehicle', function(netId)
        ...
        TriggerServerEvent("inventory:server:addTrunkItems", INDCore.Functions.GetPlate(veh), Config.VehicleItems)
...
function MenuGarage()
    ...
    local Vehicles = Config.Vehicles[INDCore.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "IND-newsjob:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
function MenuHeliGarage()
    ...
    local Helicopters = Config.Helicopters[INDCore.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Helicopters) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "IND-newsjob:client:TakeOutHelicopters",
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('IND-newsjob:client:TakeOutVehicle', function(data)
    local vehicle = data.vehicle
    TakeOutVehicle(vehicle)
end)

RegisterNetEvent('IND-newsjob:client:TakeOutHelicopters', function(data)
    local vehicle = data.vehicle
